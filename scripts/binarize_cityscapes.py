...
def process_single_file(
    ...
    image_old_path = (
        cityscapes_images_path
        / city_name
        / f"{mask_file.stem.replace('gtFine_labelIds', 'leftImg8bit')}.png"
    )
    image_new_path = (
        images_dump_path / f"{image_old_path.stem.replace('_leftImg8bit', '')}.png"
    )
...
def process_common_split(
    ...
    cityscapes_images_path = cityscapes_path / "leftImg8bit" / split
    if not cityscapes_images_path.is_dir():
        raise FileNotFoundError(f"{cityscapes_images_path} is not a directory")

    cityscapes_annotations_path = cityscapes_path / "gtFine" / split
    mask_files = tuple(cityscapes_annotations_path.rglob("*gtFine_labelIds.png"))

    if not mask_files:
        raise FileNotFoundError(f"No masks found in {cityscapes_annotations_path}")

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_desc: dict[
            concurrent.futures.Future[bool], tuple[Label, Path, str]
        ] = {}
...
def main(
    ...
    common_args = (dump_path, cityscapes_path, max_workers)

    if split is not None:
        process_common_split(*common_args, split=split)
    else:
        for s in typing.get_args(SplitType):
            process_common_split(*common_args, split=s)

...
class SelectChunk:
    ...
    def _select_fit_memory_chunk_region(
        self, possible_chunk_regions, chunk_infos, mem_peak
    ):
        ...
        for region in possible_chunk_regions:
            ...
            cur_node_list, cur_region = self.reorder_graph.tmp_reorder(
                self.node_mgr.get_node_list(), cur_region
            )
            cur_chunk_infos = [*chunk_infos, cur_region]
            cur_mem = self.estimate_memory.estimate_chunk_inference_mem(
                cur_node_list, cur_chunk_infos
            )[0]
            cur_chunk_region_peak = cur_mem[
                cur_region["region"][0] : cur_region["region"][1] + 1
            ]
        ...
        # get max chunk size
        return self._get_fit_chunk_size(best_region, chunk_infos)
    ...
    def _select_min_memory_chunk_region(self, possible_chunk_regions, chunk_infos):
        ...
        for region in possible_chunk_regions:
            ...
            cur_chunk_region_peak = cur_mem_peak[
                max_possible_chunk_region[0] : max_possible_chunk_region[1] + 1
            ]
            cur_chunk_region_max_peak = max(cur_chunk_region_peak)

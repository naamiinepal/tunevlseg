...
def _gen_loop_start(
    chunk_input: list[Node],
    chunk_output: list[Node],
    chunk_output_dim: int,
    chunk_size=2,
) -> str:
    """
    ...
    for i in range(len(chunk_output)):
        ...
        if get_node_name(chunk_output[i]) in ["split", "unbind"]:
            ...
            context += f"{chunk_output[i].name} = {tensor_str};  "
        else:
            context += f"{chunk_output[i].name} = torch.empty({shape_str}, dtype={input_node.name}.dtype, device={input_node.name}.device);  "
...
def _gen_loop_end(
    ...
    for chunk_input in chunk_inputs + chunk_non_compute_inputs:
        if all(
            ...
            for user in chunk_input.users
        ):
            context += f";  {chunk_input.name} = None"
    for (
        chunk_output_non_tensor,
        chunk_output_non_tensor_val,
    ) in chunk_outputs_non_tensor.items():
...
def _replace_new_tensor_shape(
    ...
    if get_node_name(node) in ["ones", "zeros", "empty"]:
        ...
        new_shape[chunk_dim] = (
            "min(chunk_size, %d - chunk_idx)" % get_node_shape(meta_node)[chunk_dim]
        )
    ...
    for chunk_node_idx, chunk_node in enumerate(chunk_nodes[region_idx]):
        ...
        if isinstance(chunk_nodes_dim[region_idx][chunk_node_idx], dict):
            for idx, dim in chunk_nodes_dim[region_idx][chunk_node_idx].items():
                if idx == node_idx:
                    chunk_slice = _gen_chunk_slice_dim(
                        dim[0], "chunk_idx", get_node_shape(chunk_node)
                    )
                    body[-1] = _replace_name(
                        body[-1], chunk_node.name, chunk_node.name + chunk_slice
                    )
        # outputs node
        else:
            if chunk_node.name == node.name or (
                chunk_node.name in [i.name for i in node.all_input_nodes]
            ):
                chunk_slice = _gen_chunk_slice_dim(
                    chunk_nodes_dim[region_idx][chunk_node_idx],
                    "chunk_idx",
                ...
                if get_node_name(chunk_node) in ["split", "unbind"]:
                    ...
                    body[-1] = _replace_name(
                        body[-1], chunk_node.name, split_chunk_slice
                    )
                else:
                    body[-1] = _replace_name(
                        body[-1], chunk_node.name, chunk_node.name + chunk_slice
...
def emit_code_with_chunk(
    ...
    while node_idx < len(node_list):
        ...
        if within_chunk_region:
            ...
            body = _add_node_slice(
                chunk_inputs, region_idx, chunk_inputs_dim, node_idx, body, node
            )
            # replace output var with chunk var
            body = _add_node_slice(
                chunk_outputs, region_idx, chunk_outputs_dim, node_idx, body, node
            )
            # new tensor like
            body = _replace_new_tensor_like_shape(
                search_chunk, chunk_infos, region_idx, node_idx, node, body
            )
            # new tensor
            body = _replace_new_tensor_shape(
                search_chunk, chunk_infos, region_idx, node_idx, node, body
            )
            ...
            if eval_mem:
                body.append(
                    f"    if chunk_idx == 0:\n        print('{node.name}', torch.cuda.max_memory_allocated() / 1024**2 - init_memory);  torch.cuda.reset_peak_memory_stats()\n"
                )
        else:
            emit_node_func(node, body)
            ...
            if eval_mem:
                body.append(
                    f"print('{node.name}', torch.cuda.max_memory_allocated() / 1024**2 - init_memory);  torch.cuda.reset_peak_memory_stats()\n"
                )
...
if AUTOCHUNK_AVAILABLE:
    ...
    class AutoChunkCodeGen(CodeGen):
        def __init__(
            ...
            self.search_chunk = SearchChunk(
                meta_graph, max_memory, print_mem, print_progress
            )
        ...
        def _gen_python_code(
            self, nodes, root_module: str, namespace: _Namespace, verbose=None
        ) -> PythonCode:
            free_vars: list[str] = []
            body: list[str] = []
            ...
            def _format_args(
                ...
                def _get_repr(arg):
                    ...
                    if isinstance(arg, tuple) and hasattr(arg, "_fields"):
                        ...
                        return f"{global_name}{tuple(arg)!r}"
                    return repr(arg)
            ...
            def delete_unused_values(user: Node, body, to_keep=None):
                """
            ...
            def emit_node(node: Node, body):
                ...
                if node.op == "placeholder":
                    ...
                    maybe_default_arg = "" if not node.args else f" = {node.args[0]!r}"
                    free_vars.append(
                        f"{node.target}{maybe_type_annotation}{maybe_default_arg}"
                    )
                    raw_name = node.target.replace("*", "")
                    if raw_name != repr(node):
                        body.append(f"{node!r} = {raw_name}\n")
                    return
                if node.op == "call_method":
                    assert isinstance(node.target, str)
                    body.append(
                        f"{node!r}{maybe_type_annotation} = {_format_target(repr(node.args[0]), node.target)}"
                ...
                if node.op == "call_function":
                    ...
                    if (
                        ...
                        body.append(
                            f"{node!r}{maybe_type_annotation} = "
                    ...
                    if (
                        ...
                        body.append(
                            ...
                            f"{node!r}{maybe_type_annotation} = {node.args[0]!r}"
                        )
                        return
                    ...
                    if (
                        ...
                        body.append(
                            f"{node!r}{maybe_type_annotation} = {_format_target(repr(node.args[0]), node.args[1])}"
                        )
                        return
                    body.append(
                        f"{node!r}{maybe_type_annotation} = {global_name}({_format_args(node.args, node.kwargs)})"
                    )
                ...
                if node.op == "get_attr":
                    assert isinstance(node.target, str)
                    body.append(
                        f"{node!r}{maybe_type_annotation} = {_format_target(root_module, node.target)}"
                    )
                    return
                if node.op == "output":

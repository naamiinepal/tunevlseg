...
def is_non_compute_node(node: Node) -> bool:
    if any(i == node.op for i in NON_COMPUTE_OP) or any(
        i == get_node_name(node) for i in NON_COMPUTE_NAME
    ):
        return True
...
def find_chunk_compute_input_and_output_nodes(nodes: list[Node]) -> list:
    """
    Find non-compute input and output node names.
    input nodes are nodes used in the list
    output nodes are nodes will use nodes in the list
    ...
    if "_" in node_name:
        for i in range(len(node_name) - 1, -1, -1):
            if node_name[i] == "_":
                ...
                break
            if node_name[i] in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]:
                continue
            break
    return node_name
...
def find_tensor_shape_node(node_list: list[Node]) -> list[Node]:
    ...
    out = []
    for node in node_list:
        if get_node_shape(node) is not None or (

...
class TraceFlow:
    ...
    def check_index_source(self, start_dim, start_node, start_idx, end_dim, end_node):
        ...
        sorted_source = sorted(
            end_node_trace_source.items(), key=operator.itemgetter(0), reverse=True
        )
        ...
        if cur_node_dim is not None:
            ...
            if arg_idx not in cur_node_source[cur_node_dim]:
                ...
                # chunk shape should equal cur node
                    ...
                    if (
                        cur_node_dim is not None
                        and get_node_shape(cur_node)[cur_node_dim] != 1
                    ):
                        if (
                            get_node_shape(arg_node)[arg_dim]
                            != get_node_shape(cur_node)[cur_node_dim]
                        ):
                            return False
        else:
            arg_dim = None
        ...
        while len(cur_node_list) > 0:
            ...
            for cur_node in cur_node_list:
                ...
                for arg in cur_node.all_input_nodes:
                    ...
                    if flow_flag is False:
                        return None

            cur_node_list = next_node_list
        return all_node_info

    def _get_input_nodes_dim(
        self, inputs: list[Node], start_idx: int, end_idx: int, all_node_info: dict
    ) -> tuple:
        """
    ...
    def _get_prepose_nodes(
        self, all_node_info: dict, start_idx: int, end_idx: int, chunk_info
    ) -> list[Node]:
        """
        ...
        maybe_prepose_nodes.sort(
            key=self.node_mgr.find_node_idx,
            reverse=True,
            ...
            while len(tmp_cur_prepose_nodes) > 0:
                ...
                for cur_prepose_node in tmp_cur_prepose_nodes:
                    ...
                    for cur_prepose_node_arg in cur_prepose_node.all_input_nodes:
                        ...
                        if not (
                            start_idx
                            <= self.node_mgr.find_node_idx(cur_prepose_node_arg)
                            < end_idx
                        ):
                            continue
                        # compute op in loop
                        if cur_prepose_node_arg in all_node_info:
        ...
        # sort by index
        prepose_nodes.sort(key=self.node_mgr.find_node_idx)
    ...
    def flow_search(self, start_idx, start_dim, end_idx, end_dim):
        ...
        if len(find_tensor_shape_node(outputs)) > 1:
            chunk_info = self._get_other_output_info(
                outputs, start_idx, start_dim, end_idx, end_dim, chunk_info
            )
        ...
        return self._reassign_reshape_size(chunk_info)
    ...
    def _get_other_output_info(
        ...
        for output in outputs:
            ...
            if get_node_shape(output) is None:
                # log shape tensor
                if len(output.meta["fwd_out"]) > 0 and isinstance(
                    output.meta["fwd_out"][0], int
                ):
                    chunk_info["outputs_non_tensor"][output] = str(
                        output.meta["fwd_out"]
                    )
                continue
            # loop every dim of outputs, try to find a legal one
            for output_dim in range(len(get_node_shape(output))):
                if not self.check_region_start_end(
                    start_node, start_dim, start_idx, output, output_dim, output_idx
                ):
                    continue
                new_all_node_info = self._get_all_node_info(
                    output_dim, start_idx, output_idx
                )
                ...
                if (
                    self._update_chunk_info(
                        chunk_info, new_all_node_info, output, output_dim
                    )
                    is True
                ):
                    output_legal = True
                    break
            # not legal
            if output_legal is False:
                return None
        return chunk_info

    def _update_chunk_info(
        self, chunk_info: dict, new_all_node_info: dict, output: Node, output_dim: int
    ) -> bool:
        """
    ...
    def _reassign_reshape_size(self, chunk_info):
        ...
        chunk_shape = get_node_shape(chunk_info["outputs"][0])[
            chunk_info["outputs_dim"][0]
        ]
        for node in self.node_mgr.get_node_slice_by_idx(
            chunk_region[0], chunk_region[1] + 1
        ):
    ...
    def check_region_start_end(
        ...
        if not self.check_index_source(

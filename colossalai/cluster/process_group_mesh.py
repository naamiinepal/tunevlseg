...
class ProcessGroupMesh:
    ...
    def __init__(self, *size: int) -> None:
        ...
        assert (
            prod(size) == dist.get_world_size()
        ), "The product of the size must be equal to the world size."
        self._shape = size
    ...
    def coordinate(self, dim: int | None = None) -> int | tuple[int, ...]:
    ...
    @staticmethod
    def ravel(
        coord: tuple[int, ...], shape: tuple[int, ...], mode: str = "raise"
    ) -> int:
    ...
    def get_group_along_axis(
        ...
        coords_in_group = ProcessGroupMesh.get_coords_along_axis(
            self._coord, axis, indices_at_axis
        )
        ranks_in_group = tuple(

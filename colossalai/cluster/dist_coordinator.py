...
class DistCoordinator(metaclass=SingletonMeta):
    ...
    def __init__(self):
        assert dist.is_initialized(), "Distributed is not initialized. Please call `torch.distributed.init_process_group` or `colossalai.launch` first."
    ...
    @contextmanager
    def priority_execution(
        self, executor_rank: int = 0, process_group: ProcessGroup = None
    ):
        """

...
from colossalai.inference.kv_cache.batch_infer_state import (
    BatchInferState,
    MemoryManager,
)
...
class BaseSmoothForCausalLM(nn.Module, PushToHubMixin):
    ...
    def init_batch_state(self, max_output_len=256, **kwargs):
        ...
        if "max_total_token_num" in kwargs:
        ...
        block_loc = torch.empty(
            (batch_size, max_len_in_batch + max_output_len),
            dtype=torch.long,
            device="cuda",
        )
    ...
    def quantize(
        self,
        examples: list[dict[str, list[int] | torch.LongTensor]],
    ):
        if self.quantized:
            raise OSError("can't execute quantize because the model is quantized.")
    ...
    def collect_act_scales(
        self, model, tokenizer, dataset, device, num_samples=512, seq_len=512
    ):
    ...
    def collect_act_dict(
        self, model, tokenizer, dataset, act_dict, device, num_samples=512, seq_len=512
    ):
        pbar = tqdm(dataset)
        for text in pbar:
            input_ids = tokenizer(
    ...
    def save_quantized(
        ...
        safetensors_metadata: dict[str, str] | None = None,
    ):
        ...
        if is_local:
            ...
            for ext in extensions:
                resolved_archive_file = cached_file(
                    model_name_or_path, model_basename + ext, **cached_file_kwargs
                )

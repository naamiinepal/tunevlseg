...
try:
    from lightllm.models.chatglm2.triton_kernel.rotary_emb import (
        rotary_emb_fwd as chatglm2_rotary_emb_fwd,
    )
...
class ChatGLM2InferenceForwards:
    ...
    def chatglm_for_conditional_generation_forward(
        ...
        return_last_logit: bool | None = False,
        ...
        if stage_manager.is_first_stage():
            ...
            if position_ids is None:
                position_ids = get_position_ids(
                    batch_size, seq_length, input_ids.device
                )
            hidden_states = inputs_embeds
        else:
            assert (
                hidden_states is not None
            ), "hidden_states should not be None in non-first stage"
            seq_length, batch_size, _ = hidden_states.shape
            if position_ids is None:
                position_ids = get_position_ids(
                    batch_size, seq_length, hidden_states.device
                )
    ...
    def chatglm_encoder_forward(
        ...
        for idx, kv_cache in zip(range(start_idx, end_idx), kv_caches, strict=False):
        ...
        if self.post_layer_norm and (
            stage_manager.is_last_stage() or stage_manager.num_stages == 1
        ):
            # Final layer norm.
    ...
    def chatglm_flash_attn_kvcache_forward(
        ...
        assert (
            use_cache is True
        ), "use_cache should be set to True using this chatglm attention"
        ...
        if infer_state.is_context_stage:
            ...
            attn_output = torch.empty_like(
                query_layer.contiguous().view(-1, self.projection_size)
            )

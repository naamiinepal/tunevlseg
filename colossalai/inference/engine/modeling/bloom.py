...
def generate_alibi(n_head, dtype=torch.float16):
    ...
    def get_slopes(n):
        ...
        return slopes_power_of_2 + slopes_double[0::2][: n - closest_power_of_2]
...
class BloomInferenceForwards:
    ...
    def bloom_for_causal_lm_forward(
        ...
        **deprecated_arguments,
        ...
        return BloomInferenceForwards.bloom_model_forward(
            self.transformer,
            input_ids,
        ...
        **deprecated_arguments,
        ...
        if seq_length != 1:
            ...
            if alloc_mem is not None:
                ...
                print(
                    f"    infer_state.max_len_in_batch : {infer_state.max_len_in_batch}"
                )
        ...
        if attention_mask is None:
            attention_mask = torch.ones(
                (batch_size, infer_state.max_len_in_batch), device=hidden_states.device
            )
        else:
        ...
        alibi = (
            ...
            .contiguous()[
                curr_tp_rank * self.num_heads : (curr_tp_rank + 1) * self.num_heads
            ]
            .cuda()
        )
    ...
    def bloom_attention_forward(
        ...
        k = key_layer.reshape(
            -1, H, D_HEAD
        )  # batch_size * q_length, H, D_HEAD, q_lenth == 1
        v = value_layer.reshape(
            -1, H, D_HEAD
        )  # batch_size * q_length, H, D_HEAD, q_lenth == 1
        ...
        if infer_state.is_context_stage:
            ...
            copy_kv_cache_to_dest(
                k, infer_state.context_mem_index, mem_manager.key_buffer[layer_id]
            )
            copy_kv_cache_to_dest(
                v, infer_state.context_mem_index, mem_manager.value_buffer[layer_id]
            )
            ...
            if HAS_LIGHTLLM_KERNEL:
                lightllm_bloom_context_attention_fwd(
                    q, k, v, output, alibi, b_start_loc, b_seq_len, max_input_len
                )
            else:
                bloom_context_attn_fwd(
                    q, k, v, output, b_start_loc, b_seq_len, max_input_len, alibi
                )
            ...
            assert (
                q_length == 1
            ), "for non-context process, we only support q_length == 1"
            q = query_layer.reshape(-1, H, D_HEAD)
            ...
            if infer_state.decode_is_contiguous:
                ...
                copy_kv_cache_to_dest(
                    k, infer_state.decode_mem_index, mem_manager.key_buffer[layer_id]
                )
                copy_kv_cache_to_dest(
                    v, infer_state.decode_mem_index, mem_manager.value_buffer[layer_id]
                )
        ...
        ), "we do not support output_attentions at this time"

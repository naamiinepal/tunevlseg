...
class MicroBatchDescription:
    ...
    def state(self):
        ...
        if self.cur_length == self.target_length:
            return Status.DONE
        if self.cur_length == self.target_length - 1:
...
class HeadMicroBatchDescription(MicroBatchDescription):
    ...
    def __init__(
        ...
        assert (
            inputs_dict.get("input_ids") is not None
            and inputs_dict.get("attention_mask") is not None
        )
    ...
    def _update_attnmask(self):
        self.attn_mask = torch.cat(
            (
                self.attn_mask,
                torch.ones(
                    (self.attn_mask.shape[0], 1), dtype=torch.int64, device="cuda"
                ),
            ),
            dim=-1,
        )
    ...
    def add_description(self, inputs_dict: dict[str, torch.Tensor]):
        if self.stage == 0:
            self.mb_description_buffer[self.idx] = HeadMicroBatchDescription(
                ...
                self.max_input_len,
                self.max_output_len,
                self.cache_manager_list[self.idx],
            )
        else:

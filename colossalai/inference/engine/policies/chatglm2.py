...
class ChatGLM2InferPolicy(ChatGLMModelPolicy):
    ...
    def module_policy(self):
        ...
        self.append_or_create_method_replacement(
            description=method_replacement, policy=policy, target_key=ChatGLMModel
        )
        ...
        self.append_or_create_method_replacement(
            description=method_replacement, policy=policy, target_key=GLMBlock
        )

        attn_infer_forward = (
            ChatGLM2InferenceForwards.chatglm_flash_attn_kvcache_forward
        )
        ...
        if self.shard_config.enable_tensor_parallelism:
            policy[GLMBlock].attribute_replacement[
                "self_attention.num_multi_query_groups_per_partition"
            ] = (
                self.model.config.multi_query_group_num
    ...
    def get_held_layers(self) -> list[nn.Module]:
        ...
        if stage_manager.is_first_stage():
            held_layers.extend((module.embedding, module.output_layer))
        start_idx, end_idx = stage_manager.get_stage_index(layers_per_stage)
        held_layers.extend(module.encoder.layers[start_idx:end_idx])
        if stage_manager.is_last_stage() and module.encoder.post_layer_norm:

...
class InferenceEngine:
    ...
    def __init__(
        ...
        micro_batch_buffer_size: int | None = None,
        ...
        if quant == "gptq":
            ...
            self.gptq_manager = GPTQManager(
                model.quantize_config, max_input_len=max_input_len
            )
            model = model.model
        ...
        assert (
            model.__class__.__name__ in _supported_models
        ...
        assert dtype in [
            ...
            "fp32",
            "bf16",
        ], "dtype should be one of 'fp16', 'fp32', 'bf16'"

        assert max_batch_size <= 64, "Max batch size exceeds the constraint"
        assert (
            max_input_len + max_output_len <= 4096
        ), "Max length exceeds the constraint"
        assert quant in [
            "smoothquant",
            "gptq",
            None,
        ], "quant should be one of 'smoothquant', 'gptq'"
        ...
        if quant == "smoothquant" and dtype != "fp32":
            dtype = "fp32"
            logger.warning_once(
                "Warning: smoothquant only support fp32 and int8 mix precision. set dtype to fp32"
            )
        ...
        stage_manager = PipelineStageManager(pg_mesh, PP_AXIS, pp_size * tp_size > 1)
        ...
        self.model = self._shardformer(
            ...
            model_policy,
            stage_manager,
            pg_mesh.get_group_along_axis(TP_AXIS) if pp_size * tp_size > 1 else None,
        )
        if quant == "gptq":
            self.gptq_manager.post_init_gptq_buffer(self.model)

    def generate(self, input_list: list | dict):
        """
        Args:
    ...
    def _init_manager(
        self, model, max_batch_size: int, max_input_len: int, max_output_len: int
    ) -> None:
        ...
        if self.quant == "smoothquant":
            cache_manager = MemoryManager(
                max_total_token_num, torch.int8, head_num, head_dim, layer_num
            )
        else:
            cache_manager = MemoryManager(
                max_total_token_num, self.dtype, head_num, head_dim, layer_num
            )

...
class MixedPrecisionOptimizer(OptimizerWrapper):
    ...
    def zero_grad(self, *args, **kwargs):
        for p in self.working_to_master_map:
            p.grad = None
    ...
    def get_working_to_master_map(self) -> dict[int, torch.Tensor]:
        return {
            id(working_p): master_p
            for working_p, master_p in self.working_to_master_map.items()
        }

    def get_master_to_working_map(self) -> dict[int, torch.Tensor]:
        return {
            id(master_p): working_p
            for master_p, working_p in self.master_to_working_map.items()
        }

...
class ShardingStrategy:
    ...
    name: str
    sharding_specs: dict[OperationData, ShardingSpec | tuple[ShardingSpec]] = None
    ...
    def get_op_data_by_name(self, name: str):
        for op_data in self.sharding_specs:
    ...
    def get_sharding_spec_by_name(self, name: str):
        ...
        raise KeyError(
            f"Could not find the ShardingSpec for OperationData with name {name}"
        )
    ...
    def clone(self):
        ...
        sharding_specs = (
            _deepcopy_dict_vals(self.sharding_specs)
            if self.sharding_specs is not None
            else None
        )
        ...
        communication_actions = (
            _deepcopy_dict_vals(self.communication_actions)
            if self.communication_actions is not None
            else None
        )
        # same reason as communication_actions
        resharding_costs = (
            _deepcopy_dict_vals(self.resharding_costs)
            if self.resharding_costs is not None
            else None
        )

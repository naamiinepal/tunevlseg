...
__all__ = ["AddBMMFunctionHandler", "BMMFunctionHandler"]
...
def _get_data_mapping_for_bmm_op(node, input_idx, other_idx, bias_idx=None):
    ...
    physical_output = OperationData(
        name=str(node), type=OperationDataType.OUTPUT, data=node._meta_data
    )
    mapping = {
...
class AddBMMFunctionHandler(NodeHandler):
    ...
    def post_process(
    ...
    ) -> ShardingStrategy | list[ShardingStrategy]:
        ...
        if "bias" in op_data_mapping:
            ...
            bias_sharding_spec, removed_dims = (

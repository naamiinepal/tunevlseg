...
class WhereHandler(NodeHandler):
    ...
    def convert_physical_operand_to_logical_operand(
        self, physical_operand, target_shape
    ):
    ...
    def post_process(self, strategy: ShardingStrategy):
        logical_op_data_mapping, physical_op_data_mapping = (
            self.get_operation_data_mapping()
        )
        for key in logical_op_data_mapping:
            logical_sharding_spec = strategy.sharding_specs[
                logical_op_data_mapping[key]
            ]
            logical_shape = logical_op_data_mapping[key].logical_shape
            physical_shape = physical_op_data_mapping[key].logical_shape
            physical_sharding_spec, _removed_dims = (
            ...
            strategy.sharding_specs[physical_op_data_mapping[key]] = (

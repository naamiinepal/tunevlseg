...
__all__ = ["LinearFunctionHandler", "LinearModuleHandler"]
...
def _update_sharding_spec_for_transposed_weight_for_linear(
    ...
    assert (
        op_data.logical_shape[0] == op_data.data.shape[1]
        and op_data.logical_shape[1] == op_data.data.shape[0]
...
def _convert_logical_sharding_to_physical_sharding_spec_for_linear(
    ...
    if 0 in input_sharding_spec.dim_partition_dict:
        ...
        for i in range(num_input_dims - 1):
            ...
            input_sharding_spec = strategy_copy.get_sharding_spec_by_name(
                input_op_data.name
            )
            output_sharding_spec = strategy_copy.get_sharding_spec_by_name(
                output_op_data.name
            )
            try:
...
class LinearModuleHandler(MetaInfoModuleHandler):
    ...
    def get_operation_data_mapping(self) -> dict[str, OperationData]:
        ...
        output_logical_shape = output_meta_data.view(
            -1, output_meta_data.shape[-1]
        ).shape
    ...
    def post_process(
        ...
        strategy = _update_sharding_spec_for_transposed_weight_for_linear(
            strategy=strategy, weight_name="weight"
        )

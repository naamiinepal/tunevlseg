...
class SoftmaxHandler(NodeHandler):
    ...
    def get_strategy_generator(self) -> list[StrategyGenerator]:
        ...
        generators = []
        generators.append(
            SoftmaxGenerator(op_data_mapping, self.device_mesh, self.node.args[0])
        )
        return generators
    ...
    def get_operation_data_mapping(self) -> dict[str, OperationData]:
        ...
        physical_dim_operand = OperationData(
            name="softmax_dim", type=OperationDataType.ARG, data=softmax_dim
        )

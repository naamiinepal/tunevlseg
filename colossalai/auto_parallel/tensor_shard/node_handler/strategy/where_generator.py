...
class WhereGenerator(StrategyGenerator):
    ...
    def update_memory_cost(self, strategy: ShardingStrategy):
        ...
        total_mem_cost = MemoryCost(
            activation=fwd_activation_cost + bwd_activation_cost, parameter=0
        )
    ...
    def enumerate_all_possible_output_spec(
        self, mesh_dim_0, mesh_dim_1, dimension_length
    ):
        dim_partition_list = []
        dim_partition_list.extend(
            enumerate_all_possible_1d_sharding(mesh_dim_0, dimension_length)
        )
        dim_partition_list.extend(
            enumerate_all_possible_1d_sharding(mesh_dim_1, dimension_length)
        )
        dim_partition_list.extend(
            enumerate_all_possible_2d_sharding(mesh_dim_0, mesh_dim_1, dimension_length)
        )
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        dim_partition_list = self.enumerate_all_possible_output_spec(
            0, 1, dimension_length
        )

...
class BinaryElementwiseStrategyGenerator(StrategyGenerator):
    ...
    def validate(self) -> bool:
        ...
        for name, op_data in self.op_data.items():
            if not isinstance(op_data.data, torch.Tensor | int | float):
                raise TypeError(
                    f"The operation data {name} is not a torch.Tensor/int/float."
                )

    def update_compute_cost(self, strategy: ShardingStrategy) -> ShardingStrategy:
        shape = strategy.sharding_specs[
            self.op_data["input"]
    ...
    def update_memory_cost(self, strategy: ShardingStrategy) -> ShardingStrategy:
        ...
        total_memory_cost = MemoryCost(
            activation=input_bytes + other_bytes + output_bytes
        )
        memory_cost = TrainCycleItem(
            fwd=fwd_memory_cost, bwd=bwd_memory_cost, total=total_memory_cost
        )
    ...
    def enumerate_all_possible_output(self, mesh_dim_0, mesh_dim_1):
        ...
        sharding_list_2d = enumerate_all_possible_2d_sharding(
            mesh_dim_0, mesh_dim_1, dim_size
        )
        dim_partition_list.extend(sharding_list_2d)

        # enumerate all the 1D sharding cases
        sharding_list_1d_on_dim_0 = enumerate_all_possible_1d_sharding(
            mesh_dim_0, dim_size
        )
        dim_partition_list.extend(sharding_list_1d_on_dim_0)
        sharding_list_1d_on_dim_1 = enumerate_all_possible_1d_sharding(
            mesh_dim_1, dim_size
        )

...
class LayerNormGenerator(StrategyGenerator):
    ...
    def update_compute_cost(self, strategy: ShardingStrategy):
        ...
        sharded_input_shape = strategy.sharding_specs[
            self.op_data["input"]
        ].get_sharded_shape_per_device()
        sharded_weight_shape = strategy.sharding_specs[
            self.op_data["other"]
        ].get_sharded_shape_per_device()
        if self.has_bias:
        ...
        backward_compute_cost = (
            backward_activation_compute_cost + backward_weight_compute_cost
        )
        if self.has_bias:
        ...
        compute_cost = TrainCycleItem(
            fwd=forward_compute_cost,
            bwd=backward_compute_cost,
    ...
    def update_memory_cost(self, strategy: ShardingStrategy):
        ...
        fwd_mem_cost = MemoryCost(
            activation=fwd_activation_cost, parameter=fwd_parameter_cost
        )
        ...
        bwd_mem_cost = MemoryCost(
            activation=bwd_activation_cost, parameter=bwd_parameter_cost
        )
        ...
        memory_cost = TrainCycleItem(
            fwd=fwd_mem_cost, bwd=bwd_mem_cost, total=total_mem_cost
        )
    ...
    def split_input_batch_single_mesh_dim(self, mesh_dim_0, batch_dimension_length):
        strategy_list = []
        dim_partition_list = enumerate_all_possible_1d_sharding(
            mesh_dim_0, batch_dimension_length
        )
    ...
    def split_input_batch_both_mesh_dim(
        self, mesh_dim_0, mesh_dim_1, batch_dimension_length
    ):
        strategy_list = []
        dim_partition_list = enumerate_all_possible_2d_sharding(
            mesh_dim_0, mesh_dim_1, batch_dimension_length
        )
        for dim_partition in dim_partition_list:
            strategy = self._generate_strategy_with_dim_partition(dim_partition)
            strategy_list.append(strategy)
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        strategy_list.extend(
            self.split_input_batch_single_mesh_dim(0, batch_dimension_length)
        )
        strategy_list.extend(
            self.split_input_batch_single_mesh_dim(1, batch_dimension_length)
        )

        # SR = SR x R with both mesh dims on batch dimensions
        strategy_list.extend(
            self.split_input_batch_both_mesh_dim(0, 1, batch_dimension_length)
        )

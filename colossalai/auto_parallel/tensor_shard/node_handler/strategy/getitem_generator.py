        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            try:
                ...
                dim_partition_dict_for_input = copy.deepcopy(
                    strategy.output_sharding_specs[
                        self.op_data["input"]
                    ].dim_partition_dict
                )
                ...
                dim_partition_dict_for_output = copy.deepcopy(
                ...
                if int_index:
                    ...
                    for dim, mesh_dim_list in dim_partition_dict_for_output.items():
                        shift_dim_partition_dict_for_output[dim - shift_length] = (
                            mesh_dim_list
                        )
...
class TensorTupleStrategyGenerator(GetItemStrategyGenerator):
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        for strategy_index, strategy in enumerate(
            self.predecessor_node.strategies_vector
        ):
            # the sharding spec for input in this case is a tuple of ShardingSpec.
            sharding_spec_for_input = strategy.output_sharding_specs[
                self.op_data["input"]
            ]
            dim_partition_dict_for_output = sharding_spec_for_input[
                index
            ].dim_partition_dict
            dim_partition_dict_mapping = {}
            communication_action_mapping = {}

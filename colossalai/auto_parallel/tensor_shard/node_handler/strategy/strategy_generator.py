...
class StrategyGenerator(ABC):
    ...
    def __init__(
        self, operation_data_mapping: dict[str, OperationData], device_mesh: DeviceMesh
    ):
    ...
    def get_sharding_strategy(
        ...
        communication_actions = self.replace_op_name_with_op_data(
            ...
            if op_data_name in self.op_data:
                ...
                def _to_sharding_spec(
                    ...
                    if isinstance(data, list | tuple):
                        ...
                        ) in zip(data, logical_shape, dim_partition_dict, strict=False):
                ...
                sharding_spec = _to_sharding_spec(
                    op_data.data, op_data.logical_shape, dim_partition_dict
                )
    ...
    def get_communication_spec(
        ...
        logical_process_axis: int | list[int],
    ):
        """
        A factory method to produce a CommSpec object.
        """
        return CommSpec(
            comm_pattern=communication_pattern,
        ...
        if strategy.communication_actions is not None:
            for operand, comm_action in strategy.communication_actions.items():
                ...
                if isinstance(comm_spec, dict):
                    ...
                    _, comm_action_sequence, _ = (
                        shape_consistency_manager.shape_consistency(src_spec, tgt_spec)
                    )
    ...
    def _compute_size_in_bytes(self, strategy: ShardingStrategy, key: str):
        ...
        if isinstance(op_data.data, tuple):
            ...
            if isinstance(op_data.data, torch.Tensor):
                total_bytes = _compute_size_in_bytes_helper(
                    strategy.sharding_specs[op_data], op_data.data
                )
            else:

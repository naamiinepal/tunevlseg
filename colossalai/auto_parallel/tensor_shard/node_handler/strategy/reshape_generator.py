...
from colossalai.auto_parallel.tensor_shard.node_handler.strategy.strategy_generator import (
...
class ViewGenerator(ReshapeGenerator):
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            ...
            keep_sharding_status = check_keep_sharding_status(
            ...
            if keep_sharding_status:
                ...
                if len(total_mesh_dim_list) == 1:
                    ...
                    shard_dim = next(iter(dim_partition_dict_for_input.keys()))
                    ...
                    input_comm_action = CommAction(
                        comm_spec=comm_spec, comm_type=CommType.BEFORE, arg_index=0
                    )
        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            ...
            for dim_index, permute_dim in enumerate(permute_dims):
                if permute_dim in dim_partition_dict_for_input:
                    dim_partition_dict_for_output[dim_index] = (
                        dim_partition_dict_for_input[permute_dim]
                    )
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            ...
            for dim, sharded_dims in dim_partition_dict_for_input.items():
                if dim == dim_0:
                    dim_partition_dict_for_output[dim_1] = dim_partition_dict_for_input[
                        dim_0
                    ]
                elif dim == dim_1:
                    dim_partition_dict_for_output[dim_0] = dim_partition_dict_for_input[
                        dim_1
                    ]
                else:
...
class SplitGenerator(ReshapeGenerator):
    ...
    def collate_strategies(self) -> list[ShardingStrategy]:
        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            ...
            _split_size, split_dim = self.op_data["split_info"].data

            if split_dim in dim_partition_dict_for_input:
                recover_dims = dim_partition_dict_for_input.pop(split_dim)

            dim_partition_dict_for_output = [
                copy.deepcopy(dim_partition_dict_for_input)
                for _ in range(len(self.op_data["output"].data))
            ]

...
class SumGenerator(FollowingStrategyGenerator):
    ...
    def update_compute_cost(self, strategy: ShardingStrategy):
        ...
        sharded_output_shape = strategy.sharding_specs[
            self.op_data["output"]
        ].get_sharded_shape_per_device()
        ...
        for index, strategy in enumerate(self.predecessor_node.strategies_vector):
            ...
            for dim in dim_partition_dict_for_input:
                if dim in sum_dims:
                    ...
                    dim_partition_dict_for_output[sum_mapping_dict[dim]] = (
                        dim_partition_dict_for_input[dim]
                    )
                else:
                    raise RuntimeError(
                        f"dim {dim} is not in sum_mapping_dict or sum_dims"
                    )

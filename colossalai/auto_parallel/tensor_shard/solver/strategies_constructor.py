...
class StrategiesConstructor:
    ...
    def __init__(
        self, graph: Graph, device_mesh: DeviceMesh, solver_options: SolverOptions
    ):
        self.graph = graph
        assert (
    ...
    def generate_alias_set(self):
        ...
        common_blocks = find_repeat_blocks(
            node_list, self.root_module, common_length_threshold=10
        )
        ...
        for index, common_node in enumerate(common_blocks[0]):
            for i in range(1, repeat_block_nums):
                alias_set[node_list.index(common_blocks[i][index])] = node_list.index(
                    common_node
                )
        return alias_set
    ...
    def build_strategies_and_cost(self):
        ...
        for node in self.nodes:
            ...
            # placeholder node
                ...
                if (
                    self.solver_options.dataloader_option
                    == DataloaderOption.DISTRIBUTED
                ):
                    placeholder_option = "distributed"
                else:
                    assert (
                        self.solver_options.dataloader_option
                        == DataloaderOption.REPLICATED
            ...
            # call_module node
                ...
                if hasattr(handler, "strategies_info"):
                    node.strategies_info = handler.strategies_info
            ...
            node.strategies_vector = strategies_vector

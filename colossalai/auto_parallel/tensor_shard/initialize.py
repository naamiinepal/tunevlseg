...
from colossalai.auto_parallel.passes.runtime_preparation_pass import (
    runtime_preparation_pass,
)
from colossalai.auto_parallel.tensor_shard.options import (
    DataloaderOption,
    ShardOption,
    SolverOptions,
    SolverPerference,
...
def extract_meta_args_from_dataloader(
    data_loader: torch.utils.data.DataLoader, data_process_func: callable
):
    """
...
def extract_alpha_beta_for_device_mesh(
    alpha_beta_dict: dict[tuple[int], tuple[float]], logical_mesh_shape: tuple[int]
):
    """
...
def build_strategy_constructor(
    ...
    solver_options = SolverOptions(
        solver_perference=solver_preference,
...
def solve_solution(
    gm: ColoGraphModule,
    strategy_constructor: StrategiesConstructor,
    ...
    solver = Solver(
        gm.graph, strategy_constructor, cost_graph, memory_budget=memory_budget
    )
...
def transform_to_sharded_model(
    ...
    """
    gm, sharding_spec_dict, origin_spec_dict, comm_actions_dict = (
        runtime_preparation_pass(
    ...
    shape_prop_pass(
        gm, *meta_args.values(), sharding_spec_dict, origin_spec_dict, comm_actions_dict
    )
    gm.recompile()
...
def initialize_device_mesh(
    world_size: int = -1,
    physical_devices: list[int] | None = None,
    alpha_beta_dict: dict[tuple[int], tuple[float]] | None = None,
    logical_mesh_shape: tuple[int] | None = None,
    ...
    if alpha_beta_dict is None:
        ...
        ab_profiler = AlphaBetaProfiler(
            physical_devices, alpha_beta_dict=alpha_beta_dict
        )
    ...
    if logical_mesh_shape is None and logical_mesh_id is None:
        ...
        # extract alpha and beta values for the chosen logical mesh shape
        mesh_alpha, mesh_beta = extract_alpha_beta_for_device_mesh(
            alpha_beta_dict, logical_mesh_id
        )
...
def autoparallelize(
    ...
    save_solver_solution: bool = False,
    load_solver_solution: bool = False,
    solver_solution_path: str | None = None,
    return_solution: bool = False,
    memory_budget: float = -1.0,
):
    """

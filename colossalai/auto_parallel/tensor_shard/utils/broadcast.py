...
def is_broadcastable(shape1: torch.Size, shape2: torch.Size) -> bool:
    ...
    """
    for s1, s2 in zip(shape1[::-1], shape2[::-1], strict=False):
...
def get_broadcast_shape(shape1: torch.Size, shape2: torch.Size) -> list[int]:
    ...
    assert is_broadcastable(
        shape1, shape2
    ), f"{shape1} and {shape2} are not broadcastable"
    shape1_reverse = shape1[::-1]
    shape2_reverse = shape2[::-1]
    min_common_dim = min(len(shape1), len(shape2))
    dims = []
    for s1, s2 in zip(shape1_reverse, shape2_reverse, strict=False):
...
def recover_sharding_spec_for_broadcast_shape(
    logical_sharding_spec: ShardingSpec,
    ...
    for shape_dim, mesh_dim in logical_dim_partition.items():
        ...
        if (
            logical_broadcast_type == BroadcastType.PADDING
            or logical_broadcast_type == BroadcastType.MULTIPLE
        ):

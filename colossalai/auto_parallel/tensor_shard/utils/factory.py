...
__all__ = ["generate_resharding_costs", "generate_sharding_spec"]


def generate_sharding_spec(
    input_: Node | torch.Tensor,
    ...
    for dim_index, sharding_index_list in dim_partition_dict.items():
        sharding_list = [
            device_mesh.mesh_shape[sharding_index]
            for sharding_index in sharding_index_list
        ]
...
def generate_resharding_costs(
    ...
    for input_node, input_spec in zip(nodes, sharding_specs, strict=False):
        resharding_costs[input_node] = []
        for strategy in input_node.strategies_vector:
            input_sharding_spec = strategy.output_sharding_spec
            if not isinstance(input_sharding_spec, ShardingSpec):
                assert isinstance(
                    input_sharding_spec, list
                ), "only ShardingSpec or List[ShardingSpec] is expected."
...
def find_repeat_blocks(
    node_list: list[torch.fx.Node], root_module, common_length_threshold: int = 20
):
    """
    ...
    def _all_equal(check_list, check_fn):
        ...
        return all(check_fn(e, base_value) for e in check_list)

    def _check_node_list_equal(l1, l2):
        if len(l1) != len(l2):
            return False
        for node1, node2 in zip(l1, l2, strict=False):
    ...
    for index, node in enumerate(node_list):
        ...
        hash_key = (node.op, target, *new_args) if node.op != "get_attr" else (node.op,)
    ...
    for index, node in enumerate(node_list):
        ...
        if len(hash_value_to_node_dict[hash(node.hash_key)]) >= 2:
            ...
            check_block_list = [
                node_list[start : start + max_common_length]
            ...
            if common_label:
                ...
                max_step = (
                    len(node_list) - common_blocks_index[-1] - max_common_length - 1
                )

                for i in range(max_step):
                    # add assertion to avoid out of index
                    next_node_list = [
                        node_list[index + max_common_length + i]

...
class TrainingSimulator(ABC):
    ...
    def __init__(
        ...
        comp_power: float,
        link_to_bw: dict[str, dict[float, float]],
    ) -> None:
...
class AsynTrainingSimulator(TrainingSimulator):
        ...
        for reg in self.region_list:
            if reg.param_size and reg.r_id < self.region_num - 1:
                for nr in self.region_list[reg.r_id + 1 :]:
                    if nr.param_size and requires_upload_p_in_fwd(
                        self.region_list[nr.shared_rid]
                    ):
    ...
    def _insert_h2d_exec(self, region: Region, is_fwd: bool = True):
        ...
        pref_end_time = pref_start_time + 2.0 * self._get_communication_overhead(
            "h2d", region.param_size
        )
    ...
    def _insert_comp_exec(self, region: Region, is_fwd: bool = True):
        ...
        if is_fwd:
            ...
            flop_key = "bwd_flop"
        comp_start_time = max(
            self.last_comp.end_time,
            reg_to_pref.get(region.r_id, ExecutionPeriod(0, 0)).end_time,
        )
    ...
    def _insert_d2h_exec(self, region: Region):
        ...
        offl_end_time = offl_start_time + self._get_communication_overhead(
            "d2h", region.param_size
        )
    ...
    def _eval_fwd_cost_per_region(self, region: Region):
        ...
        if fwd_prefetch_region and requires_upload_p_in_fwd(
            self.region_list[fwd_prefetch_region.shared_rid]
        ):

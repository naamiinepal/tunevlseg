...
def compute_act_peak_mem(region_list: list[Region]) -> float:
    ...
    for region in region_list:
        for node in region.nodes:
            runtime_mem = (
                runtime_mem + calculate_fwd_tmp(node) + calculate_fwd_out(node)
            )
    ...
    for region in region_list.__reversed__():
        for node in region.nodes.__reversed__():
            ...
            runtime_mem = (
                runtime_mem + node.meta["bwd_mem_tmp"] + node.meta["bwd_mem_out"]
            )

            act_peak_mem = max(runtime_mem, act_peak_mem)

            runtime_mem = (
                runtime_mem - node.meta["bwd_mem_tmp"] - calculate_fwd_tmp(node)
            )

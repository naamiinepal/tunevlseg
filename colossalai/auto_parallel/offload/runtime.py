...
def convert_fwd_upload_bwd_offload_to_action(tensor, fwd_info, bwd_info):
    ...
    with torch._C.DisableTorchFunction():
        return SynPreFwdPostBwdOP.apply(tensor, fwd_info, bwd_info)
...
def convert_fwd_prefetch_bwd_offload_to_action(tensor, fwd_info, bwd_info):
    ...
    """
    with torch._C.DisableTorchFunction():
        return AsynPreFwdPostBwdOP.apply(tensor, fwd_info, bwd_info)


def replace_node_users(
    orig_node: Node, inserted_node: Node, rep_user_nodes: list[Node] | None = None
):
...
def runtime_syn_offload_apply_pass(gm: torch.fx.GraphModule, region_list: list[Region]):
    ...
    """
    mod_graph = gm.graph
    last_inp_node = next(iter(mod_graph.nodes))
...
def runtime_asyn_offload_apply_pass(
    gm: torch.fx.GraphModule, region_list: list[Region]
):
    """
    ...
    last_inp_node = next(iter(mod_graph.nodes))
    first_region_with_p = next(region for region in region_list if region.param_size)
    ...
    for r_idx, region in enumerate(region_list):
        ...
        if fwd_prefetch_region and requires_upload_p_in_fwd(
            region_list[fwd_prefetch_region.shared_rid]
        ):

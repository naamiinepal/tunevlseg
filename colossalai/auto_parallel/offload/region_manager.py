...
class RegionManager:
    ...
    def __init__(
        ...
        cnode: list[str] | None = None,
    ):
        self.graph = graph
        assert (
            graph.owning_module is not None
        ), "The given graph is not associated with a owning_module"
        self.root_module = self.graph.owning_module
        self.nodes = list(graph.nodes)
        self.cnode = cnode
        self.only_param_ops = []
        self.param_region_map: dict[torch.nn.Parameter, Region] = {}
        ...
        self.reg_to_block: dict[int, int] = {}
    ...
    def _pre_process(self):
        ...
        if len(self.shared_region_pairs) == 1:
            ...
            assert (
                shared_regs[0].shared_rid == shared_regs[1].r_id
        ...
        self.memory_budget -= (
            self.max_param_num * torch.tensor([], dtype=torch.float32).element_size()
        )
    ...
    def _early_region_placement(self, ts: TrainingSimulator):
        ...
        for reg in self.region_list:
            if reg.r_id in self.rid_in_pool:
                ...
                cur_reg_coexists = torch.sum(
                    coexist_matrix[cur_reg_appears], dim=0
                ).bool()
                for block_idx in range(mem_block_num):
        ...
        self.memory_pool = torch.chunk(
            torch.zeros(
                int(mem_block_num * self.mem_block_size / 2),
                dtype=torch.half,
                device="cuda",
            ),
    ...
    def _search_block_size(
        ...
        search_range_byte: int = 128 * 1024**2,
    ) -> int:
        """
    ...
    def _init_region_data(self):
        ...
        self.temp_fp32_data = torch.zeros(
            self.max_param_num, device="cuda", dtype=torch.float32
        )
    ...
    def _process_shared_region(self):
        ...
        if len(self.shared_region_pairs):
            ...
            assert embedding_node.op == "call_module" and isinstance(
                self.root_module.get_submodule(embedding_node.target),
                torch.nn.Embedding,
            )
            if latter_reg.param_num > former_reg.param_num:
                for idx, n in enumerate(latter_reg.nodes):
                    if (
                        n.op == "call_module"
                        and isinstance(
                            self.root_module.get_submodule(n.target), torch.nn.Linear
                        )
                    ) or (
                        n.op == "call_function"
                        and n.target is torch.nn.functional.linear
                    ):
                        cut_node_idx = idx + 1
                        break
    ...
    def _linearize_graph(self) -> list[Region]:
        ...
        def _is_param_comp_end() -> bool:
            ...
            if n.op == "call_module":
                ...
                label = any(
                    x.name in self.only_param_ops for x in n.all_input_nodes
                ) and any(
                    x.name not in self.only_param_ops and not _is_cop(n.target)
                ...
                if (
                    ...
                    == len(
                        [
                            ...
                            if node.name in self.only_param_ops
                        ]
                    )
                    or n.op == "get_attr"
                ):
    ...
    def _set_node_and_region_info(self, node_id: int, cur_n: Node, cur_reg: Region):
        ...
        if cur_n.op == "call_module":
            ...
            if isinstance(attr_itr, torch.nn.Parameter):
                ...
                cur_reg.param_size += (
                    attr_itr.data.numel() * attr_itr.data.element_size()
                )
    ...
    def __update_param_region_map(
        self, params: list[torch.nn.Parameter], region: Region
    ):
        for p in params:

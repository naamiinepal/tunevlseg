...
def batchnormnd_meta_info(
    *args, **kwargs
    ...
    # calculate cost
    fwd_compute_cost = flop_mapping[torch.ops.aten.cudnn_batch_norm.default](
        fwd_in_args, fwd_out_args
    )
    bwd_compute_cost = flop_mapping[torch.ops.aten.cudnn_batch_norm_backward.default](
    ...
    fwd_buffer = [
        torch.zeros_like(mean_tensor, device="meta"),
        torch.zeros_like(var_tensor, device="meta"),
    ]
...
def layernorm_meta_info(
    *args, **kwargs
    ...
    # compute cost
    fwd_compute_cost = flop_mapping[torch.ops.aten.native_layer_norm.default](
        fwd_in_args, fwd_out_args
    )
    bwd_compute_cost = flop_mapping[torch.ops.aten.native_layer_norm_backward.default](
        bwd_in_args, bwd_out_args
    )
    compute_cost = TrainCycleItem(
        fwd=fwd_compute_cost,
    ...
    fwd_in = [torch.zeros_like(input_tensor, device="meta")]
    fwd_buffer = [
        torch.zeros_like(running_mean, device="meta"),
        torch.zeros_like(running_var, device="meta"),
    ]
    fwd_out = [torch.zeros_like(output_tensor, device="meta")]

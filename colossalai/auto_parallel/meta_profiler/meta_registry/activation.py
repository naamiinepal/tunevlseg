...
from colossalai._analyzer._subclasses.flop_tensor import (
    ewise_flop_counter as elementwise_flop_counter,
)
...
def elementwise_meta_info(
    temp_mem_scale: float = 0, buffer_mem_scale: float = 0
) -> Callable:
    ...
    def meta_func(
        *args, **kwargs
        ...
        input_tensor = next(
            filter(
                lambda x: (
                    x.type == OperationDataType.ARG or x.type == OperationDataType.PARAM
                )
        ...
        bwd_memory_cost = MemoryCost(
            activation=activation_size(input_tensor)
            - activation_size(input_tensor) * buffer_mem_scale,
            parameter=0,
...
# (0, 0)
meta_register.register([torch.nn.ReLU, torch.nn.functional.relu, torch.tanh])(
    elementwise_meta_info(0, 0)
)

# (1, 0)
meta_register.register([torch.nn.Softmax, torch.nn.functional.softmax])(
    elementwise_meta_info(1, 0)
)

# (0, 0.25) for dropout, the buffer is in bool type so that the buffer memory cost is 0.25 times of input tensor
meta_register.register([torch.nn.Dropout, torch.nn.functional.dropout])(
    elementwise_meta_info(0, 0.25)
)

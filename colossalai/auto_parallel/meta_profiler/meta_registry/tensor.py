...
def tensor_related_metainfo(
    bwd_mem_out_factor: float = 1, bwd_mem_tmp_factor: float = 0
) -> Callable:
    ...
    def meta_func(
        *args, **kwargs
        ...
        fwd_mem_cost = MemoryCost(
            activation=compute_size_in_bytes(outputs) * 2, parameter=0, temp=0, buffer=0
        )
...
# (1, 1)
meta_register.register([torch.Tensor.type, torch.Tensor.contiguous])(
    tensor_related_metainfo(1, 1)
)

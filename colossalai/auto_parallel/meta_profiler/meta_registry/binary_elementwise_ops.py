...
def binary_elementwise_meta_info(
    *args, **kwargs
    ...
    output_op_data = next(
        filter(lambda arg: arg.type == OperationDataType.OUTPUT, args)
    )
    ...
    fwd_compute_cost = flop_mapping[torch.ops.aten.add.Tensor](
    ...
    param_mem_cost = activation_size(
        [arg.data for arg in input_op_data if arg.type == OperationDataType.PARAM]
    )

...
class ShardMetaInfo:
    ...
    def __init__(
        self, strategy: ShardingStrategy = None, target: Callable | None = None
    ) -> None:
    ...
    def compute_sharded_opdata(
        self, operation_data: OperationData, sharding_spec: ShardingSpec
    ):
        """
        ...
        if isinstance(sharding_spec, ShardingSpec):
            op_data = OperationData(
                ...
                data=torch.zeros(
                    sharding_spec.get_sharded_shape_per_device(), device="meta"
                ),
            ...
            assert isinstance(
                data, list | tuple
            ), f"Data Should be list or tuple, but got {type(data)}."
            assert len(data) == len(
                sharding_spec
            ), "Length of data and sharding spec should be the same."
            sharded_data = []
            for _d, s in zip(data, sharding_spec, strict=False):
                sharded_data.append(
                    torch.zeros(s.get_sharded_shape_per_device(), device="meta")
                )
            op_data = OperationData(
                name=operation_data.name, data=sharded_data, type=operation_data.type
            )
        else:
            raise ValueError(
                f"Sharding spec should be ShardingSpec or list, but got {type(sharding_spec)}."
            )
    ...
    def compute_shard_metainfo(self):
        ...
        args = list(
            starmap(self.compute_sharded_opdata, self._strategy.sharding_specs.items())
        )

...
class MetaInfoProp:
    ...
    def _is_inplace(self, node: Node):
        ...
        if node.op == "call_module":
            return (
                node.graph.owning_module.get_submodule(node.target).__class__
                in OUTPUT_SAVED_MOD
            )
    ...
    def node_handler(self, node: Node) -> None:
        ...
        """
        assert hasattr(
            node, "best_strategy_info"
        ), f"Cannot find best_strategy_info in node {node}, {node.op}"
        graph_info = GraphInfo()
        ...
        if self._is_inplace(node):
            ...
            parent_node = next(iter(node._input_nodes.keys()))
                ...
                for tensor in par.meta.get("fwd_out", []):
                    ...
                    target_input_tensor = next(
                        (
                            ...
                            if not x.data_ptr() and x.shape == tensor.shape
                        ),
                        None,
                    )

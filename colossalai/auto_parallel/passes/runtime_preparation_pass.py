...
from colossalai.auto_parallel.tensor_shard.solver.strategies_constructor import (
...
def solution_annotation_pass(
    ...
    nodes = [
        strategies_vector.node
        for strategies_vector in strategies_constructor.leaf_strategies
    ]
    no_strategy_nodes = strategies_constructor.no_strategy_nodes

    # the dict to get origin sharding spec of node
    origin_node_sharding_spec_dict = {}
    for node_index, (node, strategy_index) in enumerate(
        zip(nodes, solution, strict=False)
    ):
        strategies_vector = node.strategies_vector
        # stick the solution strategy to the corresponding node
        node.best_strategy = strategies_vector[strategy_index]
        node.sharding_spec = strategies_vector[
            strategy_index
        ].get_sharding_spec_by_name(str(node))
        origin_node_sharding_spec_dict[node_index] = strategies_vector[
            strategy_index
        ].get_sharding_spec_by_name(str(node))

        # attach the corresponding metainfo if node has the attribute `strategies_info`
        if hasattr(node, "strategies_info"):
            node.best_strategy_info = node.strategies_info[strategy_index]
    ...
    for index, node in enumerate(nodes):
        ...
        for user_node in node.strategies_vector.successor_nodes:
            if user_node in no_strategy_nodes:
                target_sharding_spec = node.best_strategy.get_sharding_spec_by_name(
                    str(node.name)
                )
            else:
                target_sharding_spec = (
                    user_node.best_strategy.get_sharding_spec_by_name(str(node.name))
                )
            target_sharding_specs.append(target_sharding_spec)
        sharding_spec_convert_dict[index] = target_sharding_specs
        node.target_sharding_specs = target_sharding_specs
        ...
        if node.op == "get_attr":
            ...
            if (
                ...
                and target_node.target in RESHAPE_FUNC_OP
            ):
            ...
            if op_data_in_user in user_strategy.communication_actions:
                new_communication_action = user_strategy.communication_actions.pop(
                    op_data_in_user
                )
    ...
    for node in nodes:
        if node.op != "placeholder":
            with mod_graph.inserting_before(node):
                mod_graph.create_node(
                    "placeholder", target="sharding_spec_convert_dict"
                )
                mod_graph.create_node(
                    "placeholder", target="origin_node_sharding_spec_dict"
                )
                mod_graph.create_node("placeholder", target="comm_actions_dict")
            break
    return (
        gm,
        sharding_spec_convert_dict,
...
def node_args_converting_pass(gm: torch.fx.GraphModule, device_mesh: DeviceMesh):
    ...
    for node in nodes:
        ...
        output_dim_partition_dict, device_mesh = _extract_info_from_sharding_spec(
            node.sharding_spec
        )
        if _filter_node_with_shape_args(node):
            _scale_args_adapt_sharding_spec(
                output_dim_partition_dict, device_mesh, node
            )

    return gm


def module_params_sharding_pass(
    gm: torch.fx.GraphModule, device_mesh: DeviceMesh, overlap=False
):
    """
    ...
    for node in nodes:
        if node.op == "call_module":
            ...
            for name, buffer in target_module.named_buffers():
                ...
                buffer.sharding_spec = origin_sharding_spec
                target_sharding_spec = node.best_strategy.get_sharding_spec_by_name(
                    name
                )
                buffer_sharded = shape_consistency_manager.apply(
                    buffer, target_sharding_spec
                )
...
def runtime_preparation_pass(
    ...
    (
        ...
        origin_node_sharding_spec_dict,
        comm_actions_dict,
    ) = solution_annotation_pass(gm, solution, strategies_constructor)

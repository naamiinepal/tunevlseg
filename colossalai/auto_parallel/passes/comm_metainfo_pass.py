...
def _runtime_apply_meta_info(
    node: Node, origin_spec_dict, sharding_spec_dict
) -> ShardMetaInfo:
    """
...
def _runtime_comm_spec_apply_meta_info(
    node: Node, comm_actions_dict: dict
) -> ShardMetaInfo:
    ...
    if isinstance(comm_action.comm_spec, CommSpec):
        ...
        input_shape = output_shape = (
            comm_action.comm_spec.sharding_spec.get_sharded_shape_per_device()
        )
        meta_info.fwd_in = [torch.rand(input_shape, device="meta")]
        meta_info.fwd_buffer = []
        meta_info.fwd_out = [torch.rand(output_shape, device="meta")]
        ...
        meta_info = _construct_shard_meta_info(
...
def comm_metainfo_pass(
    gm: GraphModule,
    sharding_spec_dict: dict,
    origin_spec_dict: dict,
    ...
    for node in gm.graph.nodes:
        if node.target == runtime_apply:
            node.best_strategy_info = _runtime_apply_meta_info(
                node, origin_spec_dict, sharding_spec_dict
            )
        elif node.target == runtime_comm_spec_apply:
            node.best_strategy_info = _runtime_comm_spec_apply_meta_info(
                node, comm_actions_dict
            )
        else:
            pass
    return gm

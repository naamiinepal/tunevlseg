...
class T5PipelineForwards:
    ...
    def t5_stack_forward(
        ...
        decoder_starting_stage: int | None = None,
    ) -> dict | tuple | BaseModelOutputWithPastAndCrossAttentions:
        ...
        if past_key_values:
            logger.warning_once(
                "Non-empty past_key_values is not supported for pipeline models at the moment."
            )
        ...
        at_last_stage = (stage == decoder_starting_stage - 1) or (
            stage == stage_manager.num_stages - 1
        )
    ...
    def t5_model_forward(
        ...
        position_bias: torch.Tensor | None = None,
        encoder_decoder_position_bias: torch.Tensor | None = None,
        backward_tensor_keys: list[str] | None = None,
        stage_index: list[int] | None = None,
        decoder_starting_stage: int | None = None,
        ...
        if encoder_outputs is not None:
            ...
            raise ValueError(
                "Non-empty encoder_hidden_states should be passed in at decoder stages."
            )

        if not at_first_decoder_stage and hidden_states is None:
            raise ValueError(
                "If not at the first layer of decoder, non-empty hidden_states must be provided."
            )
    ...
    def t5_encoder_model_forward(
        ...
        return T5PipelineForwards.t5_stack_forward(
            self.encoder,
            input_ids=input_ids,
            attention_mask=attention_mask,
            inputs_embeds=inputs_embeds,
...
def get_jit_fused_T5_layer_ff_forward():
    ...
    def forward(self: T5LayerFF, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        return self.dropout_add(
            forwarded_states, hidden_states, self.dropout.p, self.dropout.training
        )
...
def get_T5_layer_self_attention_forward():
    ...
    def forward(
        ...
        hidden_states = self.dropout_add(
            attention_output[0], hidden_states, self.dropout.p, self.dropout.training
        )

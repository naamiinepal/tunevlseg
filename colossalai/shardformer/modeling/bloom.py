...
def build_bloom_alibi_tensor_fn(process_group: ProcessGroup) -> torch.Tensor:
    def build_bloom_alibi_tensor(
        ...
        if dist.is_initialized():
            ...
            return alibi.reshape(batch_size * num_heads_per_rank, 1, seq_length).to(
                dtype
            )
        ...
        **deprecated_arguments,
        ...
        for i, (block, layer_past) in enumerate(
            zip(
                ...
                past_key_values[start_idx:end_idx],
                strict=False,
            ),
            start=start_idx,
        ):
...
def get_bloom_flash_attention_forward(enable_jit_fused=False):
    ...
    def forward(
        ...
        attention_numerical_mask = (
            attention_numerical_mask
            + alibi.view(batch_size, self.num_heads, 1, kv_length) * self.beta
        )
        ...
        output_tensor = self.dropout_add(
...
def get_jit_fused_bloom_mlp_forward():
    ...
    def forward(
        self: BloomMLP, hidden_states: torch.Tensor, residual: torch.Tensor
    ) -> torch.Tensor:

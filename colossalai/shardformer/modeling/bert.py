...
class BertPipelineForwards:
    ...
    def bert_model_forward(
        ...
        | None = None,  # this is from the previous stage
...
def get_jit_fused_bert_self_output_forward():
    ...
    def forward(
        self: BertSelfOutput, hidden_states: torch.Tensor, input_tensor: torch.Tensor
    ) -> torch.Tensor:
...
def get_jit_fused_bert_output_forward():
    ...
    def forward(
        self: BertOutput, hidden_states: torch.Tensor, input_tensor: torch.Tensor
    ) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout_add(
            hidden_states, input_tensor, self.dropout.p, self.dropout.training

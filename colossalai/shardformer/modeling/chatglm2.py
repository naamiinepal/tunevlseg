...
def get_flash_core_attention_forward():
    ...
    def forward(
        self: CoreAttention, query_layer, key_layer, value_layer, attention_mask
    ):
        ...
        if attention_mask is None and query_layer.shape[2] == key_layer.shape[2]:
            ...
            attn_bias.masked_fill_(
                temp_mask.logical_not(), torch.finfo(query_layer.dtype).min
            )
        else:
...
def get_jit_fused_glm_block_forward():
    ...
    def forward(
        ...
        layernorm_input = self.dropout_add(
            attention_output, residual, self.hidden_dropout, self.training
        )
        ...
        output = self.dropout_add(
            mlp_output, residual, self.hidden_dropout, self.training
        )

    ...
    def llama_model_forward(
        ...
        if self.gradient_checkpointing and self.training:
            ...
            if shard_config.gradient_checkpoint_config is not None:
                num_ckpt_layers = (
                    shard_config.gradient_checkpoint_config.get_num_ckpt_layers(
                        stage=stage_manager.stage,
                        num_stages=stage_manager.num_stages,
                        num_layers=end_idx - start_idx,
                        model_chunk_id=(
                            stage_manager.model_chunk_id
    ...
    def forward(
        ...
        if sp_mode in ["ring", "split_gather"]:
            ...
            inputs_embeds = split_forward_gather_backward(
                inputs_embeds, 1, sp_group, 1 / sp_size
            )
        ...
        if (
            self.gradient_checkpointing or sp_mode in ["ring", "all_to_all"]
        ) and self.training:
            if use_cache:
                logger.warning_once(
                    "`use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`..."
        ...
        if sp_mode == "ring" or sp_mode == "split_gather":
            ...
            hidden_states = gather_forward_split_backward(
                hidden_states, 1, sp_group, grad_scale=sp_size
            )

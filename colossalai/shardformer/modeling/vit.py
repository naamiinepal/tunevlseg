...
def ViTModel_pipeline_forward(
    stage_manager: PipelineStageManager, stage_index: list[int]
):
...
def ViTForImageClassification_pipeline_forward(
    stage_manager: PipelineStageManager, stage_index: list[int]
):
...
def ViTForMaskedImageModeling_pipeline_forward(
    stage_manager: PipelineStageManager, stage_index: list[int]
):
    import math
...
def get_vit_flash_self_attention_forward():
    ...
    def forward(
        ...
        context_layer = ColoAttention.attention(
            query_layer, key_layer, value_layer, dropout_p=dropout_p
        )
...
def get_jit_fused_vit_output_forward():
    ...
    def forward(
        self: ViTOutput, hidden_states: torch.Tensor, input_tensor: torch.Tensor
    ) -> torch.Tensor:

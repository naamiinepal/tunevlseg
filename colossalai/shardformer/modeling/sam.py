...
def get_sam_flash_attention_forward():
    ...
    try:
        ...
        raise ImportError(
            "Error: xformers module is not installed. Please install it to use flash attention."
        )
...
def get_sam_vision_flash_attention_forward():
    ...
    def add_decomposed_rel_pos(
        ...
        reshaped_query = query.transpose(1, 2).reshape(
            batch_size * nHead, query_height, query_width, dim
        )
        rel_h = torch.einsum("bhwc,hkc->bhwk", reshaped_query, relative_position_height)
        rel_w = torch.einsum("bhwc,wkc->bhwk", reshaped_query, relative_position_width)
        rel_pos = rel_h[:, :, :, :, None] + rel_w[:, :, :, None, :]
        return rel_pos.reshape(
            batch_size, nHead, query_height * query_width, key_height * key_width
        )
    ...
    def forward(
        self: SamVisionAttention, hidden_states: torch.Tensor, output_attentions=False
    ) -> torch.Tensor:
        batch_size, height, width, _ = hidden_states.shape
        # qkv with shape (3, batch_size, nHead, height * width, channel)
        qkv = (
        ...
        query, key, value = qkv.reshape(
            3, batch_size, height * width, self.num_attention_heads, -1
        ).unbind(0)

        rel_pos = None
        if self.use_rel_pos:
            rel_pos = add_decomposed_rel_pos(
                query, self.rel_pos_h, self.rel_pos_w, (height, width), (height, width)
            )

        attn_output = me_attention(
            query, key, value, attn_bias=rel_pos, p=self.dropout, scale=self.scale
        )

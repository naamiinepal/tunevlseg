...
class OPTPipelineForwards:
    ...
    def opt_model_forward(
        ...
        if stage_manager.is_first_stage():
            ...
            if hidden_states is None:
                raise ValueError(
                    "hidden_states shouldn't be None for intermediate stages."
                )
        ...
        for attn_mask, mask_name in zip([head_mask], ["head_mask"], strict=False):
    ...
    def opt_for_sequence_classification_forward(
        ...
        if stage_manager.is_last_stage():
            ...
            batch_size = (
                input_ids.shape[0] if input_ids is not None else hidden_states.shape[0]
            )
...
def get_jit_fused_opt_decoder_layer_forward():
    ...
    def forward(
        ...
        hidden_states = self.dropout_add(
            hidden_states, residual, self.dropout, self.training
        )

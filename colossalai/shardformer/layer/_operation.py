...
def _ring_as_gather(
    ...
    def communicate_step():
        ...
        for k in recv_tensors:
            comm_ops.extend(
                (
                    dist.P2POp(
                        dist.irecv, recv_tensors[k], recv_rank, group=process_group
                    ),
                    dist.P2POp(
                        dist.isend, send_tensors[k], send_rank, group=process_group
                    ),
                )
            )
    ...
    return torch.cat(
        output_tensors[group_size - cur_rank :]
        + output_tensors[: group_size - cur_rank],
        dim=gather_dim,
    )
...
class _GatherForwardReduceScatterBackward(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_list = [
            item.contiguous()
            for item in torch.chunk(
                grad_output, dist.get_world_size(process_group), dim=dim
            )
        ]
        output = torch.empty(
            new_shape, dtype=grad_output.dtype, device=grad_output.device
        )
        ...
        if not overlap:
            ...
            if ctx.async_grad_reduce_scatter:
                ...
                input_list = [
                    item.contiguous()
                    for item in torch.chunk(
                        grad_input, dist.get_world_size(process_group), dim=dim
                    )
                ]
                output = torch.empty(
                    input_.shape,
                ...
                handle = dist.reduce_scatter(
                    output, input_list, group=process_group, async_op=True
                )
                # Relay on CUDA_DEVICE_MAX_CONNECTIONS=1 to have
                # all-reduce scheduled first and have GPU resources allocated, CUDA_DEVICE_MAX_CONNECTIONS=1 is set in shardformer.py
            ...
            gather_handle = dist.all_gather(
                tensor_list, input_, group=process_group, async_op=True
            )
            ...
            reducescatter_handle = dist.reduce_scatter(
                output, input_list, group=process_group, async_op=True
            )
            ...
            if _grad_accum_fusion_available and weight.grad is not None:
                ...
                if grad.dtype == torch.float32:
                    fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32(
                        input_parallel, grad_output, grad
                    )
                    grad_weight = None
                elif grad.dtype == torch.float16:
                    fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16(
...
def _ring_as_reducescatter(
    ...
    for k, v in input_to_reducescatter.items():
        ...
        _input_tensors = list(
            torch.split(
                v, input_shape[reducescatter_dim] // group_size, dim=reducescatter_dim
            )
        )
    ...
    def forward(ctx, input_, weight, bias, process_group, dim, ring):
        ...
        if ring is True:
            ...
            output_list = [
                item.contiguous()
                for item in torch.chunk(
                    partial_output, dist.get_world_size(process_group), dim=dim
                )
            ]
            output = torch.empty(
                output_shape, dtype=partial_output.dtype, device=partial_output.device
            ).contiguous()
...
class _ReduceScatterForwardGatherBackward(torch.autograd.Function):
    ...
    def forward(ctx, input_, process_group, dim):
        ...
        input_list = [
            item.contiguous()
            for item in torch.chunk(input_, dist.get_world_size(process_group), dim=dim)
        ]
...
def _split(input_, dim=-1, process_group=None):
    ...
    return tensor_list[rank].clone().contiguous()
...
def matmul_with_async_comm(input_, weight, bias, process_group, async_grad_allreduce):
    return MatmulWithAsyncCommunication.apply(
        input_, weight, bias, process_group, async_grad_allreduce
    )
...
def linear_reducescatter_forward_gather_backward(
    input_, weight, bias=None, process_group=None, dim=1, ring=False
):
    return _LinearWithReduceScatterForwardGatherBackward.apply(
        input_, weight, bias, process_group, dim, ring
    )

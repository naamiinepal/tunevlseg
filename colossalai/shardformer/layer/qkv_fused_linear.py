...
class GPT2FusedLinearConv1D_Col(ParallelModule):
    ...
    def __init__(
        ...
        seq_parallel_mode: str | None = None,
        overlap: bool = False,
        skip_bias_add: bool = False,
        n_fused: int = 3,
        weight: Parameter | None = None,
        bias_: Parameter | None = None,
        ...
        self.randomizer = create_randomizer_with_offset(
            seed, process_group=self.process_group
        )
        ...
        def shard_fn(tensor):
            return split_fused_qkv_in_gpt2_style(
                tensor, self.n_fused, self.process_group, True
            )
        ...
        if not is_customized_distributed_tensor(self.weight):
            with torch.no_grad():
                sharded_weight = distribute_tensor_with_customization(
                    self.weight.data, shard_fn, gather_fn
                )
        ...
        if bias:
            ...
            if not is_customized_distributed_tensor(self.bias):
                with torch.no_grad():
                    sharded_bias = distribute_tensor_with_customization(
                        self.bias.data, shard_fn, gather_fn
                    )
    ...
    def from_native_module(
        module: nn.Module,
        process_group: ProcessGroup | list[ProcessGroup],
        *args,
        **kwargs,
    ) -> ParallelModule:
        r"""
        ...
        return GPT2FusedLinearConv1D_Col(
...
class GPT2FusedLinearConv1D_Row(ParallelModule):
    ...
    def from_native_module(
        ...
        **kwargs,
        ...
        return GPT2FusedLinearConv1D_Row(
            in_features=in_features,
            out_features=out_features,
            bias=bias,
            device=device,
        ...
        with self.randomizer.fork_rng(enable_cpu=True):
            ...
            if self.bias is not None:
                ...
                if self.process_group is None:
                    ...
                    src_rank = dist.distributed_c10d._get_global_rank(
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            assert (
                input_.shape[-1] == self.weight.shape[0]
            ), f"Invalid shapes in Linear1D_Row forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[0]}."
            input_ = input_
        else:
            assert (
                divide(input_.shape[-1], self.num_partitions) == self.weight.shape[0]
            ), f"Invalid shapes in Linear1D_Row forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[0] * self.num_partitions}."
            input_ = split_forward_gather_backward(
                input_, dim=-1, process_group=self.process_group
            )
        ...
        if self.stream_chunk_num > 1:
            ...
            if self.seq_parallel_mode is None:
                ...
                self.seq_parallel_mode == "split_gather"
                or self.seq_parallel_mode == "ring"
            ):
                output_parallel = torch.matmul(input_, self.weight)
                output = reducescatter_forward_gather_backward(
                    output_parallel, self.process_group, 1
                )
...
class FusedLinear1D_Col(ParallelModule):
    ...
    def __init__(
        ...
        def shard_fn(tensor):
            return split_fused_qkv_in_gpt2_style(
                tensor, self.n_fused, self.process_group, False
            )
    ...
    def forward(self, input_: Tensor) -> tuple[Tensor, Tensor]:
        ...
        output_parallel = linear_with_async_comm(
            input_parallel, self.weight, bias, self.process_group, True
        )

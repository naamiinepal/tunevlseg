...
class DropoutForParallelInput(ParallelModule, nn.Dropout):
    ...
    def __init__(
        self, p: float = 0.5, inplace: bool = False, process_group: ProcessGroup = None
    ):
        ...
        self.randomizer = create_randomizer_with_offset(
            seed, process_group=process_group
        )

    @staticmethod
    def from_native_module(
        module: nn.Dropout, process_group: ProcessGroup | list[ProcessGroup] = None
        ...
        return DropoutForParallelInput(
            p=p, inplace=inplace, process_group=process_group
        )
...
class DropoutForReplicatedInput(ParallelModule, nn.Dropout):
    ...
    def __init__(
        ...
        seed = torch.random.initial_seed()
        self.randomizer = create_randomizer_with_offset(
            seed, process_group=process_group, offset_by_rank=False
        )
    ...
    def from_native_module(
        ...
        p = module.p
        inplace = module.inplace
        return DropoutForReplicatedInput(
            p=p, inplace=inplace, process_group=process_group
        )

...
class SeqParallelUtils:
    ...
    def allreduce_partial_data_grad(
        ...
        assert (model is not None) ^ (
            grads is not None
        ), "Exactly one of model and grads must be not None."
        ...
        if model is not None:
            ...
            for buf, synced in zip(
                grads, _unflatten_dense_tensors(coalesced, grads), strict=False
            ):
                buf.copy_(synced)
        else:
...
class Randomizer:
    ...
    def _get_device_rng_state(self):
        return get_accelerator().get_rng_state()
    ...
    def is_randomizer_index_synchronized(process_group: ProcessGroup = None):
        ...
        if dist.is_initialized():
            ...
            index_tensor = torch.tensor(
                index, dtype=torch.int32, device=get_accelerator().get_current_device()
            )

            # all gather the index
            gathered_index = [
                torch.zeros_like(index_tensor)

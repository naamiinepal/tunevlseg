...
__all__ = ["BaseLayerNorm", "FusedLayerNorm", "FusedRMSNorm", "LayerNorm", "RMSNorm"]
...
try:
    ...
    class FusedLayerNormWithHook(ApexFusedLayerNorm):
        ...
        def forward(self, input):
            ...
            return hook_parameter_in_backward(output, self.weight, self.bias)
    ...
    class FusedRMSNormWithHook(ApexFusedRMSNorm):
        ...
        def forward(self, input):
            output = super().forward(input)
            return hook_parameter_in_backward(output, self.weight)
...
class LayerNorm(BaseLayerNorm):
    ...
    @staticmethod
    def from_native_module(
        module: nn.LayerNorm, sp_partial_derived: bool = False, *args, **kwargs
    ) -> nn.Module:
        r"""
        ...
        assert isinstance(
            module, nn.LayerNorm
        ), "Only support conversion from nn.LayerNorm."

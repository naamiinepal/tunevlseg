...
__all__ = ["Embedding1D", "PaddingEmbedding", "VocabParallelEmbedding1D"]
...
class Embedding1D(ParallelModule):
        ...
        **kwargs,
        ...
        if weight is None:
            ...
            self.weight = nn.Parameter(
                torch.empty((num_embeddings, self.embedding_dim), **factory_kwargs)
            )
        else:
    ...
    def from_native_module(
        module: nn.Embedding,
        process_group: ProcessGroup | list[ProcessGroup] = None,
        *args,
        **kwargs,
    ) -> "Embedding1D":
        r"""
        ...
        if sparse:
            raise NotImplementedError(
                "The Embedding1D module does not support sparse embedding yet."
            )
...
class VocabParallelEmbedding1D(PaddingParallelModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_embeddings % multiple != 0:
            self.num_embeddings = (
                num_embeddings + multiple - (num_embeddings % multiple)
            )
        ...
        self.num_embeddings_per_partition = divide(
            self.num_embeddings, tensor_parallel_size
        )
    ...
    def from_native_module(
        ...
        **kwargs,
        ...
        # create the parallel module
        return VocabParallelEmbedding1D(
            num_embeddings=num_embeddings,
            embedding_dim=embedding_dim,
            padding_idx=padding_idx,
            device=device,
    ...
    def _select_padding_idx(self, padding_idx: int):
        ...
        if padding_idx is None:
            return None
        if padding_idx < self.vocab_end_index and padding_idx >= self.vocab_start_index:
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        return reduce_forward(embedding_output, self.process_group)

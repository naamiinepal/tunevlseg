...
class DistCrossEntropy(Function):
    ...
    def forward(
        ...
        pred_logits_1d = logits_2d[
            torch.arange(start=0, end=logits_2d.shape[0], device=logits_2d.device),
            masked_target_1d,
        ]
        ...
        # loss = log(sum(exp(x[i]))) - x[class]
        loss = torch.where(
            target == ignore_index, 0.0, torch.log(sum_exp_logits) - pred_logits
        )
    ...
    def backward(ctx, grad_output):
        ...
        grad_logits_2d[torch.arange(0, grad_logits_2d.shape[0]), masked_target_1d] -= (
            update
        )
...
def cross_entropy_1d(
    ...
    return DistCrossEntropy.apply(
        vocab_logits, labels, ignore_index, process_group, vocab_size
    )

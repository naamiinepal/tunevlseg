...
class ColoAttention:
    _kernel_dispatch_map: (
        dict[torch.dtype, dict[AttnMaskType | None, Callable]] | None
    ) = None
    ...
    def _dispatch_kernel(
        dtype: torch.dtype, mask_type: AttnMaskType | None
    ) -> Callable:
        ...
        if (
            ...
            raise ValueError(
                f"FlashAttention kernel is not available for dtype {dtype} and mask_type {mask_type}"
            )
        # lazy load
        if isinstance(
            ColoAttention._kernel_dispatch_map[dtype][mask_type], KernelLoader
        ):
            ColoAttention._kernel_dispatch_map[dtype][mask_type] = (
                ColoAttention._kernel_dispatch_map[dtype][mask_type].load()
            )
    ...
    def prepare_attn_kwargs(
        ...
        if (q_padding_mask is None or q_padding_mask.bool().all()) and (
            ...
            attention_mask = (
                torch.ones(s_q, s_kv, dtype=dtype, device=device)
                .tril(diagonal=0)
                .expand(b, s_q, s_kv)
            )
        else:
            assert (
                q_padding_mask.shape
                == (
                    b,
                    s_q,
                )
            ...
            if kv_padding_mask is None:
                ...
                max_seqlen_kv, cu_seqlens_kv, kv_indices = (
                    max_seqlen_q,
                    cu_seqlens_q,
                    q_indices,
                )
            ...
            if is_causal:
                ...
                attention_mask = attention_mask * attention_mask.new_ones(
                    s_q, s_kv
                ).tril(diagonal=0)
            else:
    ...
    def attention(
        ...
        if attention_mask is not None:
            assert torch.is_floating_point(
                attention_mask
            ), "attention_mask should be a floating point tensor."

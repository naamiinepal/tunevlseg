...
class Linear1D_Col(ParallelModule):
    ...
    def from_native_module(
        module: nn.Linear, process_group: ProcessGroup | list[ProcessGroup], **kwargs
    ) -> ParallelModule:
        r"""
    ...
    def forward(self, input_: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.seq_parallel_mode is None:
            ...
            output_parallel = linear_with_async_comm(
                input_parallel, self.weight, bias, self.process_group, False
            )
...
class Linear1D_Row(ParallelModule):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            ...
            ), f"Invalid shapes in Linear1D_Row forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1] * self.num_partitions}."
        ...
        if self.stream_chunk_num > 1:
            ...
            if self.seq_parallel_mode is None:
                output_parallel = linear_with_async_comm(
                    input_, self.weight, None, self.process_group, False
                )
...
class PaddingLMHead(PaddingParallelModule):
    def __init__(
        ...
        if out_features % make_vocab_size_divisible_by != 0:
            self.out_features = (
                ...
                - (out_features % make_vocab_size_divisible_by)
            )
        if weight is None:
            factory_kwargs = {"device": device, "dtype": dtype}
            weight = Parameter(
                torch.empty(out_features, self.in_features, **factory_kwargs)
            )
        else:
            weight.data = weight.data.to(device=device, dtype=dtype)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        return output[..., : self.old_num_embeddings]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.old_num_embeddings >= (tp_rank + 1) * partition_size:
            ...
            self.num_valid_embeddings_local = (
                self.old_num_embeddings - tp_rank * partition_size
            )
        else:

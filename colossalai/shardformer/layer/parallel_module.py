...
class ParallelModule(nn.Module, ABC):
    ...
    def from_native_module(
        module: nn.Module, process_group: ProcessGroup | list[ProcessGroup] = None
    ) -> "ParallelModule":
        """
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        for name, param in self._parameters.items():
            if param is not None:
                destination[prefix + name] = gather_distributed_param(
                    param, keep_vars=keep_vars
                ).data
            ...
            if key in state_dict:
                ...
                if is_distributed_tensor(param):
                    ...
                    sharded_tensor = distribute_tensor(
                        input_param, device_mesh, sharding_spec
                    )
                    ...
                    input_param = distribute_tensor_with_customization(
                        input_param, param.shard_fn, param.gather_fn
                    )
...
class PaddingParallelModule(ParallelModule):
    def __init__(
        ...
        **kwargs,
        ...
        if not (
            is_distributed_tensor(self.weight)
            or self.weight.shape[0] == self.new_num_embeddings
        ):
            self.resize_embedding_weight()

        if self.bias is not None and not (
            is_distributed_tensor(self.bias)
            or self.bias.shape[0] == self.new_num_embeddings
        ):
            ...
            if key in state_dict:
                ...
                if is_padded_tensor(param):
                    input_param = to_padded_tensor(
                        input_param, param._current_length, param._padding_dim
                    )

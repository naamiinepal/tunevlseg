...
class GLUEDataBuilder:
    ...
    def setup(self):
        ...
        self.eval_splits = [x for x in self.dataset if "validation" in x]
    ...
    def test_dataloader(self):
        if self.plugin is None:
            return self.native_prepare_dataloader(
                self.dataset["test"], batch_size=self.train_batch_size
            )
        if len(self.eval_splits) == 1:
            return self.plugin.prepare_dataloader(
    ...
    def native_prepare_dataloader(
        self, dataset, batch_size, shuffle=False, drop_last=False, pin_memory=False
    ):
        return DataLoader(
            dataset,

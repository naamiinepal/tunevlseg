...
def train(args):
    ...
    if args.model == "bert":
        cfg = BertConfig.from_pretrained(
            args.pretrain, num_labels=data_builder.num_labels
        )
    ...
    if coordinator.is_master():
        ...
        if args.target_f1 is not None and "f1" in results:
            assert (
                results["f1"] >= args.target_f1
            ), f'f1 score {results["f1"]} is lower than target {args.target_f1}'
...
def evaluate_model(
    model: nn.Module,
    test_dataloader: DataLoader | list[DataLoader],
    num_labels: int,
    task_name: str,
    ...
    metric = evaluate.load(
        "glue",
        task_name,
        process_id=coordinator.rank,
        num_process=coordinator.world_size,
    )
    model.eval()
    ...
    final_results = {}
    for split, sub_loader in zip(eval_splits, test_dataloader, strict=False):

...
class ModelSharder:
    ...
    def __init__(
        self, model: nn.Module, policy: Policy, shard_config: ShardConfig = None
    ) -> None:
        self.model = model
    ...
    def _replace_module(self, include: set[nn.Module] | None = None) -> None:
        r"""
    ...
    def _replace_method(
        self, module: nn.Module, method_replacement: dict[str, Callable]
    ):
    ...
    def _replace_sub_module(
        ...
        for description in sub_module_replacement:
            ...
            if (
                (include is not None)
                and (native_sub_module is not None)
                and (native_sub_module not in include)
            ):
                continue
    ...
    def _get_recursive_held_layers(
        self, held_layers: list[nn.Module] | None
    ) -> list[nn.Module] | None:
    ...
    def _release_unheld_layers(self) -> set[nn.Module] | None:
        r"""

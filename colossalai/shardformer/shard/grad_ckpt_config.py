...
class PipelineGradientCheckpointConfig(GradientCheckpointConfig):
    ...
    """
    num_ckpt_layers_per_stage: list[int] | None = None

    def __post_init__(self):
        if self._enable_customized_ckpt_layers_per_stage:
            assert all(
                num_ckpt_layers >= 0
                for num_ckpt_layers in self.num_ckpt_layers_per_stage
            )
    ...
    def get_num_ckpt_layers(
        ...
        if (
            not self._enable_gradient_checkpointing_ratio
            and not self._enable_customized_ckpt_layers_per_stage
        ):
        ...
        if self._enable_customized_ckpt_layers_per_stage:
            ...
            num_ckpt_layers = self.num_ckpt_layers_per_stage[
                stage + model_chunk_id * num_stages
            ]

...
class ShardConfig:
    ...
    tensor_parallel_process_group: ProcessGroup | None = None
    sequence_parallel_process_group: ProcessGroup | None = None
    pipeline_stage_manager: PipelineStageManager | None = None
    ...
    def __post_init__(self):
        ...
        if self.enable_sequence_parallelism:
            self.sequence_parallelism_mode = (
                ...
                else self.sequence_parallelism_mode
            )
            assert (
            ...
            if self.sequence_parallelism_mode in ["split_gather", "ring"]:
                assert self.enable_tensor_parallelism, f"sequence parallelism mode {self.sequence_parallelism_mode} can only be used when enable_tensor_parallelism is True"
            elif self.sequence_parallelism_mode == "all_to_all":
                assert not self.enable_tensor_parallelism, f"sequence parallelism mode {self.sequence_parallelism_mode} can only be used when enable_tensor_parallelism is False"
            ...
            if self.sequence_parallelism_mode:
                ...
                warnings.warn(
                    "The sequence_parallelism_mode will be ignored when enable_sequence_parallelism is False"
                )
            assert not self.enable_sequence_overlap, "enable_sequence_overlap can only be set to True when enable_sequence_parallelism is True"
    ...
    def _turn_on_all_optimization(self):
        ...
        try:
            ...
            apex_avail = False
            warnings.warn(
                "You set enable_all_optimization=True, but apex is not installed."
            )

...
class T5BasePolicy(Policy):
    ...
    def get_held_layers(self) -> list[nn.Module]:
        ...
        start_idx, end_idx = self.get_t5_stage_index(
        ...
        if stage_manager.stage < decoder_starting_stage:
            ...
            if stage_manager.is_first_stage():
                held_layers.extend(
                    (model.shared, encoder.embed_tokens, encoder.dropout)
                )
            if stage_manager.stage == decoder_starting_stage - 1:
                held_layers.extend((encoder.final_layer_norm, encoder.dropout))
            held_layers.extend(encoder.block[start_idx:end_idx])
        else:
            # current stage is in t5's decoder
            if stage_manager.stage == decoder_starting_stage:
                held_layers.extend((decoder.embed_tokens, decoder.dropout))
            if stage_manager.is_last_stage():
                held_layers.extend((decoder.final_layer_norm, decoder.dropout))
    ...
    def set_pipeline_forward(
        ...
        stage_index = self.get_t5_stage_index(
            layers_per_stage, stage_manager.stage, decoder_starting_stage
        )

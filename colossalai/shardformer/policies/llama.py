...
class LlamaPolicy(Policy):
    ...
    def module_policy(self) -> dict[str | nn.Module, ModulePolicyDescription]:
        ...
        if self.pipeline_stage_manager is not None:
            ...
            warnings.warn(
                "For llama, sequence parallelism is currently not compatible with pipeline parallelism, set to be False"
            )
        sp_mode = (
        ...
        if sp_mode in ["split_gather", "ring"]:
            ...
            if getattr(self.model.config, "num_key_value_heads", False):
                decoder_attribute_replacement["num_key_value_heads"] = (
                    self.model.config.num_key_value_heads // sp_size
                )
        ...
        if use_flash_attention:
            self.append_or_create_method_replacement(
                description={
                    "forward": get_llama_flash_attention_forward(
                        self.shard_config, sp_mode, sp_group, sp_size
                    ),
                },
                policy=policy,
                target_key=attn_cls,
            ...
            if self.pipeline_stage_manager is None:
                ...
                self.append_or_create_method_replacement(
                    description={
                        "forward": get_llama_model_forward_for_flash_attn(
                            self.shard_config
                        ),
                    },
                    policy=policy,
...
class LlamaForCausalLMPolicy(LlamaPolicy):
    def module_policy(self):
        ...
        if (
            ...
            and not self.shard_config.enable_sequence_parallelism
        ):

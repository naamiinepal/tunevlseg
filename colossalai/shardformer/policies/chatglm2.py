...
class ChatGLMPolicy(Policy):
    ...
    def preprocess(self):
        if self.pipeline_stage_manager is not None:
            ...
            bsz_dim = 1
            self.model.batch_size_dim = bsz_dim
    ...
    def module_policy(self) -> dict[str | nn.Module, ModulePolicyDescription]:
        ...
        if self.shard_config.enable_fused_normalization:
            ...
            norm_cls = col_nn.RMSNorm if self.model.config.rmsnorm else col_nn.LayerNorm
        ...
        assert (
            sp_mode != "all_to_all"
        ), "all_to_all sequence parallelism is not supported for ChatGLM2"
        if sp_mode == "ring":
            warnings.warn(

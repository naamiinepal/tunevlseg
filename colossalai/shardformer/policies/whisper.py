...
class WhisperPolicy(Policy):
    ...
    def module_policy(self):
        ...
        if self.shard_config.enable_jit_fused:
            ...
            warnings.warn(
                "Whisper doesn't support jit fused operator now, will ignore the jit fused operator flag."
            )

        if self.shard_config.enable_tensor_parallelism:
            policy[WhisperEncoderLayer] = ModulePolicyDescription(
                attribute_replacement={
                    "self_attn.embed_dim": self.model.config.d_model
            ...
            policy[WhisperDecoderLayer] = ModulePolicyDescription(
                attribute_replacement={
                    ...
                    // self.shard_config.tensor_parallel_size,
                    "encoder_attn.embed_dim": self.model.config.d_model
                    // self.shard_config.tensor_parallel_size,
        ...
        if self.shard_config.enable_flash_attention:
            ...
            if not self.shard_config.pipeline_stage_manager:
                self.append_or_create_method_replacement(
                    description={
                        "forward": get_whisper_decoder_forward_for_flash_attention(
                            self.shard_config
                        ),
                    },
                    policy=policy,
    ...
    def distribute_whisper_layers(
        ...
        if num_encoder_layers <= 0:
            raise ValueError(
                "The number of encoder layers for whisper must be a positive integer."
            )
        ...
        if num_decoder_layers == 0:
            return stage_manager.distribute_layers(
        ...
        encoder_distribution = stage_manager.distribute_layers(
            num_encoder_layers, num_encoder_stages
        )
        decoder_distribution = stage_manager.distribute_layers(
    ...
    def get_held_layers(self) -> list[nn.Module]:
        ...
        num_decoder_layers = len(decoder.layers) if decoder else 0
        ...
        if stage_manager.stage < decoder_starting_stage:
            ...
            if stage_manager.is_first_stage():
                held_layers.extend(
                    (encoder.embed_positions, encoder.conv1, encoder.conv2)
                )
            ...
            if stage_manager.stage == decoder_starting_stage:
                held_layers.extend((decoder.embed_tokens, decoder.embed_positions))
    ...
    def set_pipeline_forward(
        ...
        stage_index = self.get_whisper_stage_index(
            layers_per_stage, stage_manager.stage, decoder_starting_stage
        )

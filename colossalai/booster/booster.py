...
class Booster:
    ...
    def __init__(
        ...
        mixed_precision: MixedPrecision | str | None = None,
        ...
        if self.plugin and self.plugin.control_device():
            ...
            if device is not None:
                warnings.warn(
                    "The plugin will control the accelerator, so the device argument will be ignored."
                )
        else:
    ...
    def execute_pipeline(
        ...
        return self.plugin.execute_pipeline(
            data_iter, model, criterion, optimizer, return_loss, return_outputs
        )

    def no_sync(
        self, model: nn.Module = None, optimizer: OptimizerWrapper = None
    ) -> contextmanager:
    ...
    def load_model(
        self, model: nn.Module | ModelWrapper, checkpoint: str, strict: bool = True
    ) -> None:
    ...
    def save_optimizer(
        ...
        self.checkpoint_io.save_optimizer(
            optimizer, checkpoint, shard, gather_dtensor, prefix, size_per_shard
        )

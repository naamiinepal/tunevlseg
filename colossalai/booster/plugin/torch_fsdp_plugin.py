...
class TorchFSDPCheckpointIO(GeneralCheckpointIO):
    ...
    def save_sharded_model(
        ...
        state_dict_shard = utils.shard_model_checkpoint(
            state_dict, max_shard_size=size_per_shard
        )

        weights_name, save_index_file = utils.get_model_base_filenames(
    ...
    def load_sharded_model(
        ...
        for shard_file in checkpoint_files:
            fsdp_state_dict.update(
                utils.load_shard_state_dict(Path(shard_file), use_safetensors)
            )
    ...
    def save_sharded_optimizer(
        ...
        if self.coordinator.is_master():
            ...
            states_name, save_index_file, param_group_file = (
                utils.get_optimizer_base_filenames(prefix)
            )
            ...
            sharded_state = utils.shard_optimizer_checkpoint(
                fsdp_optim_state, max_shard_size=size_per_shard
            )
    ...
    def load_sharded_optimizer(
        self, optimizer: Optimizer, index_file_path: str, size_per_shard: int
    ):
        """
        ...
        checkpoint_files, _ = ckpt_index_file.get_checkpoint_filenames()
        for shard_file in checkpoint_files:
            state_dict_shard = utils.load_shard_state_dict(
                Path(shard_file), use_safetensors=False
            )
            fsdp_optim_state.update(state_dict_shard)

        fsdp_optim_dict = {
            "state": fsdp_optim_state,
            "param_groups": saved_param_groups,
        }

        with FSDP.state_dict_type(
            optimizer.unwrap_model().unwrap(), StateDictType.FULL_STATE_DICT
        ):
            fsdp_state = FSDP.optim_state_dict_to_load(
                model=optimizer.unwrap_model().unwrap(),
                optim=optimizer,
                optim_state_dict=fsdp_optim_dict,
            )

...
class HybridParallelZeroOptimizer(LowLevelZeroOptimizer):
    def __init__(
        ...
        pp_process_group: ProcessGroup | None = None,
...
class MoeHybridParallelPlugin(HybridParallelPlugin):
    ...
    def __init__(
        ...
        checkpoint_io: MoECheckpointIO | None = None,
    ) -> None:
        assert (
        ...
        if extra_dp_size > 1:
            ...
            if use_ep_inside:
                self.pg_mesh_moe = ProcessGroupMesh(
                    self.pp_size, extra_dp_size, ep_size
                )
                self.moe_extra_dp_group = self.pg_mesh_moe.get_group_along_axis(1)
                if dist.get_rank() == 0:
                    print(
                        f"Zero Parallel: pp {self.pp_size}, outer_dp {extra_dp_size}, inner_dp {ep_size}"
                    )
            else:
                self.pg_mesh_moe = ProcessGroupMesh(
                    self.pp_size, ep_size, extra_dp_size
                )
                self.moe_extra_dp_group = self.pg_mesh_moe.get_group_along_axis(2)
                if dist.get_rank() == 0:
                    print(
                        f"Zero Parallel: pp {self.pp_size}, outer_dp {ep_size}, inner_dp {extra_dp_size}"
                    )
        else:
    ...
    def configure(
        ...
        if optimizer is not None and not isinstance(optimizer, OptimizerWrapper):
            if self.zero_stage == 0:
                ...
                assert (
                    self.dp_size > 1
                ), "Please use Zero when data parallel size is greater than 1."
                assert (
                    self.precision != "fp32"

...
class HybridParallelModule(ModelWrapper, AMPModelMixin):
    ...
    def sync_shared_params(self):
        for shared_param, group in zip(
            self.shared_params, self.shared_param_process_groups, strict=False
        ):
    ...
    def sync_sp_grads(self, grads: list[torch.Tensor] | None = None):
        ...
        if self.shard_config.enable_sequence_parallelism:
            ...
            if self.shard_config.sequence_parallelism_mode in ["split_gather", "ring"]:
                ...
                raise ValueError(
                    f"Unknown sequence parallelism mode: {self.shard_config.sequence_parallelism_mode}"
                )
...
class HybridParallelNaiveOptimizer(OptimizerWrapper):
    def __init__(
        ...
        max_norm: float = 0,
        tp_process_group: ProcessGroup | None = None,  # if using tp
        pp_process_group: ProcessGroup | None = None,  # if using pp
    ):
    ...
    def _compute_grad_norm(
        self, param_gradient_pairs: list[tuple[Tensor]], norm_type: int = 2
    ) -> int:
        r"""
        ...
        if norm_type == inf:
            ...
            if self.tp_size > 1:
                dist.all_reduce(
                    tensor=total_norm_cuda, op=dist.ReduceOp.MAX, group=self.tp_pg
                )
            if self.pp_size > 1:
                dist.all_reduce(
                    tensor=total_norm_cuda, op=dist.ReduceOp.MAX, group=self.pp_pg
                )
            ...
            grad_to_param_mapping = {
                id(grad): param for param, grad in param_gradient_pairs
            }

            total_norm_exponentiated = 0.0
            for grad in gradients:
                grad_norm_exponentiated = (
                    grad.data.double().norm(norm_type) ** norm_type
                )
            ...
            if self.tp_size > 1:
                ...
                dist.all_reduce(
                    tensor=total_norm_exponentiated_cuda,
        ...
        if norm_type == inf:
            ...
            for grad in gradients:
                ...
                if self.pp_size > 1:
                    for shared_param in self.shared_params:
                        if self.stage_manager.stage in shared_param:
                            stage_working_shared_param = shared_param[
                                self.stage_manager.stage
                            ]
                            stage_master_shared_param = self.working_to_master_map[
                                stage_working_shared_param
                            ]
...
class HybridParallelZeroOptimizer(LowLevelZeroOptimizer):
    def __init__(
        ...
        dp_process_group: ProcessGroup | None = None,  # the dp pg for comm
        tp_process_group: ProcessGroup | None = None,  # if using tp
        pp_process_group: ProcessGroup | None = None,  # if using pp
    ...
    def _sync_sp_grads(self):
        ...
        def _get_grads_to_sync(all_working_grads) -> list[Tensor] | None:
        ...
        if self.require_grad_sync and grads_to_sync is not None:
            ...
            SeqParallelUtils.allreduce_partial_data_grad(
                process_group=self.tp_pg, grads=grads_to_sync
            )
        else:
            return
    ...
    def backward(self, loss, retain_graph=False):
        ...
        if (
            self.require_grad_sync
            and self.model.shard_config.enable_sequence_parallelism
        ):
            # If gradient synchronization is required, sync sequence parallelism gradients.
            self._sync_sp_grads()
        else:
    ...
    def _compute_grad_norm(self, gradients: list[Tensor], norm_type: int = 2) -> float:
        ...
        if norm_type == inf:
            ...
            for grad in gradients:
                ...
                if tp_size > 1:
                    param_id_for_grad = self._grad_store.get_param_id_for_grad(grad)
                    param_for_grad = ctypes.cast(
                        param_id_for_grad, ctypes.py_object
                    ).value
                ...
                if pp_size > 1:
                    for shared_param in self.shared_params:
                        if self.stage_manager.stage in shared_param:
                            stage_shared_param = shared_param[self.stage_manager.stage]
                            working_grad = (
                                self._grad_store.get_working_grad_by_param_id(
                                    id(stage_shared_param)
                                )
                            )
...
class HybridParallelPlugin(PipelinePluginBase):
    ...
    def __init__(
        ...
        sequence_parallelism_mode: str | None = None,
        ...
        num_layers_per_stage: list[int] | None = None,
        gradient_checkpoint_config: GradientCheckpointConfig | None = None,
        ...
        if enable_sequence_parallelism:
            self.sequence_parallelism_mode = (
                ...
                if sequence_parallelism_mode is not None
                else "1"
            )
            assert (
            ...
            elif self.sequence_parallelism_mode == "all_to_all":
                assert (
                    tp_size == 1
            ...
            ), "sp_size can only be set to a >1 number when enable_sequence_parallelism is True"
            self.sp_size = 1
        ...
        if dp_outside:
            ...
            self.pg_mesh = ProcessGroupMesh(
                self.dp_size, self.pp_size, self.tp_size, self.sp_size
            )
        else:
            self.pp_axis, self.dp_axis, self.tp_axis, self.sp_axis = 0, 1, 2, 3
            self.pg_mesh = ProcessGroupMesh(
                self.pp_size, self.dp_size, self.tp_size, self.sp_size
            )
        ...
        if self.pp_size > 1:
            assert pp_style in [
                "1f1b",
                "interleaved",
            ], "Unsupported pipeline parallelism style"
            assert (
                pp_style == "interleaved" or num_model_chunks == 1
            ...
            if pp_style == "interleaved":
                assert (
                    num_model_chunks > 1
                ), "number of model chunks must be > 1 when using interleaved"
        ...
        if self.enable_sequence_parallelism and self.sequence_parallelism_mode in [
            "split_gather",
            "ring",
        ]:
    ...
    def configure(
        ...
        if not isinstance(model, ModelWrapper):
            use_ddp = (
                self.dp_size > 1 and self.pp_size == 1 and self.zero_stage == 0
            ) or (
                self.dp_size == 1
                and self.pp_size == 1
            ...
            if (
                ...
                dp_group = self.pg_mesh.create_group_along_axis(
                    [self.dp_axis, self.sp_axis]
                )
            else:
    ...
    def execute_pipeline(
        ...
        if model.require_grad_sync is False or (
            isinstance(optimizer, HybridParallelZeroOptimizer)
            and optimizer.require_grad_sync is False
        ):
            return outputs
    ...
    def get_checkpoint_io(self) -> CheckpointIO:
        return HybridParallelCheckpointIO(
            self.dp_group, self.pp_group, self.tp_group, self.zero_stage
        )

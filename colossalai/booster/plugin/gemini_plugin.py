...
class GeminiCheckpointIO(GeneralCheckpointIO):
    ...
    def save_unsharded_optimizer(
        self, optimizer: GeminiOptimizer, checkpoint: str, gather_dtensor: bool
    ):
        """
    ...
    def save_sharded_model(
        ...
        state_dict_shard = model.state_dict_shard(
            max_shard_size=max_shard_size, only_rank_0=True
        )
    ...
    def save_sharded_optimizer(
        ...
        state_dict_shard = optimizer.state_shard(
            prefix=prefix, max_shard_size=size_per_shard, only_rank_0=True
        )
    ...
    def load_sharded_optimizer(
        self, optimizer: GeminiOptimizer, checkpoint_index_file: Path, prefix: str
    ):
        """
        ...
        for shard_file in checkpoint_files:
            state_dict_shard = load_shard_state_dict(
                Path(shard_file), use_safetensors=False
            )
...
class GeminiPlugin(DPPluginBase):
    ...
    def __init__(
        ...
        chunk_init_device: torch.device | None = None,
        ...
        self.gemini_config = {
            ...
            "chunk_init_device": (
                chunk_init_device or get_accelerator().get_current_device()
            ),
        ...
        self.enable_sequence_parallelism = (
            enable_sequence_parallelism if self.enable_tensor_parallelism else False
        )
        ...
        ), "The global group size can't be evenly divided by the subgroup size."

        self.pg_mesh = ProcessGroupMesh(
            self.zero_size, self.extra_dp_size, self.tp_size
        )
        self.zero_group = (
            self.pg_mesh.get_group_along_axis(ZERO_AXIS)
            if self.zero_size < world_size
            else _get_default_group()
        )
        self.extra_dp_group = (
            self.pg_mesh.get_group_along_axis(DP_AXIS)
            if self.extra_dp_size > 1
            else None
        )
        self.tp_group = (
            self.pg_mesh.get_group_along_axis(TP_AXIS) if self.tp_size > 1 else None
        )

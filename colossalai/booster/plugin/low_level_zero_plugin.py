...
class LowLevelZeroCheckpointIO(TorchDDPCheckpointIO):
    def save_unsharded_optimizer(
        self, optimizer: OptimizerWrapper, checkpoint: str, gather_dtensor: bool = False
    ):
    ...
    def load_sharded_optimizer(
        self, optimizer: OptimizerWrapper, index_file_path: str, prefix: str
    ):
                ...
                for k, v in state.items():
                    if isinstance(v, torch.Tensor) and k != "step":
                        padding_size = (
                            ...
                            - v.numel() % self.coordinator.world_size
                        ) % self.coordinator.world_size
                        with torch.no_grad():
                            v = v.flatten()
                            if padding_size > 0:
                            ...
                            state_dict[param_idx][k] = (
                                v_list[self.coordinator.rank].detach().clone()
                            )
            load_states_into_optimizer(optimizer, state_dict, id_map)
        sharded_optimizer_loading_epilogue(optimizer)

    def load_unsharded_model(
        self, model: ModelWrapper, checkpoint: str, strict: bool = True
    ):
        assert isinstance(
    ...
    def load_sharded_model(
        ...
        super().load_sharded_model(
            model, checkpoint_index_file, strict, use_safetensors, load_sub_module
        )
        model.update_master_params()
...
class LowLevelZeroPlugin(DPPluginBase):
    ...
    def __init__(
        ...
        assert norm_type == 2.0, "LowLevelZeroPlugin only supports norm_type=2.0 now"
        self.stage = stage
        ...
        self.__class__.__name__ = f"LowLevelZeroPlugin_ZeRO-{stage}"

...
def assert_equal(a: Tensor, b: Tensor):
    assert torch.all(
        a == b
    ), f"expected a and b to be equal but they are not, {a} vs {b}"


def assert_not_equal(a: Tensor, b: Tensor):
    assert not torch.all(
        a == b
    ), f"expected a and b to be not equal but they are, {a} vs {b}"
...
def check_state_dict_equal(
    ...
    for k, v1 in d1.items():
        ...
        if isinstance(v1, dict):
            ...
            for v1_i, v2_i in zip(v1, v2, strict=False):
...
def check_state_dict_equal_pytree(
    d1: OrderedDict, d2: OrderedDict, ignore_device: bool = True
):
    flat_d1, _ = tree_flatten(d1)
    flat_d2, _ = tree_flatten(d2)
    assert len(flat_d1) == len(flat_d2)
    for v1, v2 in zip(flat_d1, flat_d2, strict=False):
    ...
    if isinstance(out1, dict) and isinstance(out2, dict):
        ...
        if out1.shape != out2.shape:
            raise AssertionError(
                f"{track_name}: shape mismatch: {out1.shape} vs {out2.shape}"
            )
        (
            assert_close(out1, out2, atol=atol, rtol=rtol),
            f"{track_name}: tensor value mismatch\nvalue 1: {out1}\nvalue 2: {out2}, \nmean error: {torch.abs(out1 - out2).mean()}",
        )
    else:
        assert (
            out1 == out2
        ), f"{track_name}: value mismatch.\nout1: {out1}\nout2: {out2}"

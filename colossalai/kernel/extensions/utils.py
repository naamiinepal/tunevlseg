...
def print_rank_0(message: str) -> None:
    ...
    try:
        ...
        is_main_rank = True if not dist.is_initialized() else dist.get_rank() == 0
    except ImportError:
        is_main_rank = True
...
def get_cuda_bare_metal_version(cuda_dir) -> list[int]:
    ...
    if cuda_dir is None:
        raise ValueError(
            "[extension] The argument cuda_dir is None, but expected to be a string. Please make sure your have exported the environment variable CUDA_HOME correctly."
        )

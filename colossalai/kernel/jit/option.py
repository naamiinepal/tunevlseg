...
def warmup_jit_fusion(
    ...
    embed = Embedding(vocab_size, hidden_size).to(
    ...
    for bias_grad, input_grad in zip([True, True], [False, True], strict=False):
        for _ in range(10):
            bias = torch.rand_like(
                y_bias, dtype=dtype, device=get_accelerator().get_current_device()
            )
            input_ = torch.rand_like(
                y, dtype=dtype, device=get_accelerator().get_current_device()
            )
    ...
    for input_grad, bias_grad, residual_grad in zip(
        [False, True], [True, True], [True, True], strict=False
    ):
        for _ in range(10):
            input_ = torch.rand_like(
                z, dtype=dtype, device=get_accelerator().get_current_device()
            )
            residual = torch.rand_like(
                x, dtype=dtype, device=get_accelerator().get_current_device()
            )
            bias = torch.rand_like(
                z_bias, dtype=dtype, device=get_accelerator().get_current_device()
            )

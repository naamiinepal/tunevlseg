...
def cai_gptq_matmul_248_kernel(
    ...
    if ADD_BIAS:
        ...
        bias = tl.load(
            bias_ptrs, mask=bias_mask, other=0.0
        )  # (BLOCK_SIZE_M, BLOCK_SIZE_K)
    ...
    if ADD_RESIDUAL:
        residual_ptrs = (
            residual_ptr + stride_cm * offs_am[:, None] + stride_cn * offs_bn[None, :]
        )
...
def gptq_fused_linear_triton(
    ...
    with torch.cuda.device(input.device):
        if qkv_fused:
            ...
            output = torch.empty(
                (input.shape[0] * 3, qweight.shape[1]),
                device=input.device,
                dtype=torch.float16,
            )
        else:

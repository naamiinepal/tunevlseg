...
if HAS_TRITON:
    ...
    def _token_attn_1_kernel(
        ...
        off_q = (
            current_batch * q_batch_stride
            + current_head * q_head_stride
            + offs_d * q_head_dim_stride
        )
        ...
        for start_mark in range(0, block_mask, 1):
            ...
            k_loc = tl.load(
                kv_cache_loc
                + kv_cache_loc_b_stride * current_batch
                + kv_cache_loc_s_stride * offs_n_new,
                mask=offs_n_new < current_batch_end_index,
                other=0,
            )
            off_k = (
                k_loc[:, None] * k_batch_stride
                + current_head * k_head_stride
                + offs_d[None, :] * k_head_dim_stride
            )
            k = tl.load(
                K + off_k, mask=offs_n_new[:, None] < current_batch_end_index, other=0.0
            )
            k = k.to(tl.float16) * k_input_scale.to(tl.float16)
            att_value = tl.sum(q[None, :] * k, 1)
            att_value *= sm_scale
            off_o = (
                current_head * attn_head_stride
                + (current_batch_in_all_start_index + offs_n) * attn_batch_stride
            )
            tl.store(
                attn_out + off_o, att_value, mask=offs_n_new < current_batch_end_index
            )
        return
    ...
    def _token_attn_2_kernel(
        ...
        v_loc_off = (
            current_batch * kv_cache_loc_b_stride
            + (current_batch_start_index + offs_n) * kv_cache_loc_s_stride
        )
        p_offs = (
            current_head * prob_head_dim_stride
            + (current_batch_in_all_start_index + offs_n) * prob_batch_stride
        )
    ...
    def smooth_token_attention_fwd(
        ...
        att_m_tensor = torch.empty(
            (head_num, total_token_num), dtype=torch.float32, device="cuda"
        )

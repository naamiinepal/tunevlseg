...
def alpa_dp_impl(
                        ...
                        for i in range(num_layers, k, -1):
                            ...
                            if stage_cost <= max_stage_cost and new_cost < f[s, k, d]:
                                ...
                                f_stage_max[s, k, d] = max(
                                    stage_cost,
                                    f_stage_max[s - 1, i, d - n_submesh_devices],
                                )
    ...
    while current_s > 0 and current_layer < num_layers and current_devices > 0:
        next_start_layer, submesh_choice, autosharding_choice = f_argmin[
            current_s, current_layer, current_devices
        ]
        assert next_start_layer != -1 and current_devices != -1
        res.append(
            ((current_layer, next_start_layer), submesh_choice, autosharding_choice)
        )
        current_s -= 1

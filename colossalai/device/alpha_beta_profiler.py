...
class AlphaBetaProfiler:
    ...
    def __init__(
        ...
        alpha_beta_dict: dict[tuple[int, int], tuple[float, float]] | None = None,
        ctype: str = "a",
        warmup: int = 5,
        repeat: int = 25,
        latency_iters: int = 5,
    ...
    def profile_ab(self):
        ...
        def get_max_nbytes(process_group: tuple[int], pg_handler: dist.ProcessGroup):
            ...
            return min(
                int(1 * GB), int(GB << int(math.log2(rank_max_nbytes.item() / GB)))
            )
        ...
        for process_group, pg_handler in self.process_group_dict.items():
            if rank not in process_group:
                ...
                bandwidth = self.profile_bandwidth(
                    process_group, pg_handler, maxbytes=max_nbytes
                )
    ...
    def search_best_logical_mesh(self):
        ...
        def _detect_homogeneous_device(alpha_beta_dict):
            ...
            for process_group, (_, beta) in alpha_beta_dict.items():
                ...
                for beta_value in homogeneous_device_dict:
                    ...
                    ) and beta >= beta_value * (1 - self.homogeneous_tolerance):
        ...
        def _construct_largest_ring(homogeneous_group: list[tuple[int]]):
            ...
            for rank in self.physical_devices:
                ...
                while not stable_status:
                    ...
                    rank_to_check_list = []
                    for _i in range(len(check_rank_list)):
                        check_rank = check_rank_list.pop()
                        for process_group in homogeneous_group:
                            if check_rank in process_group:
                                rank_to_append = (
                                    process_group[0]
                                    if process_group[1] == check_rank
                                    else process_group[1]
                                )
        ...
        row_size, column_size = (
            balanced_logical_mesh_shape[0],
            balanced_logical_mesh_shape[1],
        )
        balanced_logical_mesh = []
        for row_index in range(row_size):
            balanced_logical_mesh.append([])
            for column_index in range(column_size):
                balanced_logical_mesh[row_index].append(
                    self.physical_devices[row_index * column_size + column_index]
                )

        homogeneous_device_dict = _detect_homogeneous_device(self.alpha_beta_dict)
        beta_list = list(homogeneous_device_dict.keys())
        beta_list.sort()
        beta_list.reverse()
    ...
    def extract_alpha_beta_for_device_mesh(self):
        ...
        first_latency, first_bandwidth = _extract_alpha_beta(
            first_axis, first_axis_process_group
        )
        second_latency, second_bandwidth = _extract_alpha_beta(
            second_axis, second_axis_process_group
        )

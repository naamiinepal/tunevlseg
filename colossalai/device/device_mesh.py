...
class DeviceMesh:
    ...
    def __init__(
        ...
        mesh_alpha: list[float] | None = None,
        mesh_beta: list[float] | None = None,
        ...
        assert (
            torch.unique(self._physical_mesh_id).numel()
            == self._physical_mesh_id.numel()
        ...
        assert (
            len(self.mesh_alpha) == len(self.mesh_beta)
    ...
    def from_process_group(
        ...
        device_mesh = DeviceMesh(
            physical_mesh_id=fake_physical_mesh_id,
        ...
        device_mesh._process_group_dict = {
            device_mesh._global_rank_of_current_process: dict(enumerate(process_group))
        }

        return device_mesh

    def get_process_group(
        self, axis: int, global_rank: int | None = None
    ) -> ProcessGroup:
        """
    ...
    def get_process_group_for_all_axes(
    ...
    def get_ranks_in_process_group(
        self, axis: int, global_rank: int | None = None
    ) -> list[int]:
        """
    ...
    def _init_global_to_logical_rank_mapping(
        self, mapping: dict, tensor: torch.Tensor, index_list: list[int] | None = None
    ) -> dict[int, list[int]]:
        """
        ...
        for index, inner_tensor in enumerate(tensor):
            ...
            if inner_tensor.numel() == 1:
                ...
                self._init_global_to_logical_rank_mapping(
                    mapping, inner_tensor, [*index_list, index]
                )
    ...
    def init_logical_process_group(self):
        ...
        for global_rank in global_rank_flatten_list:
            ...
            ranks_in_same_group_by_axis = (
                self._collate_global_ranks_in_same_process_group(global_rank)
            )
            ...
            for axis, ranks_in_same_group in ranks_in_same_group_by_axis.items():
                ...
                pg_handler = dist.new_group(
                    ranks=ranks_in_same_group, backend=self._dist_backend
                )
    ...
    def _init_ranks_in_the_same_group(self):
        ...
        for global_rank in global_rank_flatten_list:
            ...
            for axis, ranks_in_same_group in ranks_in_same_group_by_axis.items():
                ...
                # keep this process group in the process_groups_dict
                self._ranks_in_the_process_group[global_rank][axis] = (
                    ranks_in_same_group
                )

    def global_rank_to_local_rank(
        self, rank: int, axis: int | None = None
    ) -> list[int] | int:
        """
    ...
    def _collate_global_ranks_in_same_process_group(self, global_rank):
        ...
        for dim in range(self.logical_mesh_id.dim()):
            ...
            for _local_rank in range(self.logical_mesh_id.shape[dim]):
                ...
                process_coordinates = self._global_to_local_rank_mapping[
                    global_rank
                ].copy()
        ...
        for (
            dim,
            coordinates_of_all_processes,
        ) in processes_in_the_same_process_group.items():
            global_pg_ranks[dim] = []
            for process_coordinates in coordinates_of_all_processes:
                ...
                ) in self._global_to_local_rank_mapping.items():

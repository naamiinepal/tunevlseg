...
class Recorder:
    ...
    def visualize_record(self):
        with open(self.export_name + ".json", encoding="utf-8") as f:
            records = json.load(f)
        records = dict(records)
        ranks = sorted(records.keys())
        ...
        plt.yticks(
            ticks=ranks,
            labels=[f"Device:{rank}" for rank in ranks],
            fontsize=self.device_fontsize,
        )
        plt.grid(axis="x")
        plt.savefig(f"{self.export_name}.{self.export_format}")

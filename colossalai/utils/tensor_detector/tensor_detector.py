...
class TensorDetector:
        ...
        for obj in gc.get_objects():
            if torch.is_tensor(obj):
                ...
                if id(obj) not in self.tensor_info:
                    ...
                    if (
                        id(obj) in self.saved_tensor_info
                        and name == self.saved_tensor_info[id(obj)][0]
                    ):
                        continue
    ...
    def print_tensors_state(self):
        ...
        outdated = [x for x in self.saved_tensor_info if x in self.order]
        minus = [x for x in self.saved_tensor_info if x not in self.detected]
        ...
        if self.log is not None:
            with open(self.log + ".log", "a", encoding="locale") as f:
                f.write(self.info)

...
class CudaAccelerator(BaseAccelerator):
    ...
    def __init__(self):
        super().__init__(
            name="cuda", communication_backend="nccl", is_synchronous=False
        )
    ...
    def set_device(self, device: torch.device | int | None = None) -> None:
        """
        ...
        if device is None:
            if not dist.is_initialized():
                raise RuntimeError(
                    "Cannot get current device when distributed is not initialized."
                )
    ...
    def synchronize(self, device: torch.device | int = None):
        """

...
def check_installation():
    ...
    (
        colossalai_version,
        prebuilt_torch_version_required,
        prebuilt_cuda_version_required,
    ) = _parse_colossalai_version()
    ...
    if not cuda_version:
        ...
        sys_torch_cuda_compatibility = _is_compatible(
    ...
    if not cuda_version or not prebuilt_cuda_version_required:
        ...
        sys_colossalai_cuda_compatibility = _is_compatible(
            [cuda_version, prebuilt_cuda_version_required]
        )
    ...
    if prebuilt_torch_version_required is None:
        ...
        torch_compatibility = _is_compatible(
            [torch_version, prebuilt_torch_version_required]
        )
    ...
    click.echo(
        f"CUDA version required by PyTorch: {to_click_output(torch_cuda_version)}"
    )
    click.echo("")
    click.echo("Note:")
    click.echo(
    ...
    click.echo(
        f"PyTorch version used for AOT compilation: {to_click_output(prebuilt_torch_version_required)}"
    )
    click.echo(
        f"CUDA version used for AOT compilation: {to_click_output(prebuilt_cuda_version_required)}"
    )
    click.echo("")
    click.echo("Note:")
    click.echo(
    ...
    click.echo(
        f"System and PyTorch CUDA version match: {to_click_output(sys_torch_cuda_compatibility)}"
    )
    click.echo(
        f"System and Colossal-AI CUDA version match: {to_click_output(sys_colossalai_cuda_compatibility)}"
    )
    click.echo("")
    click.echo("Note:")
    click.echo(
...
def _is_compatible(versions):
    ...
    for idx, version_values in enumerate(zip(*versions, strict=False)):
...
def _parse_colossalai_version():
    ...
    try:
        torch_version_for_aot_build = colossalai.__version__.split("torch")[1].split(
            "cu"
        )[0]

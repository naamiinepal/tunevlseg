...
def fetch_hostfile(hostfile_path: str, ssh_port: int) -> HostInfoList:
    ...
    with open(hostfile_path, encoding="locale") as fd:
...
def parse_device_filter(
    device_pool: HostInfoList, include_str=None, exclude_str=None
) -> HostInfoList:
    ...
    if include_str and exclude_str:
        click.echo(
            "--include and --exclude are mutually exclusive, only one can be used"
        )
        exit()
...
def get_launch_command(
    ...
    node_rank: int,
    num_nodes: int,
    extra_launch_args: str | None = None,
) -> str:
    """
...
def launch_multi_processes(args: Config) -> None:
    ...
    if args.host and args.hostfile:
        click.echo(
            "Error: hostfile and hosts are mutually exclusive, only one is required"
        )
    ...
    if args.hostfile:
        ...
        active_device_pool = parse_device_filter(
            device_pool, args.include, args.exclude
        )

...
class NVMeOptimizer(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        if self.nvme_offload_fraction > 0.0:
            try:
                ...
                raise ModuleNotFoundError(
                    "Please install tensornvme to use NVMeOptimizer"
                )
        ...
        self.total_numel: int | None = None
        self.can_offload_numel: int | None = None
    ...
    def _pre_step(self, *state_keys: str) -> None:
        if self.total_numel is None:
            ...
            self.can_offload_numel = math.floor(
                self.total_numel * self.nvme_offload_fraction
            )

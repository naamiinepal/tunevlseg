...
def flop_count(
    module: torch.nn.Module | Callable = None, *args, verbose: bool = False, **kwargs
) -> Number:
    """
    ...
    def enter_module(name):
        def f(module, inputs):
            ...
            return create_backwards_pop(name)(*inputs)
...
def conv_flop_count(
    ...
    return batch_size * reduce(operator.mul, w_shape) * reduce(operator.mul, conv_shape)
...
def norm_flop_counter(affine_arg_index: int, input_arg_index: int) -> Callable:
    ...
    def norm_flop_jit(inputs: list[Any], outputs: list[Any]) -> Number:
        ...
        return reduce(operator.mul, input_shape) * (5 if has_affine else 4)

    return norm_flop_jit


def batchnorm_flop_jit(
    inputs: list[Any], outputs: list[Any], training: bool | None = None
) -> Number:
    if training is None:
        training = inputs[-3]

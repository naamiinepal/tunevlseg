...
class LazyTensor(torch.Tensor):
    ...
    _repr = True
    _meta_data: torch.Tensor | None = None  # shape, dtype, device
    _pre_op_fn: Callable[["LazyTensor"], None] = lambda *args: None
    ...
    def __init__(self, func, *args, meta_data=None, concrete_data=None, **kwargs):
        ...
        self._materialized_data: torch.Tensor | None = (
    ...
    def device(self) -> torch.device:
        return (
            self._materialized_data.device
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        is_inplace: bool = (
            func.__name__.endswith("_") and not (func.__name__.endswith("__"))
        ...
        def wrap(y, i=None):
            if isinstance(y, torch.Tensor):
                if y.is_meta:
                    ...
                    def fn(*a, **kw):
                        return func(*a, **kw) if i is None else func(*a, **kw)[i]

                    fn.__name__ = func.__name__
                    return LazyTensor(fn, *args, meta_data=y, **kwargs)
    ...
    def to(self, *args, **kwargs) -> torch.Tensor:
        if self._materialized_data is not None:
            return LazyTensor(
                lambda: None, concrete_data=self._materialized_data.to(*args, **kwargs)
            )
    ...
    def clone(self) -> "LazyTensor":
        ...
        return LazyTensor(factory_fn, self, meta_data=self._meta_data)
...
class LazyInitContext:
    ...
    def __init__(
        self,
        tensor_cls: _MyTensor | LazyTensor = LazyTensor,
        default_device: torch.device | str | int | None = None,
    ):
    ...
    def __enter__(self):
        if LazyInitContext._replaced:
            raise RuntimeError("LazyInitContext is not reentrant")
        ...
        def wrap_legacy_constructor(target, dtype):
            ...
            def wrapper(*args, **kwargs):
                ...
                if len(args) == 1:
                    ...
                    replaced, _orig = self.overrides["tensor"]
                    return replaced(*args, **kwargs)
                if _is_int_tuple(args):
                    # (tuple of ints size, *, torch.device device)
                    kwargs = {**kwargs, "dtype": dtype}
                    replaced, _orig = self.overrides["empty"]
                    return replaced(*args, **kwargs)
                raise TypeError(
        ...
        overrides.update(
            {
                target + "_like": wrap_factory_like_method(
...
def _apply_to_lazy_module(
    ...
    if verbose:
        non_lazy_numel_ratio = (
            non_lazy_numel / total_numel * 100 if non_lazy_numel != 0 else 0
        )

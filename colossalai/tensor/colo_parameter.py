...
def is_no_hook_op(func) -> bool:
    return (
        func.__name__.startswith("__") and func not in WHITE_LIST_FUNCS
    ) or func in NO_HOOK_FUNCS
...
def replace_args(args, kwargs, new_args):
    ...
    for k, v in zip(kwargs.keys(), new_args[len(args) :], strict=False):
        kwargs[k] = v
        ...
        if ColoParamOpHookManager.has_hook() and not is_no_hook_op(func):
            ...
            if len(params) > 0:
                with torch._C.DisableTorchFunction():
                    new_args = ColoParamOpHookManager.pre_op(

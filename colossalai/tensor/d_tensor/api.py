...
def _apply_layout(tensor, layout):
    ...
    return layout_converter.apply(
        tensor=tensor, source_layout=source_layout, target_layout=layout
    )


def distribute_tensor(
    tensor: torch.Tensor, device_mesh: DeviceMesh, sharding_spec: ShardingSpec
) -> torch.Tensor:
    """
    ...
    """
    assert not is_distributed_tensor(
        tensor
    ), "The input tensor is already a distributed tensor."
    dist_layout = Layout(
        device_mesh=device_mesh, sharding_spec=sharding_spec, global_shape=tensor.shape
    )
...
def init_as_dtensor(
    ...
    dist_layout = Layout(
        device_mesh=device_mesh, sharding_spec=sharding_spec, global_shape=global_shape
    )
...
def redistribute(
    dtensor: torch.Tensor, device_mesh: DeviceMesh, sharding_spec: ShardingSpec
) -> None:
    """
...
def to_global(dtensor: torch.Tensor) -> torch.Tensor:
    ...
    return layout_converter.apply(dtensor, dtensor.dist_layout, global_layout)


def shard_rowwise(
    tensor: torch.Tensor,
    group_or_device_mesh: ProcessGroup | DeviceMesh = None,
) -> torch.Tensor:
    """
...
def shard_colwise(
    tensor: torch.Tensor, group_or_device_mesh: ProcessGroup | DeviceMesh = None
) -> torch.Tensor:
    """
    Shard the first dim of the given tensor.
...
def sharded_tensor_to_existing_param(
    dtensor: torch.Tensor, param: torch.nn.Parameter
) -> None:
    assert is_distributed_tensor(
        dtensor
    ), "The input tensor is not a distributed tensor."
...
def compute_global_numel(dtensor: torch.Tensor) -> int:
    ...
    return reduce(operator.mul, dtensor.dist_layout.global_shape)
...
def _hijack_detach_and_clone_for_customized_distributed_tensor(
...
def distribute_tensor_with_customization(
    tensor: torch.Tensor, shard_fn, gather_fn: callable
):
    """
...
def init_tensor_as_customization_distributed(
    tensor: torch.Tensor, shard_fn, gather_fn: callable
):
    """
    Distribute the given tensor with the given shard_fn and gather_fn.
...
def to_global_for_customized_distributed_tensor(dtensor: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: The global tensor.
    """
    assert is_customized_distributed_tensor(
        dtensor
...
def customized_distributed_tensor_to_param(
    dtensor: torch.Tensor, requires_grad: bool = True
):
    """
...
def customized_distributed_tensor_to_existing_param(
    dtensor: torch.Tensor, param: torch.nn.Parameter
):
    """

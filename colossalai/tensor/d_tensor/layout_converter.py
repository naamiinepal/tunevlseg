...
class LayoutConverter(metaclass=SingletonMeta):
    ...
    def shard_transform_layout(self, source_layout: Layout) -> dict[Layout, CommSpec]:
        ...
        legal_sharding_dims = list(range(len(source_layout.device_mesh.shape)))
    ...
    def layout_converting(
        ...
        spec_pairs = (
            (str(source_spec.sharding_sequence), src_shape),
            (str(target_spec.sharding_sequence), dst_shape),
        )
        ...
        if spec_pairs in self.cached_solution:
            ...
            def _group_alive_check(cached_comm_action_sequence):
                ...
                used_process_groups = [
                    pg
                    for comm_spec in cached_comm_action_sequence
                    for pg in comm_spec.process_group_dict.values()
                ]
                ...
                for process_group in used_process_groups:
                    try:
                        ...
                        if (
                            ...
                            or str(e) == "The given group does not exist"
                        ):
                            return False
            ...
            cached_transform_path, cached_comm_action_sequence = self.cached_solution[
                spec_pairs
            ]
    ...
    def get_total_comm_cost(
        ...
        for layout, comm_spec in zip(
            transform_path, comm_action_sequence, strict=False
        ):

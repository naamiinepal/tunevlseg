...
class DimSpec:
        ...
        for source_spec in source_spec_list:
            for target_spec in target_spec_list:
                ...
                # all_gather(source) -> target
                    ...
                    and source_shard_list[:-1] == target_shard_list
                ):
    ...
    def dim_diff(self, other):
        ...
        Output:
            5
        """
        return self.difference_dict[(str(self), str(other))]
...
class ShardingSpec:
    ...
    def __init__(
        self,
        dim_size: int,
        dim_partition_dict: dict[int, list[int]] | None = None,
        sharding_sequence: list[DimSpec] | None = None,
    ):
        self.dims = dim_size
    ...
    def _sanity_check(self):
        ...
        if (
            list(self.dim_partition_dict.keys())
            and max(list(self.dim_partition_dict.keys())) >= self.dims
        ):
            raise ShardingOutOfIndexError(
    ...
    def __repr__(self):
        ...
        res_list.append(
            "\n\tshard_sequence: "
            + ",".join(str(dimspec) for dimspec in self.sharding_sequence)
        )
    ...
    def spec_diff(self, other):
        ...
        difference = 0
        for orig_dim_spec, other_dim_spec in zip(
            self.sharding_sequence, other.sharding_sequence, strict=False
        ):

...
def _all_gather(tensor, comm_spec):
    ...
    return torch.cat(tuple(tensor_list), comm_spec.gather_dim).contiguous()
...
def _split(tensor, comm_spec):
    ...
    return torch.narrow(tensor, dim, start, length).contiguous()
...
def _all_to_all(tensor, comm_spec):
    ...
    output_tensor_list = [
        torch.zeros(new_shape, dtype=tensor.dtype, device=tensor.device)
        for _ in range(world_size)
    ]
    dim = comm_spec.shard_dim
    length = tensor.shape[comm_spec.shard_dim] // world_size
    input_tensor_list = [
        torch.narrow(tensor, dim, length * i, length).contiguous()
        for i in range(world_size)
    ]
    group = process_group
    dist.all_to_all(output_tensor_list, input_tensor_list, group)
    return torch.cat(tuple(output_tensor_list), comm_spec.gather_dim).contiguous()
...
def _mix_gather(tensor, comm_spec):
    ...
    if comm_spec.logical_process_axes[0] == comm_spec.logical_process_axes[1]:
        ...
        cat_slice = [
            mesh_shape[comm_spec.logical_process_axes[0]],
            mesh_shape[comm_spec.logical_process_axes[1]],
        ]
        ...
        tmp_tensor_list = [
            torch.zeros(tmp_tensor_shape, dtype=tensor.dtype, device=tensor.device)
            for _ in range(cat_slice[1])
        ]
        for i in range(cat_slice[1]):
            tmp_tensor_list[i] = torch.cat(
                tuple(tensor_list[i * cat_slice[0] : (i + 1) * cat_slice[0]]),
                comm_spec.gather_dim[0],
            ).contiguous()
...
def _mix_split(tensor, comm_spec):
    ...
    if comm_spec.logical_process_axes[0] == comm_spec.logical_process_axes[1]:
        ...
        start = [
            (rank % rank_slice[0]) * length[0],
            (rank // rank_slice[0]) * length[1],
        ]
...
class CommSpec:
    ...
    def __init__(
        ...
        if isinstance(self.logical_process_axis, list):
            if not mix_gather:
                ...
                self.device_meshes = (
                    self.sharding_spec.device_mesh.flatten_device_meshes
                )
        ...
        if self.comm_pattern == CollectiveCommPattern.GATHER_FWD_SPLIT_BWD:
            res_list.extend(
                (
                    ...
                    f"logical_process_axis:{self.logical_process_axis})",
                )
            )
            ...
            res_list.extend(
                (
                    ...
                    f"gather_dim:{self.gather_dim}, ",
                    f"logical_process_axes:{self.logical_process_axes})",
                )
            )
    ...
    def get_comm_cost(self):
        ...
        """
        comm_size = reduce(
            operator.mul, self.sharding_spec.get_sharded_shape_per_device(), 1
        )
        cost_dict = {}
        if self.comm_pattern == CollectiveCommPattern.GATHER_FWD_SPLIT_BWD:
            forward_communication_cost = self.device_mesh.all_gather_cost(
                comm_size, self.logical_process_axis
            )
        ...
        if self.comm_pattern == CollectiveCommPattern.ALL2ALL_FWD_ALL2ALL_BWD:
            forward_communication_cost = self.device_mesh.all_to_all_cost(
                comm_size, self.logical_process_axis
            )
            # grad should have same shape as input tensor
            # all to all operation has same logical process axis as forward.
            backward_communication_cost = self.device_mesh.all_to_all_cost(
                comm_size, self.logical_process_axis
            )

        if self.comm_pattern == CollectiveCommPattern.ALLREDUCE_FWD_IDENTITY_BWD:
            forward_communication_cost = self.device_mesh.all_reduce_cost(
                comm_size, self.logical_process_axis
            )
        ...
        if self.comm_pattern == CollectiveCommPattern.IDENTITY_FWD_ALLREDUCE_BWD:
            ...
            backward_communication_cost = self.device_mesh.all_reduce_cost(
                comm_size, self.logical_process_axis
            )
        ...
        if self.comm_pattern == CollectiveCommPattern.SPLIT_FWD_GATHER_BWD:
            ...
            backward_communication_cost = self.device_mesh.all_gather_cost(
                comm_size, self.logical_process_axis
            )

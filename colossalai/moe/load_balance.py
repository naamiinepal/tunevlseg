...
class LoadBalancer:
    def __init__(
        ...
        group_swap_factor: float | None = 0.4,
    ) -> None:
        self.experts: MLPExperts = experts
        self.gate: nn.Parameter = gate
        self.moe_ep_group: ProcessGroup = ep_group
        self.moe_ep_ranks = MOE_MANAGER.parallel_info_dict[
            dist.get_world_size(self.moe_ep_group)
        ].ep_group_ranks
    ...
    @staticmethod
    def _swap_data(
        data: list, group_i: int, index_i: int, group_j: int, index_j: int
    ) -> None:
    ...
    def _normalize_data(data: list) -> list:
        ...
        return [[i / max_value for i in sublist] for sublist in data]
    ...
    def _get_swap_loss(
        ...
        for swap in swap_list:
            for group_id, index_id in zip(
                [group_i, group_j], [index_i, index_j], strict=False
            ):
    ...
    def _beam_search(
        ...
        origin_diff_list = [
            self._get_diff_from_avg(data, i, avg) for i in range(group_num)
        ]
            ...
            for group_size_i in range(group_size):
                for group_num_j in range(group_num_i + 1, group_num):
                    for group_size_j in range(group_size):
                        ...
                        origin_diff = (
                            origin_diff_list[group_num_i]
                            + origin_diff_list[group_num_j]
                        )
                        # swap data
                        self._swap_data(
                            new_data,
                        ...
                        new_diff = self._get_diff_from_avg(
                            new_data, group_num_i, avg
                        ) + self._get_diff_from_avg(new_data, group_num_j, avg)
                        # caculate score
                        ...
                        if new_score > 0:
                            ...
                            new_swap_list = [
                                *swap_list,
                                (group_num_i, group_size_i, group_num_j, group_size_j),
                            ]
    ...
    def _search_balance(
        ...
        beam_width: int | None = 8,
        group_swap_factor: float | None = 0.4,
        return_swapped_data: bool | None = False,
    ) -> tuple[list, list]:
        """
        ...
        while stop_flag is False:
            ...
            for i in range(len(results)):
                new_results.extend(
                    self._beam_search(results[i], beam_width, avg, group_swap_factor)
                )
            if len(new_results) == 0:
                stop_flag = True
                break
            new_results.sort(key=operator.itemgetter(1), reverse=True)
    ...
    def _swap_expert_param_and_optim(
        ...
        if master_weight_ptr is not None:
            ...
            master_weight_ptr.data.copy_(
                tmp.clone()
                .detach()
                .to(master_weight_ptr.device)
                .to(master_weight_ptr.dtype)
            )
        # exchange optim
        self._swap_expert_single_tensor(
            exp_avg_ptr, expert_idx, comm_group, send_first, comm_rank
        )
        self._swap_expert_single_tensor(
            exp_avg_sq_ptr, expert_idx, comm_group, send_first, comm_rank
        )
    ...
    def _swap_moe_param(self, swap_list: list, optim: LowLevelZeroOptimizer) -> None:
        ...
        # gather
        global_master_gate_weight = self._gather_global_dp_group(
            master_gate_weight
        ).view(gate_shape)
        global_gate_exp_avg = self._gather_global_dp_group(gate_exp_avg).view(
            gate_shape
        )
        global_gate_exp_avg_sq = self._gather_global_dp_group(gate_exp_avg_sq).view(
            gate_shape
        )
        assert (
            self.gate.shape
        ...
        global_gate_exp_avg = global_gate_exp_avg.view(-1).split(
            global_gate_exp_avg.numel() // self.global_dp_size
        )[self.global_dp_rank]
    ...
    def balance_load(self, optim: LowLevelZeroOptimizer) -> None:
        ...
        if dist.get_rank() == 0:
            if len(swap_list) > 0:
                print("[Load Balance] Applying expert swap...")
            else:
                print("[Load Balance] Invalid swap, skip...")

...
class MLPExperts(nn.Module):
    ...
    def __init__(
        ...
        expert_parallel: str | None = None,
        activation: Callable | None = None,
        drop_rate: float | None = 0,
        ...
        if expert_parallel is not None:
            self.num_local_experts, self.moe_info = MOE_MANAGER.get_info(
                num_experts, use_tp=expert_parallel == "TP"
            )
        ...
        with seed_ctx:
            if self.gated:
                torch.nn.init.normal_(
                    self.wi_gate, std=math.sqrt(0.1 / self.hidden_size)
                )
    ...
    def forward(
        ...
        if self.use_kernel and use_sparse:
            for i in range(e):
                x[i] = torch.nn.functional.pad(
                    x[i], (0, 0, 0, seq_len - x[i].shape[0]), mode="constant", value=0
                )

        x = torch.cat([x[i].unsqueeze(0) for i in range(e)], dim=0)
        x = x.reshape(inshape)
        x = x.transpose(0, 1).contiguous()
        return MoeOutGradScaler.apply(x, self.ep_size)

...
class MoECheckpointIO(HybridParallelCheckpointIO):
    ...
    def load_unsharded_model(
        self, model: nn.Module, checkpoint: str, strict: bool
    ) -> None:
    ...
    def load_sharded_model(
        self, model: nn.Module, checkpoint_index_file: Path, strict: bool = False
    ):
        """
        ...
        def _load(name: str):
            if name not in weight_map:
                raise ValueError(
                    f"{name} is not stored in checkpoint, please check your checkpointing configuration!"
                )
    ...
    def pre_save_model(self, model: nn.Module) -> dict:
        ...
        if self.pp_size > 1 and self.dp_rank == 0:
    ...
    def save_sharded_model(
        ...
        if os.path.isfile(checkpoint):
            logging.error(
                f"Provided path ({checkpoint}) should be a directory, not a file"
            )
            return
        ...
        if dist.get_rank() == 0:
            state_dict_shard = self._model_sharder(
                state_dict, size_per_shard=size_per_shard
            )
        ...
        for k, v in state_.items():
            if isinstance(v, torch.Tensor) and k != "step":
                if is_moe_tensor_flag:
                    ...
                    padding_size = (
                        self.dp_size - v.numel() % self.dp_size
                    ) % self.dp_size
                    with torch.no_grad():
                        v = v.flatten()
                        if padding_size > 0:
    ...
    def load_sharded_optimizer(
        ...
        def _get_param_id_from_optimizer_param(
            param: torch.Tensor,
            master_to_working_map: dict[int, torch.Tensor] | None = None,
            optimizer=None,
        ):
        ...
        for pg in optimizer.optim.param_groups:
            for param in pg["params"]:
                param_id = _get_param_id_from_optimizer_param(
                    param, master_to_working_map, optimizer
                )
        ...
        updated_groups = []
        for old_pg, saved_pg in zip(
            optimizer.optim.param_groups, saved_groups, strict=False
        ):
            ...
            for param in pg["params"]:
                ...
                for pid, state in list(state_dict.items()):
                    if pid in id_map:
                        param = id_map[pid]
                        if (
                            master_to_working_map is not None
                            and id(param) in master_to_working_map
                        ):
                            working_param = master_to_working_map[id(param)]
                        elif (
                            hasattr(optimizer, "moe_master_to_working_map")
                            ...
                            working_param = optimizer.moe_master_to_working_map[
                                id(param)
                            ]
                        else:
                            working_param = param
                        original_shape = optimizer.param_info["param2shape"][
                            id(working_param)
                        ]
    ...
    def load_unsharded_optimizer(self, optimizer: OptimizerWrapper, checkpoint: str):
        ...
        if self.coordinator.is_master():
            logging.warning(
                "Please avoid using unsharded checkpointing methods when dealing with large models!"
            )
        ...
        for pg in optimizer.optim.param_groups:
            for param in pg["params"]:
                ...
                if param_id is not None:
                    id_map[param_id] = param
        load_states_into_optimizer(
            optimizer.optim, state_dict["state"], id_map, strict=True
        )
        ...
        for k, v in state_.items():
            if isinstance(v, torch.Tensor) and k != "step":
                ...
                if is_moe_tensor(param):
                    ...
                    gather_tensor = [
                        torch.zeros_like(v)
                        for _ in range(dist.get_world_size(self.dp_group))
                    ]
    ...
    def save_sharded_optimizer(
        ...
        states_name, save_index_file, param_group_file = get_optimizer_base_filenames(
            prefix
        )
        index_file = CheckpointIndexFile(checkpoint)
        ...
        if self.pp_size == 1:
            ...
            save_index_file = save_index_file.replace(
                ".json", f"-stage-{self.pp_rank + 1:05d}.json"
            )
            ...
            if self.pp_rank == 0:
                ...
                for filename in os.listdir(tmp_index_file_folder):
                    stage_index_file = CheckpointIndexFile.from_file(
                        os.path.join(tmp_index_file_folder, filename)
                    )
                    final_index_file.metadata["total_size"] += (
                        stage_index_file.metadata["total_size"]
                    )
    ...
    def save_unsharded_optimizer(
        self, optimizer: OptimizerWrapper, checkpoint: str, gather_dtensor: bool
    ):
        """
        ...
        if self.pp_size == 1:
            ...
            state_dict = {
                "param_groups": optimizer.optim.param_groups,
                "state": local_states,
            }
            if self.coordinator.is_master():
            ...
            if self.coordinator.is_master():
                state_dict = {"param_groups": optimizer.optim.param_groups, "state": {}}

...
class HierarchicalAllToAll(torch.autograd.Function):
    ...
    def forward(
        ...
        num_group = (
            dist.get_world_size(inter_node_group) if inter_node_group is not None else 1
        )
    ...
    def backward(ctx: Any, *grad_outputs) -> tuple[Tensor, None, None]:
        return (
            HierarchicalAllToAll.forward(
                None, grad_outputs[0], ctx.comm_grps, ctx.src_rank
            ),
            None,
            None,
        )
...
class MoeDispatch(torch.autograd.Function):
    ...
    def backward(ctx, output_grad):
        ...
        d_tokens = MOE_KERNEL.dispatch_backward(
...
class MoeCombine(torch.autograd.Function):
    ...
    def forward(ctx, expert_tokens, logits, mask, dest_idx, ec):
        ...
        if MOE_KERNEL is None:
            load_moe()
        output = MOE_KERNEL.combine_forward(
            s, e, c, h, expert_tokens, logits, mask, dest_idx
        )
...
class AllToAllUneven(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.group = group
        return _all_to_all(
            inputs, input_split_sizes, output_split_sizes, group, overlap
        )

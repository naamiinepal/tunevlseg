...
class MoeRouter(nn.Module, ABC):
    ...
    def get_capacity(self, num_tokens, num_experts, ep_group=None):
        if ep_group is not None:
            num_tokens_tensor = torch.tensor(
                num_tokens, device=get_accelerator().get_current_device()
            )
    ...
    def set_aux_loss(
        self, router_probs: torch.Tensor, expert_indices: torch.Tensor, num_experts: int
    ) -> None:
...
class Top2Router(MoeRouter):
    ...
    def forward(
        ...
        cb_weight[indices, top1_idx[indices], rank1[indices]] += weight1[
            indices, top1_idx[indices]
        ]
        cb_weight[indices, top2_idx[indices], rank2[indices]] += weight2[
            indices, top2_idx[indices]
        ]
        sec_mask[indices, top1_idx[indices], rank1[indices]] |= mask1.bool()[
            indices, top1_idx[indices]
        ]
        sec_mask[indices, top2_idx[indices], rank2[indices]] |= mask2.bool()[
            indices, top2_idx[indices]
        ]
...
class TopKRouter(MoeRouter):
    ...
    def forward(
        ...
        _expert_gate, expert_index = torch.topk(router_probs, self.k_value)
        ...
        token_priority = token_priority.reshape(
            (num_groups, self.k_value, -1, num_experts)
        )
        # Shape: [num_groups, tokens_per_group, num_selected_experts, num_experts].
        ...
        combine_array = torch.einsum(
            "...te,...tec->...tec", router_probs, dispatch_mask
        )

...
class MoEManager(metaclass=SingletonMeta):
    ...
    def setup(
        ...
        if self.mode == "dynamic":
            ...
            assert (
                isinstance(fixed_dp_size, int)
                and isinstance(fixed_ep_size, int)
                and isinstance(fixed_pp_size, int)
    ...
    def get_info(
        self, num_experts: int, use_tp: bool = False
    ) -> tuple[int, MoeParallelInfo]:
        ...
        if ep_size not in self.parallel_info_dict:
            self.parallel_info_dict[ep_size] = get_moe_info(
                ep_size, dp_size, pp_size, ep_inside=self.use_ep_inside
            )
            if dist.get_rank() == 0:
    ...
    def get_loss(self):
        return self.router_aux_loss, self.router_z_loss

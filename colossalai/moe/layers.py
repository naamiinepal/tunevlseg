...
class SparseMLP(nn.Module):
    ...
    def __init__(
        ...
        router_noisy_policy: str | None = None,
        ...
        self.gate_weight = torch.nn.Parameter(
            torch.empty(num_experts, self.hidden_size)
        )
        ...
        if self.expert_parallel is not None:
            ...
            if enable_hierarchical_comm:
                self.ep_intra_src_rank, *self.ep_hierarchical_group = (
                    create_ep_hierarchical_group(get_ep_group_ranks(self.experts))
                )
        ...
        if self.enable_load_balance is True:
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.expert_parallel == "EP":
            expert_output = self._ep_process(
                dispatch_data, used_capacity, overlap=self.enable_comm_overlap
            )
    ...
    def _ep_process(
        ...
        if not overlap or dist.get_world_size(self.ep_group) == 1:
            if self.ep_hierarchical_group is not None:
                ...
                expert_input = expert_input.reshape(
                    self.ep_size, self.num_local_experts, -1, self.hidden_size
                )
                expert_output = self.experts(expert_input)
                return HierarchicalAllToAll.apply(
            ...
            expert_output = self.experts(expert_input)
            return AllToAll.apply(expert_output, self.ep_group, False)[0]
        ...
        assert (
            dispatch_data.shape[1] % NUM_CHUNK == 0
    ...
    def _tp_process(
        ...
        if not overlap or dist.get_world_size(self.ep_group) == 1:
            ...
            return ReduceScatter.apply(expert_out, self.ep_group, False)[0]
    ...
    def _apply_recursive(module: nn.Module):
        for _, sub_module in module.named_children():
            if isinstance(sub_module, SparseMLP):
                if sub_module.enable_load_balance is True:

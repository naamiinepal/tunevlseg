...
class NormalNoiseGenerator:
    ...
    def __init__(self, num_experts: int):
        self.normal = torch.distributions.normal.Normal(
            ...
            scale=torch.tensor(
                1.0 / num_experts**2, device=get_accelerator().get_current_device()
            ),
        ).rsample
...
def create_ep_hierarchical_group(
...
) -> tuple[int, dist.ProcessGroup, dist.ProcessGroup | None]:
    """
    ...
    for i in range(num_node):
        ep_intra_ranks = [
            i * nproc_per_node + j for j in range(nproc_per_node) if j in ep_group_ranks
        ]

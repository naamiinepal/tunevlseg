...
class CheckpointIO(ABC):
    ...
    def save_model(
        ...
        if shard:
            ...
            self.save_unsharded_model(
                model, checkpoint, gather_dtensor, use_safetensors
            )
    ...
    def save_optimizer(
        ...
        if shard:
            self.save_sharded_optimizer(
                optimizer, checkpoint, gather_dtensor, prefix, size_per_shard
            )
        else:

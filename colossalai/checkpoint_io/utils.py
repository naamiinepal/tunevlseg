...
def search_tp_partition_dim(
    current_shape: torch.Size, original_shape: torch.Size, tp_size: int
) -> int | None:
    """
...
def search_padding_dim(
    global_shape: torch.Size, original_shape: torch.Size
) -> int | None:
    padding_dim = None
...
class StateDictSharder:
    ...
    def append_param(
        ...
        if (
            self.current_block_size + tensor_size > self.max_shard_size
    ...
    def append_optim_state(
        self, param_id: int, state: OrderedDict
    ) -> tuple[OrderedDict | None, int]:
        ...
        # we need to ensure that the current block is not empty
        if (
            self.current_block_size + state_size > self.max_shard_size
            and self.current_block_size > 0
        ):
            ret_block = self.current_block
            ret_block_size = self.current_block_size
...
def gather_distributed_param(
    param: torch.Tensor, keep_vars: bool = False
) -> torch.Tensor:
    """
    ...
    if is_customized_distributed_tensor(param_):
...
def shard_optimizer_checkpoint(
    state_dict: dict, max_shard_size: int = 1024
) -> Iterator[tuple[OrderedDict, int]]:
    """
...
def generate_checkpoint_shard_file_name(
    index: int, total_number: int, use_safetensors: bool, prefix: str | None = None
) -> str:
    """
...
def load_param_groups_into_optimizer(
    optimizer: Optimizer, param_group_path: str
) -> dict:
    """
    ...
    if not isinstance(saved_groups, list):
        raise ValueError(
            f"The param_groups saved at {param_group_path} is not of List type"
        )
    ...
    updated_groups = list(
        starmap(update_group, zip(param_groups, saved_groups, strict=False))
    )
...
def load_states_into_optimizer(
    optimizer: Optimizer, state_dict: dict, id_map: dict, strict: bool = False
):
    ...
    def cast(param, value, key=None):
        ...
        if isinstance(value, container_abcs.Iterable):
...
def has_index_file(checkpoint_path: str) -> tuple[bool, Path | None]:
    """
...
def add_prefix(weights_name: str, prefix: str | None = None) -> str:
...
def get_model_base_filenames(prefix: str | None = None, use_safetensors: bool = False):
    """
...
def get_optimizer_base_filenames(prefix: str | None = None):
    """

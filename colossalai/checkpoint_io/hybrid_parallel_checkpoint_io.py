...
class HybridParallelCheckpointIO(GeneralCheckpointIO):
    ...
    def _model_sharder(
        ...
        for name, buf in model.named_buffers():
            if buf is not None and name not in model._non_persistent_buffers_set:
                ...
                block, block_size = state_dict_sharder.append_param(
                    prefix + name, buffer
                )
                if block is not None:
                    yield block, block_size
    ...
    def load_sharded_model(
        self, model: ModelWrapper, checkpoint_index_file: Path, strict: bool = False
    ):
        """
        ...
        for n, m in model.named_modules():
            non_persistent_buffers |= {
                ".".join((n, b)) for b in m._non_persistent_buffers_set
            }
        for name, buf in model.named_buffers():
        ...
        remain_keys = reduce(operator.and_, map(set, missing_keys))
            ...
            if control_saving:
                ...
                param_groups = [
                    ...
                    for group, group_info in zip(
                        ...
                        optimizer.param_info["param_groups"],
                        strict=False,
                    )
                ]
    ...
    def load_unsharded_model(
        self, model: ModelWrapper, checkpoint: str, strict: bool = False
    ):
        """
    ...
    def save_unsharded_optimizer(
        ...
        for param, state in optimizer.optim.state.items():
            ...
            original_shape = optimizer.param_info["param2shape"][id(working_param)]
            local_states[param_id] = (
                HybridParallelCheckpointIO.gather_from_sharded_optimizer_state(
                    state,
                    working_param,
                    original_shape=original_shape,
                    dp_group=self.dp_group,
        ...
        if self.pp_size == 1:
            ...
            if self.coordinator.is_master():
                ...
                state_dict = {"param_groups": param_groups, "state": {}}
    ...
    def gather_from_sharded_optimizer_state(
        ...
        for k, v in state_.items():
            if isinstance(v, torch.Tensor) and k != "step":
                ...
                partition_dim = search_tp_partition_dim(
                    current_shape, original_shape, tp_size
                )
                ...
                if padding_dim is not None:
                    v = init_as_padded_tensor(
                        v,
                        v.shape[padding_dim],
                        original_shape[padding_dim],
                        padding_dim,
                    )
    ...
    def shard_from_complete_optimizer_state(
        ...
        for k, v in state_.items():
            if isinstance(v, torch.Tensor) and k != "step":
                ...
                partition_dim = search_tp_partition_dim(
                    current_shape, original_shape, self.tp_size
                )

...
def customized_split_pass_for_gpt2(
    gm: torch.fx.GraphModule, pp_size: int, partition_list: list[int]
):
    """
...
def split_with_split_nodes_pass_for_gp2_test(annotated_gm: torch.fx.GraphModule):
        ...
        for node in gm.graph.nodes:
            if node.op == "output":
                ...
                for name in output_args:
                    if (
                        next_partition_placeholders
                        and name not in next_partition_placeholders
                    ):
                        rm_list.append(name)
                for name in rm_list:
    ...
    for index, submodule in enumerate(submodules):
        placeholder_list = [] if index == 0 else placeholder_dict[submodules[index - 1]]
        submodule, placeholder_dict[submodule] = refill_outputs_and_placeholders(
            submodule, placeholder_list
        )
        submodule.recompile()
...
def split_module_for_gpt2_test(
    ...
    def record_cross_partition_use(
        def_node: torch.fx.node.Node, use_node: torch.fx.node.Node | None
    ):

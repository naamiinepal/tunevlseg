...
class MetaInfoProp(torch.fx.Interpreter):
    ...
    def run_node(self, n: Node) -> Any:
        ...
        n.node_size = activation_size(n.meta.get("fwd_out", 0)) + activation_size(
            n.meta.get("fwd_tmp", 0)
        )
        n.fwd_flop = n.meta.get("fwd_flop", 0)
        n.bwd_flop = n.meta.get("bwd_flop", 0)
    ...
    def summary(self, unit: str = "MB") -> str:
        ...
        assert (
            self._is_proped
        ), "Please call `interp.run(input)` before calling `interp.summary()`."
...
def metainfo_trace(
    gm: torch.fx.GraphModule, *args, verbose: bool = False, unit: str = "MB", **kwargs
) -> None:
    """

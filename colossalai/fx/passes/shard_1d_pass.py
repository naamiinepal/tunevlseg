...
def weight_split(
    weight: torch.nn.parameter.Parameter, dim: int, col_normal: bool
    ...
    """
    if col_normal:
        weight.fx_attr = dim, "SHARD", "TP", "col_normal"
    else:
        weight.fx_attr = dim, "SHARD", "TP", "col_needs_many_outputs"
    return weight
    ...
    for node in mod_graph.nodes:
        if node.op == "call_module":
            ...
            if isinstance(target_module, torch.nn.Linear):
                target_module.weight = weight_split(
                    target_module.weight, dim=0, col_normal=False
                )
                if target_module.bias is not None:
                    target_module.bias.data = weight_split(
                        target_module.bias.data, dim=0, col_normal=False
                    )
...
def row_shard_linear_pass(gm: torch.fx.GraphModule):
    ...
    for node in mod_graph.nodes:
        if node.op == "call_module":
            ...
            if isinstance(target_module, torch.nn.Linear):
                target_module.weight = weight_split(
                    target_module.weight, dim=-1, col_normal=False
                )
...
def transformer_mlp_pass(
    graph_module: torch.fx.GraphModule, process_group: ProcessGroup
):
    """
            ...
            if isinstance(module, nn.Linear):
                ...
                if start_tracking:
                    ...
                    for shard_type, module in annotation_record.items():
                        ...
                        if shard_type == "row":
                            dist_spec = ShardSpec(
                                dims=[-1], num_partitions=[world_size]
                            )
                            comp_spec = ComputeSpec(ComputePattern.TP1D)
                            module.weight.pg = process_group
                            module.weight.dist_spec = dist_spec
                            module.weight.comp_spec = comp_spec
                        elif shard_type == "col":
                            weight_dist_spec = ShardSpec(
                                dims=[0], num_partitions=[world_size]
                            )
                            weight_comp_spec = ComputeSpec(ComputePattern.TP1D)
                            weight_comp_spec.output_replicate = False
                            module.weight.pg = process_group
                            module.weight.dist_spec = weight_dist_spec
                            module.weight.comp_spec = weight_comp_spec

                            if module.bias is not None:
                                bias_dist_spec = ShardSpec(
                                    dims=[0], num_partitions=[world_size]
                                )
                                bias_comp_spec = ComputeSpec(ComputePattern.TP1D)
                                bias_comp_spec.output_replicate = False
                                module.bias.pg = process_group
                                module.bias.dist_spec = bias_dist_spec
                                module.bias.comp_spec = bias_comp_spec
    ...
    placeholder_node = next(iter(graph.nodes))
    annotate_record = {}

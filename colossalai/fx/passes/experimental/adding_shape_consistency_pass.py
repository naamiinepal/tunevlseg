...
def solution_annotation_pass(
    gm: torch.fx.GraphModule, solution: list[int], device_mesh
):
    mod_graph = gm.graph
    ...
    for node_index, (node, strategy_index) in enumerate(
        ...
        node.sharding_spec = strategies_vector[strategy_index].output_sharding_spec
    ...
    for node in nodes:
        if node.op == "call_module":
            ...
            origin_sharding_spec = ShardingSpec(
                device_mesh, target_module.weight.shape, {}
            )
            target_module.weight.sharding_spec = origin_sharding_spec
...
def shape_consistency_pass(gm: torch.fx.GraphModule):
    ...
    for node in nodes:
        ...
        for user_node in node.strategies_vector.successor_nodes:
            ...
            with mod_graph.inserting_before(user_node):
                mod_graph.create_node(
                    "call_function", apply, args=(node, sharding_spec_node)
                )

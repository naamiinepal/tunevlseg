    ...
    for node in gm.graph.nodes:
        ...
        if accumulate_fwd_flop + accumulate_bwd_flop >= per_block_flop:
            with gm.graph.inserting_after(node):
                ...
                block_node.fwd_flop = accumulate_fwd_flop
                block_node.bwd_flop = accumulate_bwd_flop
            accumulate_fwd_flop = 0
            accumulate_bwd_flop = 0
...
def get_compute_costs(node_list):
    ...
    for start in tqdm.tqdm(range(num_nodes), desc="start pos", position=0):
        for end in tqdm.tqdm(
            range(start, num_nodes), desc="end pos", position=1, leave=False
        ):
            selected_flops = [
                (node_list[i].fwd_flop + node_list[i].bwd_flop)
...
def do_dp_split_gpipe_impl(
    num_nodes, num_stages, num_microbatches, compute_costs, max_compute_cost
):
    ...
    for s in tqdm.tqdm(range(1, num_stages + 1), desc="stage", position=2, leave=False):  # pylint: disable=too-many-nested-blocks
        for i in tqdm.tqdm(
            range(num_nodes - 1, -1, -1), desc="start node", position=3, leave=False
        ):
            for k in tqdm.tqdm(
                range(num_nodes, i, -1), desc="mid node", position=4, leave=False
            ):
...
def gpipe_dp_split_pass(
    ...
    _best_cost, best_solution = do_dp_split_gpipe(
        node_list, compute_costs, pp_size, num_microbatches
    )
    ...
    for _, next_start_node in best_solution:
        ...
        with gm.graph.inserting_before(node):
            gm.graph.create_node("call_function", pipe_split)
        pp_size -= 1
...
def avgcompute_split_pass(gm: torch.fx.GraphModule, pp_size: int):
    ...
    check_node = next(iter(mod_graph.nodes))
    ...
    for node in mod_graph.nodes:
        ...
        if accumulate_fwd_flop >= partition_flop:
            ...
            with mod_graph.inserting_after(node):
                mod_graph.create_node("call_function", pipe_split)
    gm.recompile()
    return gm
...
def split_with_split_nodes_pass(annotated_gm: torch.fx.GraphModule, merge_output=False):
    ...
    split_submodules = []
    for _name, submodule in split_mod.named_modules():

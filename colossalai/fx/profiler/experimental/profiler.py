...
__all__ = ["profile_function", "profile_method", "profile_module"]
...
def profile_function(target: "Target") -> Callable:
    ...
    def f(*args: tuple[Argument, ...], **kwargs: dict[str, Any]) -> Any:
        ...
        return out, GraphInfo(
            fwd_flop, fwd_flop * 2, fwd_tmp, fwd_out, fwd_tmp + fwd_out, 0
        )
...
def profile_module(module: torch.nn.Module) -> Callable:
    ...
    def f(*args: tuple[Argument, ...], **kwargs: dict[str, Any]) -> Any:
        assert meta_profiler_module.has(type(module)), CALL_MODULE_MSG.format(
            type(module)
        )

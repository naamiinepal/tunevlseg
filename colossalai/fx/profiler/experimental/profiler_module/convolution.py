...
def torch_nn_conv1d(self: torch.nn.Conv1d, input: torch.Tensor) -> tuple[int, int]:
    ...
    l_out = math.floor(
        (l_in + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1)
        / self.stride[0]
        + 1
    )
...
def torch_nn_conv3d(self: torch.nn.Conv3d, input: torch.Tensor) -> tuple[int, int]:
    ...
    c_out = self.out_channels
    d_out = math.floor(
        (d_in + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1)
        / self.stride[0]
        + 1
    )
    h_out = math.floor(
        (h_in + 2 * self.padding[1] - self.dilation[1] * (self.kernel_size[1] - 1) - 1)
        / self.stride[1]
        + 1
    )
    w_out = math.floor(
        (w_in + 2 * self.padding[2] - self.dilation[2] * (self.kernel_size[2] - 1) - 1)
        / self.stride[2]
        + 1
    )
...
def torch_nn_convtranspose1d(
    self: torch.nn.ConvTranspose1d, input: torch.Tensor
) -> tuple[int, int]:
    # the output shape is calculated using the formula stated
...
def torch_nn_convtranspose3d(
    self: torch.nn.ConvTranspose3d, input: torch.Tensor
) -> tuple[int, int]:
    # the output shape is calculated using the formula stated

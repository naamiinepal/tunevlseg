...
def torch_nn_msa(
    ...
    # Initial projections
    flops += 2 * (
        (qlen * qdim * qdim) + (klen * kdim * kdim) + (vlen * vdim * vdim)
    )  # QW  # KW  # VW
    ...
    head_flops = (
        2 * (qlen * klen * qk_head_dim)
        + (qlen * klen)
        + 2 * (qlen * klen * v_head_dim)  # QK^T  # softmax  # AV
    )
    (qlen * klen * qk_head_dim) + 2 * (qlen * klen * v_head_dim)  # QK^T  # AV

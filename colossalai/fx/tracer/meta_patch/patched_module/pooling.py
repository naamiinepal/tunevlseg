...
def torch_nn_avgpool1d(self, input):
    ...
    assert num_dim in [
        2,
        3,
    ], f"expected the input to have 2 or 3 dimensions, but got {num_dim} dimensions"
...
def torch_nn_avgpool2d(self, input):
    ...
    assert num_dim in [
        3,
        4,
    ], f"expected the input to have 3 or 4 dimensions, but got {num_dim} dimensions"
    ...
    result_shape = (*tuple(input.shape[:-2]), h_out, w_out)
    return torch.empty(result_shape, device="meta")
...
def torch_nn_avgpool3d(self, input):
    ...
    assert num_dim in [
        4,
        5,
    ], f"expected the input to have 4 or 5 dimensions, but got {num_dim} dimensions"
    ...
    result_shape = (*tuple(input.shape[:-3]), d_out, h_out, w_out)
    return torch.empty(result_shape, device="meta")

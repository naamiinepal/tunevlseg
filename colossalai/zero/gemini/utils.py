...
def get_temp_total_chunk_on_cuda(chunk: Chunk, dtype: torch.dtype):
    ...
    total_temp = torch.zeros(
        chunk.chunk_size, dtype=dtype, device=get_accelerator().get_current_device()
    )
...
def _get_dfs_module_list(
    module: nn.Module, memo: set[nn.Module] | None = None, prefix: str = ""
):
    ...
    if module not in memo:
        for name, submodule in module._modules.items():
            ...
            yield from _get_dfs_module_list(submodule, memo, submodule_prefix)
...
def _get_shallow_copy_model(model: nn.Module):
    ...
    """
    old_to_new = {}
    for _name, module in _get_dfs_module_list(model):
...
def get_static_torch_model(
    ...
    if not only_rank_0 or dist.get_rank() == 0:
        for (name, colo_module), (_, torch_module) in zip(
            _get_dfs_module_list(colo_model),
            _get_dfs_module_list(torch_model),
            strict=False,
        ):
            # clean the parameter list of the new torch module
            torch_module._parameters = OrderedDict()
            for sufix_param_name, _param in colo_module.named_parameters(recurse=False):
                ...
                torch_param = torch.nn.Parameter(
                    state_param.data.to(device=device, dtype=dtype)
                )

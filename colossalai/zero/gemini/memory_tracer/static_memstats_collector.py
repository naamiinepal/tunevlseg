...
class StaticMemStatsCollector(ChunkMemStatsCollector):
    ...
    def init_mem_stats(self, *inputs):
        ...
        data = [
            MetaTensor(torch.rand(inp.shape, device="meta"), fake_device="cpu")
            for inp in inputs
        ]
        ...
        for node in gm.graph.nodes.__reversed__():
            cur_module_mem_fwd = (
                cur_module_mem_fwd + calculate_fwd_tmp(node) + calculate_fwd_out(node)
            )
            cur_module_mem_bwd = (
                cur_module_mem_bwd + node.meta["bwd_mem_tmp"] + node.meta["bwd_mem_out"]
            )
            if node.op == "call_module":
                if node.name.endswith("_0") and node.name[:-2] in module_name_list:
                    self._non_model_data_cuda_list.append(
                        total_mem + grad_module_out + cur_module_mem_bwd
                    )
    ...
    def register_opnodes_recursively(
        ...
        for child_name, child in module.named_children():
            self.register_opnodes_recursively(
                child, child_name, full_name + "_" + child_name, module
            )
        ...
        self.module_info_list.append(
            ModuleInfos(module, name, full_name[1:], parent_module)
        )

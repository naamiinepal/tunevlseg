...
__all__ = ["GeminiAdamOptimizer", "GeminiOptimizer"]
...
class GeminiOptimizer(OptimizerWrapper):
    ...
    def __init__(
        ...
        **defaults: Any,
        ...
        self.param_to_range: dict[Parameter, tuple[int, int]] = {}
        self.param_to_chunk16: dict[Parameter, Chunk] = {}
        ...
        self.id_to_real_params: dict[int, Parameter] = {}
        self.id_to_fake_params: dict[int, Parameter] = {}
        ...
        ), "gpu_margin_mem_ratio must >=0.0 and <=1.0"
        ...
        if (
            ...
            self._logger.warning(
                'gpu_margin_mem_ratio is meaningless when placement_policy is not "auto"',
                ranks=[0],
            )
    ...
    def _set_grad_ptr(self):
        for group in self.param_groups:
            for fake_param in group["params"]:
                ...
                grad_chunk16 = (
                    chunk16 if self.module.reuse_fp16_chunk else chunk16.grad_chunk
                )
                fake_param.data = grad_chunk16.payload[begin:end]
                fake_param.grad = fake_param.data

                to_update_chunk = (
                    chunk16.paired_chunk if self.module.master_weights else chunk16
                )
    ...
    def _calc_global_norm(self) -> float:
        ...
        comm_buffer = torch.zeros(
            1, dtype=torch.float, device=get_accelerator().get_current_device()
        )
                ...
                for fake_param in group["params"]:
                    ...
                    if (
                        ...
                        self.chunk_manager.move_chunk(
                            chunk32, get_accelerator().get_current_device()
                        )
                        # stores grad now
                        self.chunk_manager.move_chunk(
                            chunk16, get_accelerator().get_current_device()
                        )
                        self.module.set_chunk_grad_device(
                            chunk16, get_accelerator().get_current_device()
                        )
                        fp32_params_used_cuda_margin_mem += chunk32.payload_mem
    ...
    def collect_states(self, param_id: int, only_rank_0: bool = True) -> dict:
            ...
            if is_collector:
                ...
                for state_name in state_names:
                    if state_name == "step":
                        ...
                        state_tensor = (
                            states[state_name].detach().clone().to(torch.float32).cpu()
                        )
                        if is_dtensor:
                            global_shape = get_global_shape(param)
                            state_tensor = torch.reshape(state_tensor, param.shape).to(
                                param.device
                            )
                            ...
                            init_tensor_as_customization_distributed(
                                state_tensor,
                                shard_fn=param.shard_fn,
                                gather_fn=param.gather_fn,
                            )
                        state_tensor = gather_distributed_param(
                            state_tensor, keep_vars=False
                        ).cpu()
                        state_tensor = state_tensor.reshape(global_shape)
                        if is_padded_tensor(param):
                            state_tensor = init_as_padded_tensor(
                                state_tensor,
                                param._current_length,
                                param._origin_length,
                                param._padding_dim,
                            )
        ...
        compacted_states = (
            self.pack_optimizer_states_to_tensor(param_id, state_names)
            if own_param
            else None
        )
        ...
        dist.all_gather_object(
            gathered_state_shards,
            [compacted_states, shard_offset, shard_size],
            group=zero_group,
        )
        ...
        if is_collector:
            for state_name, state_tensor in collected_states.items():
                if state_tensor.numel() == param.numel():
                    collected_states[state_name] = torch.reshape(
                        state_tensor, param.shape
                    )
                if is_dtensor:
                    global_shape = get_global_shape(param)
    ...
    def pack_optimizer_states_to_tensor(
        ...
        compacted_states = torch.zeros(
            compacted_size, dtype=dtype, device=device, requires_grad=False
        )
        ...
        for state_name, state_tensor in states.items():
            ...
            if state_name == "step":
                ...
                compacted_states[
                    next_state_offset : next_state_offset + shard_size
                ].copy_(state_tensor)
        ...
        for state_name in state_names:
            if state_name == "step":
                collected_states["step"].data = torch.tensor(
                    compacted_states[next_state_offset].item(),
                    dtype=torch.float32,
                    requires_grad=False,
                ).cpu()
                next_state_offset += 1
                ...
                target_segment.copy_(
                    compacted_states[next_state_offset : next_state_offset + shard_size]
                )
                next_state_offset += shard_size
    ...
    def get_param_groups_for_saving(self) -> list:
        ...
        param_groups = [
            ...
            for group, group_info in zip(
                self.optim.param_groups, self.param_groups_backup, strict=False
            )
        ]
    ...
    def state_dict(self, only_rank_0: bool = True) -> dict:
        ...
        state_dict["state"] = {}
        for param_id in self.id_to_real_params:
            dist.barrier()
            state_dict["state"][param_id] = self.collect_states(
                param_id=param_id, only_rank_0=only_rank_0
            )
        return state_dict
    ...
    def load_single_param_states(self, param_id: int, saved_states: dict):
        ...
        def cast(param, state_range, value, global_shape, origin_shape, key=None):
            ...
            if key == "step":
                ...
                if is_padded_tensor(real_param):
                    ...
                    value = to_padded_tensor(
                        value, global_shape[padding_dim], padding_dim
                    )
                ...
                if is_dtensor:
                    ...
                    value = distribute_tensor_with_customization(
                        value, real_param.shard_fn, real_param.gather_fn
                    )
        ...
        for k, v in saved_states.items():
            updated_states[k] = cast(
                fake_param, state_range, v, global_shape, origin_shape, k
            )

...
class PlacementPolicy(ABC):
    ...
    def __init__(
        ...
        mem_stats_collector: ChunkMemStatsCollector | None = None,
        **kwargs,
    ) -> None:
        self.chunk_manager = chunk_manager
        self.mem_stats_collector: ChunkMemStatsCollector | None = mem_stats_collector
    ...
    def setup_grads_device(
        ...
        grads_device_map: dict[torch.Tensor, torch.device],
    ) -> None:
        raise NotImplementedError
...
class StaticPlacementPolicy(PlacementPolicy):
    def __init__(
        ...
        **kwargs,
        ...
        if offload_param_frac > 0.0 and (
            shard_param_frac != 1.0 or offload_optim_frac != 1.0
        ):
            warnings.warn(
                "offload_param_frac is ignored when shard_param_frac != 1.0 or offload_optim_frac != 1.0"
            )
    ...
    def setup_grads_device(
        ...
        chunks = {self.chunk_manager.get_chunk(p) for p in params}
        for chunk in chunks:
            ...
            if (
                chunk.keep_gathered
                or offloaded_optim_chunk_mem >= offload_optim_chunk_mem
            ):
...
class AutoPlacementPolicy(PlacementPolicy):
    ...
    def evict_tensors(
        ...
        cuda_demand: int = 0,
        warmup: bool = True,
        compute_list: list[tuple[Chunk, ...]] | None = None,
        compute_idx: int = 0,
        **kwargs,
    ) -> tuple[int, float]:
        """
        ...
        """
        start = time()
        cuda_capacity = colo_device_memory_capacity(
    ...
    def _sort_can_evict_chunks(
        ...
        next_compute_idx = sorted(
            next_compute_idx.items(), key=operator.itemgetter(1), reverse=True
        )

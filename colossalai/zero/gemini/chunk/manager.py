...
class ChunkManager:
    ...
    def __init__(
        self, chunk_configuration, init_device: torch.device | None = None
    ) -> None:
    ...
    def rearrange_accumulated_grad_chunk(self, chunk: Chunk) -> Chunk:
        ...
        if chunk.grad_chunk.is_gathered:
            accumulated_grad = (
                chunk.grad_chunk.cuda_global_chunk.clone().detach().mul_(chunk.pg_size)
            )
            accumulated_grad_gathered = True
        else:
            if chunk.grad_chunk.cuda_shard is not None:
                accumulated_grad = (
                    chunk.grad_chunk.cuda_shard.clone().detach().mul_(chunk.pg_size)
                )
            else:
                accumulated_grad = (
                    chunk.grad_chunk.cpu_shard.to(
        ...
        if accumulated_grad_gathered:
            ...
            grad_chunk.cuda_global_chunk[
                grad_chunk.shard_begin : grad_chunk.shard_end
            ].add_(accumulated_grad)

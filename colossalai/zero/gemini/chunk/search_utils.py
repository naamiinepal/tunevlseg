...
def search_chunk_configuration(
    ...
    for dp_degree in params_dict:
        ...
        if group_acc_size < min_chunk_size:
            config_dict[dp_degree] = {
                "chunk_size": group_acc_size,
                "keep_gathered": True,
            }
        else:
    ...
    for key in size_dict:
        max_size = max(max_size, *size_dict[key])
    ...
    for dp_degree in params_dict:
        if dp_degree in config_dict:
            continue
        config_dict[dp_degree] = {"chunk_size": best_chunk_size, "keep_gathered": False}

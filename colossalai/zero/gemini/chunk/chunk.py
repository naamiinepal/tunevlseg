...
class Chunk:
    ...
    def __init__(
        ...
        self.extra_dp_size = (
            dist.get_world_size(self.extra_dp_group)
        ...
        self.shard_device = (
            torch.device("cpu")
            if cpu_shard_init
            else get_accelerator().get_current_device()
        )
    ...
    def memory_usage(self) -> dict[str, int]:
        ...
        if self.chunk_temp is not None:
            ...
            if (
                self.chunk_temp.device.type == "cuda"
                or self.chunk_temp.device.type == "npu"
            ):
        ...
        return {"cuda": cuda_memory, "cpu": cpu_memory}
    ...
    def close_chunk(self):
        ...
        if self.chunk_temp.device.type == "cpu":
            self.cuda_global_chunk = self.chunk_temp.to(
    ...
    def add_tensor_to_chunk_slice(
        ...
        tensor_info = self.tensors_info[tensor]
        self.cuda_global_chunk[tensor_info.offset : tensor_info.end].add_(
            data_slice.data.flatten()
        )
    ...
    def optim_update(self) -> None:
        ...
        if self.is_gathered is True:
            ...
            self.optim_sync_flag = True
        elif friend_chunk.device_type in ("cuda", "npu") and self.device_type in (
            "cuda",
            "npu",
        ):
    ...
    def __update_tensors_state(
        self, next_state: TensorState, prev_state: TensorState | None = None
    ):
    ...
    def __repr__(self, detailed: bool = True):
        output = [
            "Chunk Information:\n",
            f"\tchunk size: {self.chunk_size}, chunk dtype: {self.dtype}, process group size: {self.pg_size}\n",
            f"\t# of tensors: {self.num_tensors}, utilized size: {self.utilized_size}, utilized percentage: {self.utilized_size / self.chunk_size:.2f}\n",
        ]

        def print_tensor(tensor, prefix=""):
            output.append(
                f"{prefix}shape: {tensor.shape}, dtype: {tensor.dtype}, device: {tensor.device}\n"
            )
        ...
        if detailed:
            ...
            for st in TensorState:
                output.append(f"\t\t# of {st}: {self.tensor_state_cnter[st]}\n")
    ...
    def init_grad_chunk(self) -> "Chunk":
        ...
        if self.grad_chunk is None:
            ...
            for tensor, state in self.tensors_info.items():
                grad_chunk.tensors_info[tensor] = TensorInfo(
                    TensorState.HOLD, state.offset, state.end
                )

            grad_chunk.valid_end = self.valid_end

            if grad_chunk.chunk_temp.device.type == "cpu":
                grad_chunk.cuda_global_chunk = grad_chunk.chunk_temp.to(
                    get_accelerator().get_current_device()
                )
            else:
            ...
            if grad_chunk.pin_memory:
                grad_chunk.cpu_shard = torch.empty(
                    grad_chunk.shard_size,
                    dtype=grad_chunk.dtype,
                    pin_memory=grad_chunk.pin_memory,
                )

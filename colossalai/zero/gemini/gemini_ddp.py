...
class GeminiDDP(ModelWrapper):
    ...
    def __init__(
        ...
        chunk_config_dict: dict | None = None,
        ...
        hidden_dim: int | None = None,  # chunk search options
        ...
        zero_group: ProcessGroup | None = None,
        memstats: MemStats | None = None,  # genimi memory stats
        master_weights: bool = True,
        extra_dp_group: ProcessGroup | None = None,
        verbose: bool = False,
    ) -> None:
        ...
        self.fp32_params: list[torch.Tensor] = []
        self.fp16_params: list[ColoParameter] = []
        self.overflow_counter = 0
        self.grads_device: dict[torch.Tensor, torch.device] = {}
        self.param2name: dict[nn.Parameter, str] = {}
        self.name2param: dict[str, nn.Parameter] = {}
        ...
        self._init_chunks(
            ...
            cpu_offload=not (
                self.gemini_manager.policy_name == "static" and offload_param_frac == 0
            ),
    ...
    def forward(self, *args, **kwargs):
        ...
        args, kwargs = (
            _cast_float(args, self.mixed_precision),
            _cast_float(kwargs, self.mixed_precision),
        )
    ...
    def _post_backward(self):
        ...
        if self.enable_gradient_accumulation and not self.accumulating_grads:
            self.accumulating_grads = (
                True  # Turn on the state of gradient accumulation.
            )
        self._logger.debug(
    ...
    def backward(self, loss: torch.Tensor):
        self._pre_backward()
        with (
            self.param_op_hook.switch_to_backward(),
            ColoParamOpHookManager.use_hooks(self.param_op_hook),
        ):
            loss.backward()
        self._post_backward()
    ...
    def backward_by_grad(self, tensor, grad):
        raise RuntimeError(
            "Gemini is not compatible with pipeline. backward_by_grad shoudn't be called in Gemini."
        )

    def grad_handle(self, p, grad):
        p._gemini_reduced = True
        ...
        with torch._C.DisableTorchFunction():
            ...
            if not self.reuse_fp16_chunk:
                if not self.accumulating_grads:
                    ...
                    if chunk.grad_chunk not in self.chunk_manager.accessed_chunks:
                        grad_chunk = (
                            self.chunk_manager.rearrange_accumulated_grad_chunk(chunk)
                        )
                    else:
            ...
            if not self.accumulating_grads:
                grad_chunk.copy_tensor_to_chunk_slice(
                    p, grad, update_ptr=self.reuse_fp16_chunk
                )
            else:
            ...
            if reduced:
                ...
                self.chunk_manager.move_chunk(
                    grad_chunk, self.grads_device[p], force_copy=True
                )
    ...
    def state_dict(
        ...
        destination._metadata[prefix[:-1]] = local_metadata = {"version": self._version}
    ...
    def _get_chunk_to_save_data(self, chunk: Chunk, only_rank_0: bool) -> dict:
        ...
        for tensor, tensor_info in chunk.tensors_info.items():
            ...
            if record_flag:
                ...
                record_tensor = gather_distributed_param(
                    record_tensor, keep_vars=False
                ).cpu()
                if is_padded_tensor(tensor):
                    record_tensor = init_as_padded_tensor(
                        record_tensor,
                        tensor._current_length,
                        tensor._origin_length,
                        tensor._padding_dim,
                    )
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars, only_rank_0=True):
        ...
        if self.reuse_fp16_chunk:
            for p, fp32_p in zip(self.fp16_params, self.fp32_params, strict=False):
                name = self.param2name[p]
                assert (
                    fp32_p in param_to_save_data
                ), f"Parameter '{name}' is neglected in the chunk list"
    ...
    def _load_from_state_dict(
        ...
        def load(
            ...
            if state_key in state_dict:
                ...
                if is_padded_tensor(dest_tensor):
                    ...
                    input_param = to_padded_tensor(
                        input_param, global_shape[padding_dim], padding_dim
                    )

                if source_device_mesh is not None and source_sharding_spec is not None:
                    input_param = distribute_tensor(
                        input_param, source_device_mesh, source_sharding_spec
                    )
                ...
                if input_param.shape != dest_tensor.shape:
                    ...
                    error_msgs.append(
                        f"size mismatch for {state_key}: copying a param with shape {input_param.shape} from checkpoint, "
                        f"the shape in current model is {dest_tensor.shape}."
                    )
                    return
                try:
                    with torch.no_grad():
                    ...
                    error_msgs.append(
                        f'While copying the parameter named "{state_key}", '
                        f"whose dimensions in the model are {dest_tensor.size()} and "
        ...
        for chunk in chunk_list:
            ...
            for tensor, tensor_info in chunk.tensors_info.items():
                source_device_mesh, source_sharding_spec, shard_fn, gather_fn = (
                    None,
                    None,
                    None,
                    None,
        ...
        for name, param in self.name2param.items():
            if param is not None:
                if is_ddp_ignored(param):
                    ...
                    param_to_save = (
                        fp16_to_fp32[param] if self.reuse_fp16_chunk else param
                    )
                    if param_to_save not in gathered_param_buffer:
                        ...
                        gathered_param_buffer.update(
                            self._get_chunk_to_save_data(chunk, only_rank_0)
                        )

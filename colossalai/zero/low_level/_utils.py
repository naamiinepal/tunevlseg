...
def shuffle_by_round_robin(tensor_list, num_partitions):
    ...
    for tensor_idx, tensor in enumerate(tensor_list):
        ...
        partitions[partition_to_go].append({"tensor": tensor, "index": tensor_idx})
...
def get_grad_accumulate_object(tensor):
    ...
    return tensor_tmp.grad_fn.next_functions[0][0]
...
def reduce_tensor_dp_group(
    ...
    dst_local_rank: int | None = None,
    dst_global_rank: int | None = None,
    ...
    tensor_to_reduce = tensor.to(dtype) if tensor.dtype != dtype else tensor
...
def sync_tensor(flat_tensor, tensor_list):
    ...
    for p, q in zip(tensor_list, updated_params, strict=False):
        p.data = q.data

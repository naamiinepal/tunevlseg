...
class LowLevelZeroOptimizer(OptimizerWrapper):
    ...
    def __init__(
        ...
        moe_extra_dp_process_group: ProcessGroup | None = None,
        ...
        if len(self.working_moe_params) > 0:
            ...
            for master_moe_param, working_moe_param in zip(
                self.master_moe_params, self.working_moe_params, strict=False
            ):
        ...
        self.mixed_precision_mixin: MixedPrecisionMixin | None = None
            ...
            with torch.no_grad():
                ...
                if self.moe_extra_dp_pg is not None and is_moe_tensor(param):
                    splited_params = padding_param.split(
                        padding_param.numel() // self.moe_extra_dp_pg_size
                    )
                ...
                if self._master_weights is True:
                    splited_param_current_rank = (
                        splited_params.detach().float().to(device)
                    )
                else:
                ...
                if not self._partition_grads:
                    if self.moe_extra_dp_pg is None:
                        ...
                        self._update_unpartitoned_grad(
                            grad_in_bucket.values(), flat_grads_per_rank, group_id
                        )
                    ...
                    # sync extra zero group
                        ...
                        if len(non_moe_grad_list) > 0:
                            ...
                            self._update_unpartitoned_grad(
                                non_moe_grad_list, flat_grads_per_rank, group_id
                            )
                        ...
                        if len(moe_grad_list) > 0:
                            ...
                            flat_grads_per_rank = moe_flat_grads.split(
                                moe_flat_grads.numel() // self._world_size
                            )
                            self._update_unpartitoned_grad(
                                moe_grad_list, flat_grads_per_rank, group_id
                            )
                    ...
                    if self.moe_extra_dp_pg is None:
                        ...
                        self._update_partitoned_grad(
                            grad_in_bucket_current_rank, recieved_grad, group_id, 1
                        )
                    else:
                        ...
                        if len(moe_grad_list) > 0:
                            flat_grads_list = list(
                                moe_flat_grads.split(
                                    len(moe_flat_grads) // self.moe_extra_dp_pg_size
                                )
                            )
                            recieved_grad = torch.zeros_like(flat_grads_list[0])
                            dist.reduce_scatter(
                            ...
                            recieved_grad = list(
                                recieved_grad.split(len(recieved_grad) // param_slice)
                            )
                            for split_recieved_grad in recieved_grad:
                                ...
                                for real_grad, grad in zip(
                                    split_recieved_grad,
                                    moe_grad_in_bucket_current_rank,
                                    strict=False,
                                ):
                                    param_id = self._bucket_store.get_param_id_of_grad(
                                        grad
                                    )
                                    self._add_grad(
                                        real_grad, param_slice, group_id, param_id
                                    )

                self._bucket_store.reset()

    def _update_unpartitoned_grad(
        self, origin_grad_list: list, flat_grad_list: list, group_id: int
    ) -> None:
    ...
    def zero_grad(self, set_to_none=True):
        ...
        for param_group in self._working_param_groups.values():
            for param in param_group:
                if set_to_none:
                    param.grad = None
                else:
    ...
    def step(self, closure=None):
        ...
        for group_id in range(self.num_param_groups):
            ...
            for splited_param in master_params:
                ...
                if len(grads) > 0:
                    # moe hybrid zero
                    if self.moe_extra_dp_pg is not None and is_moe_tensor(
                        working_param
                    ):
                        ...
                        if self._partition_grads:
                            ...
                            param_slice = self._world_size // self.moe_extra_dp_pg_size
                            grad = grads[
                                self.moe_extra_dp_pg_rank * param_slice : (
                                    self.moe_extra_dp_pg_rank + 1
                                )
                                * param_slice
                            ]
                        grad = flatten(grad)
        ...
        if len(self.working_moe_params) > 0:
            for master_moe_param, working_moe_param in zip(
                ...
                working_moe_param.data = (
                    master_moe_param.data.to(working_moe_param.device)
                    .to(working_moe_param.dtype)
                    .detach()
                )
        ...
        for group_id in range(self.num_param_groups):
            ...
            for idx, splited_param in enumerate(master_working_param):
                ...
                if self.moe_extra_dp_pg is not None and is_moe_tensor(working_param):
                    all_splited_param = [
                        torch.zeros(
                            splited_param.shape, device=device, dtype=self._dtype
                        )
            ...
            self.optim.param_groups[group_id]["params"] = (
                self._master_param_groups_of_current_rank[group_id]
            )
    ...
    def _compute_grad_norm(self, gradients: list[Tensor], norm_type: int = 2) -> float:
        ...
        if norm_type == inf:
            ...
            dist.all_reduce(
                total_norm_cuda, op=torch.distributed.ReduceOp.MAX, group=self.dp_pg
            )
    ...
    def state_dict(self) -> dict:
            ...
            for k, v in state.items():
                if isinstance(v, torch.Tensor) and k != "step":
                    ...
                    if self.moe_extra_dp_pg is not None and is_moe_tensor(v):
                        ...
                        dist.all_gather(
                            gather_tensor, v.to(device), group=self.moe_extra_dp_pg
                        )
                    else:
                        gather_tensor = [
                            torch.zeros(v.shape, device=device, dtype=v.dtype)
        ...
        return self._pack_state(zero_state)
                    ...
                    with torch.no_grad():
                        ...
                        if self.moe_extra_dp_pg is not None and is_moe_tensor(v):
                            ...
                            zero_state_dict["state"][param_idx][k] = (
                                v_list[self.moe_extra_dp_pg_rank].detach().clone()
                            )
                        else:
            ...
            for k, v in states.items():
                if isinstance(v, torch.Tensor) and k != "step":
                    if self.moe_extra_dp_pg is not None and is_moe_tensor(v):
                        ...
                        dist.all_gather(
                            state_tensor, v.to(device), group=self.moe_extra_dp_pg
                        )
                    else:
                        state_tensor = [
                            torch.zeros(v.shape, device=device, dtype=v.dtype)
        ...
        for p in model.parameters():
            ...
            if p_id in self._param_store.working_to_master_param:
                ...
                if self.moe_extra_dp_pg is not None and is_moe_tensor(p):
                    master_param.copy_(
                        working_param.chunk(self.extra_dp_pg_size)[
                            self.extra_dp_pg_rank
                        ]
                    )
                else:
                    master_param.copy_(

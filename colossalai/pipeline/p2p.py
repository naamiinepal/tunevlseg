    ...
    if my_rank == src:
        if Version(torch.__version__) >= Version("1.13.0"):
            tensor_list, size_list = zip(
                *[
                    c10d._object_to_tensor(obj, device=current_device)
            ...
            tensor_list, size_list = zip(
                *[c10d._object_to_tensor(obj) for obj in object_list], strict=False
            )
...
P2PMetadata = namedtuple(
    "P2PMetadata",
    ["tree_spec", "tensor_metadata", "non_tensor_obj_idx", "non_tensor_objs"],
)
    ...
    for idx, obj in enumerate(objs):
        if isinstance(obj, torch.Tensor):
            ...
            tensor_metadata.append(
                TensorMetadata(obj.shape, obj.dtype, obj.requires_grad)
            )
        else:
...
def _create_recv_buffer(
    tensor_metadata: list[TensorMetadata], current_device
) -> list[torch.Tensor]:
    buffer_recv = []
...
def _batch_send_recv_tensor(
    send_tensor_list: list[torch.Tensor] | None,
    recv_tensor_metadata: list[TensorMetadata] | None,
    send_dst: int | None,
    recv_src: int | None,
    send_group: ProcessGroup | None,
    recv_group: ProcessGroup | None,
    current_device: Any,
...
def _send_recv_serialization_object(
    ...
    if object is not None and send_dst is not None:
        ...
        _filling_ops_queue(
            send_object_size_tensor, dist.isend, send_dst, ops, send_group
        )
    ...
    if recv_src is not None:
        ...
        _filling_ops_queue(
            recv_object_size_tensor, dist.irecv, recv_src, ops, recv_group
        )
    ...
    if recv_src is not None and recv_object_size_tensor is not None:
        recv_object_tensor = torch.empty(
            recv_object_size_tensor.item(), dtype=torch.uint8
        )
        if is_nccl_backend:
...
def _communicate(
    ...
    send_dst: int | None,
    recv_src: int | None,
    send_group: ProcessGroup | None = None,
    recv_group: ProcessGroup | None = None,
    send_metadata: bool = True,
    metadata_recv: P2PMetadata | None = None,
    ...
    """
    assert (
        send_dst is not None or recv_src is not None
    ), "send_dst and recv_src cannot be both None"
    assert (
        send_dst is None or send_group is not None
    ), "send_group must be specified when send_dst is not None"
    assert (
        recv_src is None or recv_group is not None
    ), "recv_group must be specified when recv_src is not None"
    assert (
    ...
    if object is not None:
        ...
        metadata_send, tensor_objs = create_send_metadata(
    ...
    if (send_dst is not None and recv_src is not None) and (
        send_metadata or metadata_recv is None
    ):
        assert (
            send_prior_fallback is not None
        ), "Priority must be set if fallback happens"
    ...
    assert not (send_dst is not None and recv_src is not None) or (
        not send_metadata and metadata_recv is not None
    )
    assert not c10d._rank_not_in_group(send_group) and not c10d._rank_not_in_group(
        recv_group
    )
    ...
    if (send_dst is not None and send_metadata) or (
        recv_src is not None and metadata_recv is None
    ):
    ...
    recv_tensor_metadata = (
        None if metadata_recv is None else metadata_recv.tensor_metadata
    )
    ...
    if metadata_recv is not None:
        ...
        return tree_unflatten(recv_tensor_objs, tree_spec)
    return None


def _send_object(
    object: Any, src: int, dst: int, group: ProcessGroup, **kwargs
) -> None:
...
def _p2p_comm(
    ...
    if recv_prev:
        tensor_recv_prev = torch.empty(
            recv_prev_shape, device=torch.cuda.current_device(), dtype=comm_dtype
        )
...
class PipelineP2PCommunication:
    ...
    def recv_forward(
        self, prev_rank: int | None = None, metadata_recv: P2PMetadata | None = None
    ) -> Any:
    ...
    def recv_backward(
        self, next_rank: int | None = None, metadata_recv: P2PMetadata | None = None
    ) -> Any:

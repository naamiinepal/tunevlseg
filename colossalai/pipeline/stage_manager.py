...
class PipelineStageManager:
    ...
    def __init__(
        ...
        assert (
            enable_interleave or num_model_chunks == 1
        ), "num_model_chunks must be 1 when enable_interleave is False"

        self.pg_mesh = pg_mesh
        self.pipeline_axis = pipeline_axis
        self.prev_rank: tuple[int, ...] | None = None
        self.next_rank: tuple[int, ...] | None = None
        ...
        prev_coord = (
            coord[: self.pipeline_axis]
            + (coord[self.pipeline_axis] - 1,)
            + coord[self.pipeline_axis + 1 :]
        )
        ...
        for prev, cur in zip(stages[:-1], stages[1:], strict=False):
        ...
        if enable_interleave:
            ...
            group = self.pg_mesh.get_group_along_axis(
                self.pipeline_axis, [stages[0], stages[-1]]
            )
            ...
            self.model_chunk_id: int | None = None
    ...
    def get_stage_index(
        ...
        num_model_chunks = (
            self.num_model_chunks if num_model_chunks is None else num_model_chunks
        )
        ...
        for model_chunk in range(num_model_chunks):
            start_idx = num_layers_per_stage_accumulated[
                stage + model_chunk * num_stages
            ]
            end_idx = num_layers_per_stage_accumulated[
                stage + model_chunk * num_stages + 1
            ]
    ...
    def is_first_stage(self, ignore_chunk: bool = False) -> bool:
        ...
        assert not self.is_interleave or (
            ignore_chunk or self.model_chunk_id is not None
        )
    ...
    def is_last_stage(self, ignore_chunk: bool = False) -> bool:
        ...
        return (
            ...
            and self.model_chunk_id == self.num_model_chunks - 1
        )

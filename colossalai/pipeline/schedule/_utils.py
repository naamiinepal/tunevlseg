...
def to_device(x: Any, device: torch.device | None = None) -> Any:
...
def model_forward(model: Module, data: Any, internal_inputs: dict | None) -> Any:
...
def merge_batch(data: list[Any], batch_size_dim=0) -> Any:
    ...
    for elem_batch in zip(*flattened_data, strict=False):

...
class InterleavedSchedule(PipelineSchedule):
    ...
    def load_batch(
        self, data_iter: Iterable, device: torch.device | None = None
    ) -> None:
    ...
    def load_micro_batch(self, model_chunk_id: int) -> Any:
        ...
        """
        assert (
            self.microbatch_offset[model_chunk_id] <= self.batch_size
        ), "Microbatches exhausted"
        micro_batch = get_micro_batch(
            self.batch, self.microbatch_offset[model_chunk_id], self.microbatch_size
        )
    ...
    def get_model_chunk_id(self, microbatch_id: int, is_forward: bool) -> int:
        ...
        microbatch_id_in_group = microbatch_id % (
            self.stage_manager.num_stages * self.num_model_chunks
        )
    ...
    def recv_forward(self, model_chunk_id: int, prev_rank: int | None = None) -> Any:
    ...
    def recv_backward(self, model_chunk_id: int, next_rank: int | None = None) -> Any:
        ...
        with self.stage_manager.switch_model_chunk_id(model_chunk_id):
            if not self.stage_manager.is_last_stage():
                output_tensor_grad = self.comm.recv_backward(
    ...
    def send_forward(
        self, model_chunk_id: int, output_tensor: Any, next_rank: int | None = None
    ) -> None:
    ...
    def send_backward(
        self, model_chunk_id: int, input_tensor_grad: Any, prev_rank: int | None = None
    ) -> None:
    ...
    def send_forward_recv_backward(
        ...
        if send_data and recv_data:
            if (
                not self.send_forward_recv_backward
    ...
    def send_backward_recv_forward(
        ...
        if send_data and recv_data:
            if (
                not self.send_backward_recv_backward
    ...
    def forward_step(
        ...
        with self.stage_manager.switch_model_chunk_id(model_chunk_id):
            if isinstance(model_chunk, ModuleList):
                output_obj = model_forward(
                    model_chunk[model_chunk_id], micro_batch, input_obj
                )
            else:
                ...
                internal_inputs["stage_index"] = self.stage_manager.stage_indices[
                    model_chunk_id
                ]
    ...
    def backward_step(
        ...
        input_obj: dict | None,
        output_obj: dict | torch.Tensor,
        output_obj_grad: dict | None,
    ) -> dict | None:
    ...
    def run_forward_only(
        ...
        outputs = (
            []
            if return_outputs and self.stage_manager.is_last_stage(ignore_chunk=True)
            else None
        )
        ...
        for i in range(self.num_microbatch * self.num_model_chunks):
            ...
            output_obj = self.forward_step(
                model_chunk, model_chunk_id, input_obj, criterion, accum_loss, outputs
            )
    ...
    def run_forward_backward(
        ...
        for i in range(num_microbatch_remaining):
            ...
            model_chunk_id = self.get_model_chunk_id(
                i + num_warmup_microbatch, is_forward=True
            )
            output_obj = self.forward_step(
                model_chunk, model_chunk_id, input_obj, criterion, accum_loss, outputs
            )
            ...
            input_obj_grad = self.backward_step(
                optimizer, _input_obj, _output_obj, output_obj_grad
            )
            ...
            def send_forward_recv_backward():
                if last_iteration and num_microbatch == num_microbatch_remaining:
                    ...
                    self.send_forward(model_chunk_id, output_obj)
                    return None
                return self.send_forward_recv_backward(
                    model_chunk_id_send=self.get_model_chunk_id(
                        i + num_warmup_microbatch, is_forward=True
                    ),
                    model_chunk_id_recv=self.get_model_chunk_id(
                        i + 1, is_forward=False
                    ),
                    output_tensor=output_obj,
            ...
            def send_backward_recv_forward():
                if last_iteration:
                    ...
                    return None
                return self.send_backward_recv_forward(
                    model_chunk_id_send=self.get_model_chunk_id(i, is_forward=False),
                    model_chunk_id_recv=self.get_model_chunk_id(
                        i + num_warmup_microbatch + 1, is_forward=True
                    ),
        ...
        for i in range(num_microbatch_remaining, num_microbatch):
            ...
            if not last_iteration:
                output_obj_grad = self.send_backward_recv_backward(
                    ...
                    input_tensor_grad=input_obj_grad,
                    send_prior=self.stage_manager.stage % 2 == 0
                    and i > num_microbatch_remaining,
                )
            else:
                model_chunk_id = self.get_model_chunk_id(i, is_forward=False)
                self.send_backward(model_chunk_id, input_obj_grad)
    ...
    def forward_backward_step(
        ...
        if self.forward_only:
            result = self.run_forward_only(
                model_chunk, data_iter, criterion, return_loss, return_outputs
            )
        else:

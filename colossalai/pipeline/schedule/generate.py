...
class GenerateSchedule(PipelineSchedule):
    ...
    def __init__(
        ...
        self.microbatch_offset: int | None = None
        self.num_microbatches: int | None = None
    ...
    def _prepare_inputs_for_interval_stage(self):
        ...
        """
        return {"infer_state": self.mb_manager.cur_description.infer_state}
    ...
    def _prepare_inputs_for_new_token(self, new_token: torch.Tensor):
        ...
        return {"input_ids": new_token, "attention_mask": new_mask}

    def _get_token_id(self, hidden_state: torch.Tensor) -> torch.Tensor:
        last_hidden_state = hidden_state[:, -1]
        return torch.argmax(last_hidden_state, dim=-1).unsqueeze(1)
    ...
    def _head_encoding_action(self, model: Module):
        ...
        ), "When first stage in GENERATE phase, the new token should not be None"
    ...
    def _body_encoding_action(self, model: Module):
        ...
        assert (
            hidden_states is not None
        ), "When not first stage, the hidden states should not be None"
        interval_inputs = {
            "hidden_states": hidden_states,
            "infer_state": self.mb_manager.cur_infer_state,
        }
    ...
    def _comm_action(self, recv_pre: bool) -> torch.Tensor:
        ...
        """
        hidden_states = self.action_interval_buffer.hidden_states
        ret = self.comm.p2p_communicate(
            hidden_states, recv_pre, comm_dtype=self.comm_dtype
        )
    ...
    def _gen_action(self, model: Module):
        ...
        if self.stage_manager.is_first_stage():
            if self.mb_manager.cur_state is Status.PREFILL:
                actions.extend(
                    (
                        partial(self._comm_action, False),
                        partial(self._load_stage_action, model),
                    )
                )
            elif (
                self.stage_manager.is_first_stage()
                and self.mb_manager.cur_state is Status.GENERATE
            ):
                actions.extend(
                    (
                        partial(self._comm_action, True),
                        partial(self._gen_token_action, model),
                        partial(self._head_encoding_action, model),
                    )
                )
            elif (
                self.stage_manager.is_first_stage()
                and self.mb_manager.cur_state is Status.COOLDOWN
            ):
                actions.extend(
                    (
                        partial(self._comm_action, True),
        ...
        # other stage
            ...
            actions.extend(
                (
                    partial(self._comm_action, True),
                    partial(self._body_encoding_action, model),
                )
            )
    ...
    def generate_step(self, model: Module, data_iter: Iterable) -> torch.Tensor | dict:
        if self.stage_manager.num_stages == 1:
    ...
    @torch.no_grad()
    def generate_step_one_stage(
        self, model: Module, data_iter: Iterable
    ) -> torch.Tensor | dict:
        """
    ...
    def generate_step_broadcast(
        ...
        for _ in range(self.round):
            ...
            while self.mb_manager.is_micro_batch_done() is False:
                ...
                if (
                    self.stage_manager.is_first_stage()
                    and self.mb_manager.cur_state is Status.PREFILL
                ):
                    inputs_dict = self.load_micro_batch()
                    if self.verbose and self.stage_manager.is_first_stage():
                        torch.cuda.synchronize()
                ...
                # In GENERATE phase
                    ...
                    if self.stage_manager.is_first_stage():
                        ...
                        if self.mb_manager.cur_state in (
                            Status.GENERATE,
                            Status.COOLDOWN,
                        ):
                            inputs_dict = self._prepare_inputs_for_new_token(new_token)
                            ...
                            output_dict = model_forward(
                                model, inputs_dict, interval_inputs
                            )
                    else:
                        assert (
                            hidden_states is not None
                ...
                if (
                    not self.stage_manager.is_first_stage()
                    or self.mb_manager.cur_state
                    in (
                        Status.GENERATE,
                        Status.COOLDOWN,
                    )

...
class ParallelContext(metaclass=SingletonMeta):
    ...
    def load_config(self, config: dict | str):
    ...
    def init_parallel_groups(self):
        ...
        pg_init.append({"type": INITIALIZER_MAPPING["data"]})

        # LSG: init model parallel process group for compatibility with amp and clip grad
        pg_init.append({"type": INITIALIZER_MAPPING["model"]})

        if self.pipeline_parallel_size > 1:
            pg_init.append({"type": INITIALIZER_MAPPING["pipeline"]})
        pg_init.append({"type": INITIALIZER_MAPPING["tensor"]})
    ...
    def set_device(self, device_ordinal: int | None = None):

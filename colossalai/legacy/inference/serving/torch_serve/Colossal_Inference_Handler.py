...
class ColossalInferenceHandler(BaseHandler, ABC):
    ...
    def initialize(self, ctx):
        ...
        if ctx is not None or not hasattr(ctx, "model_yaml_config"):
            logger.error(
                "Context ctx and model-config are not appropriately passed in."
            )
        ...
        logger.info(
            f"Loading {self.inference_config['model_type']} pretrain model and tokenizer"
        )
        if self.inference_config["model_type"] == "bloom":
            ...
            self.tokenizer = BloomTokenizerFast.from_pretrained(
                model_dir_path, return_tensors="pt"
            )
            ...
            logger.warning(
                f"Model type {self.inference_config['model_type']} not supported yet."
            )
        ...
        assert (
            world_size == 1
        ), "Colossal-Inference with tensor parallel is not supported on TorchServe for now"

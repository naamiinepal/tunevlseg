...
from colossalai.inference.dynamic_batching.ray_init_config import (
    EngineArgsClass,
    RooterArgsClass,
)
...
class Worker:
    ...
    def setup(self, world_size, rank, port):
        ...
        ray_serve_logger.info(
            f"Worker with rank {rank} (world size {world_size}) setting up.."
        )
        ...
        shard_config = ShardConfig(
            enable_tensor_parallelism=world_size > 1,
        ...
        self.start_dynamic_batching = start_dynamic_batching(
            self.router_config, self.infer_engine, []
        )
    ...
    def add_req(
        ...
        self.start_dynamic_batching.add_req(
            prompt_ids, sampling_params, request_id, prompt
        )
...
class Driver:
    def __init__(self, router_config: RooterArgsClass, engine_config: EngineArgsClass):
        ...
        _options = {
            ...
            "ranks": list(range(self.num_workers)),
            "backend": "nccl",
        }
    ...
    def add_input(self, request_id: str, prompt: str, sampling_params: SamplingParams):
        ray.get(
            [
                w.add_input.remote(request_id, prompt, sampling_params)
    ...
    def step(self):
        ...
        return results[0]  # get any one of the copies
    ...
    def add_req(
        ...
        ray.get(
            [
                w.add_req.remote(prompt_ids, sampling_params, request_id, prompt)
                for w in self.workers
            ]
        )

...
class InferBatch:
    ...
    def init_batch(
        ...
        nopad_b_loc = torch.empty(
            (len(requests), max_total_len + 12), dtype=torch.long, device="cuda"
        )
    ...
    def filter(self, request_ids: list[int]) -> "InferBatch":
        ...
        nopad_max_len_in_batch = 0
        nopad_b_loc = torch.empty(
            (len(request_ids), self.max_total_len + 12), dtype=torch.long, device="cuda"
        )
        ...
        nopad_b_loc[:, 0 : (nopad_max_len_in_batch - 1)] = self.nopad_b_loc[
            indices,
            (self.nopad_max_len_in_batch - 1) - (nopad_max_len_in_batch - 1) : (
                self.nopad_max_len_in_batch - 1
            ),
        ]
        for i, request_id in enumerate(request_ids):
    ...
    def merge(cls, batch1, batch2) -> "InferBatch":
        ...
        nopad_b_loc = torch.empty(
            (new_batch_size, batch1.max_total_len + 12), dtype=torch.long, device="cuda"
        )
        nopad_b_start_loc = torch.zeros(
        ...
        for i, batch in enumerate(batches):
            ...
            nopad_b_loc[
                ...
                - batch.nopad_max_len_in_batch : nopad_max_len_in_batch - 1,

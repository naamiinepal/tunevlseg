...
class TPInferEngine:
    ...
    def __init__(
        ...
        self.max_total_token_num = self.max_batch_size * (
        ...
        assert (
            self.max_input_len + self.max_output_len <= 4096
        ), "Max length exceeds the constraint"
    ...
    def _init_manager(self) -> None:
        ...
        assert (
            self.head_num % self.tp_size == 0
        ), f"Cannot shard {self.head_num} heads with tp size {self.tp_size}"
    ...
    def _post_init_gptq_buffer(self, model: nn.Module) -> None:
        ...
        for _name, submodule in model.named_modules():
            if isinstance(submodule, CaiQuantLinear):
                self.max_dq_buffer_size = max(
                    self.max_dq_buffer_size, submodule.qweight.numel() * 8
                )

                if self.use_act_order:
                    self.max_inner_outer_dim = max(
                        self.max_inner_outer_dim,
        ...
        gptq_cuda.set_tuning_params(
            matmul_recons_thd, matmul_fused_remap, matmul_no_half2
        )
    ...
    def _prepare_with_shard_config(
        self, shard_config: ShardConfig | None = None
    ) -> ShardConfig:
    ...
    def _shard_model_by(self, shardformer: ShardFormer, model: nn.Module) -> None:
        ...
        ), f"Unsupported model cls {model_name} for TP inference."
    ...
    def generate(
        self,
        input_tokens: BatchEncoding | dict | list | torch.Tensor,
        **generate_kwargs,
    ) -> torch.Tensor:
    ...
    def prepare_batch_state(self, inputs) -> BatchInferState:
        ...
        if not isinstance(inputs, BatchEncoding | dict | list | torch.Tensor):
            raise TypeError(
                f"inputs type {type(inputs)} is not supported in prepare_batch_state"
            )
        ...
        if isinstance(input_ids_list[0], int):  # for a single input
            ...
            attention_mask = (
                [attention_mask] if attention_mask is not None else attention_mask
            )
        ...
        if isinstance(inputs, BatchEncoding | dict):
            for i, attn_mask in enumerate(attention_mask):
                ...
                max_len_in_batch = max(max_len_in_batch, curr_seq_len)
        else:
            length = max(len(input_id) for input_id in input_ids_list)
            for i, _input_ids in enumerate(input_ids_list):
                curr_seq_len = length
                seq_lengths[i] = curr_seq_len
                seq_start_indexes[i] = start_index
                start_index += curr_seq_len
        ...
        block_loc = torch.empty(
            (batch_size, self.max_input_len + self.max_output_len),
            dtype=torch.long,
            device="cuda",
        )
    ...
    @torch.no_grad()
    def _generate_by_set_infer_state(
        self, input_tokens, **generate_kwargs
    ) -> torch.Tensor:
        """
        ...
        assert (
            batch_infer_state.max_len_in_batch <= self.max_input_len
        ), "max length in batch exceeds limit"
        ...
        model.infer_state = batch_infer_state

        outputs = self.model.generate(
            **input_tokens, **generate_kwargs, early_stopping=False
        )
    ...
    def _generate_by_pass_infer_state(
        ...
        prepare_inputs_fn: Callable[[torch.Tensor, Any], dict] | None = None,
        **model_kwargs,
    ) -> torch.Tensor:
        raise NotImplementedError(
            "generate by passing BatchInferState is not implemented."
        )
    ...
    def _update_batch_state(self, infer_state: BatchInferState | None) -> None:
    ...
    def forward(self, batch_id, is_prefill):
        ...
        for i, (r, all_input_ids, next_token_id, next_token_logprob) in enumerate(
            zip(
                batch.requests, batch.all_input_ids, predict_ids, prob_out, strict=False
            )
        ):

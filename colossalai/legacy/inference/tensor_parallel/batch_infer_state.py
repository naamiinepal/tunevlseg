...
class BatchInferState:
    ...
    def init_block_loc(
        ...
        for i, cur_seq_len in enumerate(seq_len_numpy):
            b_loc[i, max_len_in_batch - cur_seq_len : max_len_in_batch] = (
                alloc_mem_index[start_index : start_index + cur_seq_len]
            )
    ...
    def init_from_batch(
        ...
        if not isinstance(batch, BatchEncoding | dict | list | torch.Tensor):
            raise TypeError(
                f"batch type {type(batch)} is not supported in prepare_batch_state"
            )
        ...
        if isinstance(batch, BatchEncoding | dict):
            ...
            for i, _input_ids in enumerate(input_ids_list):
                ...
                start_index += curr_seq_len
                max_len_in_batch = max(max_len_in_batch, curr_seq_len)
        block_loc = torch.zeros(
            (batch_size, max_input_len + max_output_len),
            dtype=torch.long,
            device="cuda",
        )

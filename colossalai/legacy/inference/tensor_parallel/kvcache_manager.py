...
class MemoryManager:
    ...
    def _init_kv_buffers(self, size, device, dtype, head_num, head_dim, layer_num):
        ...
        self.key_buffer = [
            torch.empty((size, head_num, head_dim), dtype=dtype, device=device)
    ...
    def alloc(self, required_size):
        ...
        if required_size > self.available_size:
            self.logger.warning(
                f"No enough cache: required_size {required_size} "
        ...
        select_index = torch.logical_and(
            self.mem_cum_sum <= required_size, self.mem_state == 1
        )
    ...
    def alloc_contiguous(self, required_size):
        ...
        can_used_loc = self.indexes[0 : sum_size - required_size + 1][
            loc_sums == required_size
        ]
        if can_used_loc.shape[0] == 0:
            self.logger.info(
                f"No enough contiguous cache: required_size {required_size} "
                f"left_size {self.available_size}"
            )
            return None

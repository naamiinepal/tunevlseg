...
try:
    ...
    from lightllm.models.llama.triton_kernel.rotary_emb import (
        rotary_emb_fwd as llama_rotary_embedding_fwd,
    )
...
try:
    ...
    print(
        "please install flash attentiom from https://github.com/Dao-AILab/flash-attention"
    )
...
def llama_triton_context_attention(
    ...
    if num_key_value_groups == 1:
        ...
        assert (
            HAS_LIGHTLLM_KERNEL is True
        ), "You have to install lightllm kernels to run llama2 model"
...
def llama_triton_token_attention(
    query_states, attn_output, infer_state, num_key_value_groups=1
):
    assert (
        HAS_LIGHTLLM_KERNEL is True
    ), "You have to install lightllm kernel to run token attention for llama models"
    if num_key_value_groups == 1:
        token_attention_fwd(
            query_states,
...
class LlamaInferenceForwards:
    ...
    def llama_causal_lm_forward(
        ...
        stage_manager: PipelineStageManager | None = None,
        ...
        return LlamaInferenceForwards.llama_model_forward(
            self.model,
            input_ids=input_ids,
    ...
    def llama_model_forward(
        ...
        if stage_manager is None or stage_manager.is_first_stage():
            ...
            ), f"hidden_state should not be none in stage {stage_manager.stage}"
        ...
        if use_cache and seq_length != 1:
            ...
            infer_state.is_context_stage = (
                True  # set prefill stage, notify attention layer
            )
            ...
            if alloc_mem is not None:
                ...
                infer_state.block_loc[:, infer_state.max_len_in_batch - 1] = (
        ...
        if infer_state.is_context_stage:
            ...
            infer_state.position_cos = torch.index_select(
                self._cos_cached, 0, seq_len - 1
            ).view(seq_len.shape[0], -1)
            infer_state.position_sin = torch.index_select(
                self._sin_cached, 0, seq_len - 1
            ).view(seq_len.shape[0], -1)
            infer_state.other_kv_index = infer_state.block_loc[
                0, infer_state.max_len_in_batch - 1
            ].item()

        # embed positions
        if attention_mask is None:
            attention_mask = torch.ones(
                (batch_size, infer_state.max_len_in_batch),
                dtype=torch.bool,
                device=hidden_states.device,
            )
        ...
        for idx, past_key_value in zip(
            range(start_idx, end_idx), past_key_values, strict=False
        ):
        ...
        infer_state.start_loc += torch.arange(
    ...
    def llama_flash_attn_kvcache_forward(
        ...
        assert (
            use_cache is True
        ), "use_cache should be set to True using this llama attention"
        ...
        llama_rotary_embedding_fwd(
            query_states.view(-1, self.num_heads, self.head_dim), cos, sin
        )
        llama_rotary_embedding_fwd(
            key_states.view(-1, self.num_key_value_heads, self.head_dim), cos, sin
        )
        ...
        if infer_state.is_context_stage:
            ...
            if HAS_LIGHTLLM_KERNEL:
                ...
                copy_cache_k = cache_k.view(
                    bsz, -1, self.num_key_value_heads, self.head_dim
                )
                copy_cache_v = cache_v.view(
                    bsz, -1, self.num_key_value_heads, self.head_dim
                )

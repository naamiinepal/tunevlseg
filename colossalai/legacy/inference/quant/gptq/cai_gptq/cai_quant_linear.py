...
class CaiQuantLinear(nn.Module):
    def __init__(
        ...
        if row_split:
            self.register_buffer(
                ...
                torch.tensor(
                    [
                        (i + (tp_rank * self.infeatures)) // self.groupsize
    ...
    def pack(self, linear, scales, zeros, g_idx=None):
        ...
        for idx in range(self.infeatures):
            intweight.append(
                torch.round(
                    (linear.weight.data[:, idx] + scale_zeros[g_idx[idx]])
                    / half_scales[g_idx[idx]]
                ).to(ptype)[:, None]
            )
        ...
        qweight = np.zeros(
            (intweight.shape[0] // pbits * self.bits, intweight.shape[1]),
            dtype=unsign_type,
        )
        ...
        qzeros = np.zeros(
            (zeros.shape[0], zeros.shape[1] // pbits * self.bits), dtype=unsign_type
        )
        zeros -= 1
    ...
    def init_q4(self):
        ...
        if self.g_idx is not None:
            if (
                ...
                and torch.equal(
                    ...
                    torch.tensor(
                        [
                            (i + (self.tp_rank * self.infeatures)) // self.groupsize
                ...
                self.g_idx,
                torch.tensor(
                    [i // self.groupsize for i in range(self.infeatures)],
                    dtype=torch.int32,
                    device=self.g_idx.device,
                ),
            ):
        ...
        g_idx = self.g_idx.to("cpu") if self.g_idx is not None else self.empty_tensor

        self.q4 = gptq_cuda.make_q4(
            self.qweight, self.qzeros, self.scales, g_idx, torch.cuda.current_device()
        )
    ...
    def forward(self, x):
        ...
        if HAS_GPTQ_CUDA and self.bits == 4:
            ...
            output = torch.empty(
                (x.shape[0], self.outfeatures), dtype=torch.float16, device=x.device
            )
...
def split_column_copy(gptq_linear, cai_linear, tp_size=1, tp_rank=0, split_num=1):
    ...
    qzeros = gptq_linear.qzeros.split(
        gptq_linear.out_features // (32 // cai_linear.bits) // split_num, dim=-1
    )
    ...
    for i in range(split_num):
        cai_linear.qweight[
            :, i * cai_split_out_features : (i + 1) * cai_split_out_features
        ] = qweights[i][
            :, tp_rank * cai_split_out_features : (tp_rank + 1) * cai_split_out_features
        ]
        cai_linear.qzeros[:, i * zero_split_block : (i + 1) * zero_split_block] = (
            qzeros[i][:, tp_rank * zero_split_block : (tp_rank + 1) * zero_split_block]
        )
        cai_linear.scales[
            :, i * cai_split_out_features : (i + 1) * cai_split_out_features
        ] = scales[i][
        ...
        if cai_linear.bias is not None:
            cai_linear.bias[
                ...
                tp_rank * cai_split_out_features : (tp_rank + 1)
...
def split_row_copy(gptq_linear, cai_linear, tp_rank=0, split_num=1):
    ...
    cai_split_in_features = (
        cai_linear.infeatures // (32 // cai_linear.bits) // split_num
    )
    ...
    for i in range(split_num):
        cai_linear.qweight[
            i * cai_split_in_features : (i + 1) * cai_split_in_features, :
        ] = qweights[i][
        ...
        cai_linear.scales[i * zero_split_block : (i + 1) * zero_split_block, :] = (
            scales[i][tp_rank * zero_split_block : (tp_rank + 1) * zero_split_block, :]
        )
        cai_linear.g_idx[i * idx_split_features : (i + 1) * idx_split_features] = (
            g_idxs[i][tp_rank * idx_split_features : (tp_rank + 1) * idx_split_features]
        )
    if cai_linear.bias is not None:

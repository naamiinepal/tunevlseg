...
class LLamaSmoothquantAttention(nn.Module):
    ...
    def pack(
        ...
        int8_module.q_proj = W8A8B8O8Linear.from_float(
            module.q_proj, attn_input_scale, q_output_scale
        )
        int8_module.k_proj = W8A8B8O8Linear.from_float(
            module.k_proj, attn_input_scale, k_output_scale
        )
        int8_module.v_proj = W8A8B8O8Linear.from_float(
            module.v_proj, attn_input_scale, v_output_scale
        )
    ...
    def forward(
        ...
        infer_state: BatchInferState | None = None,
        ...
        def _copy_kv_to_mem_cache(
            layer_id, key_buffer, value_buffer, context_mem_index, mem_manager
        ):
            copy_kv_cache_to_dest(
                key_buffer, context_mem_index, mem_manager.key_buffer[layer_id]
            )
            copy_kv_cache_to_dest(
                value_buffer, context_mem_index, mem_manager.value_buffer[layer_id]
            )
            return
...
class LlamaSmoothquantMLP(nn.Module):
    ...
    def pack(
        ...
        int8_module.gate_proj = W8A8BFP32O32LinearSiLU.from_float(
            mlp_module.gate_proj, gate_proj_input_scale
        )
        int8_module.up_proj = W8A8BFP32OFP32Linear.from_float(
            mlp_module.up_proj, up_proj_input_scale
        )
        int8_module.down_proj = W8A8BFP32OFP32Linear.from_float(
            mlp_module.down_proj, down_proj_input_scale
        )
    ...
    def forward(
        ...
        inter_out = (
            inter_out.div_(self.down_proj_input_scale.item())
            .round()
            .clamp(-128, 127)
            .to(torch.int8)
        )
        down_out = self.down_proj(inter_out)
        return down_out.view(*x_shape[:-1], -1)
...
class LlamaSmoothquantDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.self_attn = LLamaSmoothquantAttention(
            config.hidden_size, config.num_attention_heads
        )

        self.mlp = LlamaSmoothquantMLP(config.intermediate_size, config.hidden_size)
        self.input_layernorm = LlamaLayerNormQ(
            config.hidden_size, eps=config.rms_norm_eps
        )

        self.post_attention_layernorm = LlamaLayerNormQ(
            config.hidden_size, eps=config.rms_norm_eps
        )
    ...
    def pack(
        ...
        int8_decoder_layer.input_layernorm = LlamaLayerNormQ.from_float(
    ...
    def forward(
        ...
        # Self Attention
        hidden_states, _self_attn_weights, _present_key_value = self.self_attn(
...
def init_to_get_rotary(config, base=10000, use_elem=False):
    ...
    try:
        ...
        base = base * (
            ntk_alpha ** (config.head_dim_ / (config.head_dim_ - 2))
        )  # Base change formula
    except:
        pass
    ...
    inv_freq = 1.0 / (
        base ** (torch.arange(0, n_elem, 2, device="cpu", dtype=torch.float32) / n_elem)
    )
    t = (
...
def llama_model_forward(
    ...
    #       block_loc require different value-assigning method for two different stage
    if infer_state.is_context_stage:
        infer_state.context_mem_index = infer_state.cache_manager.alloc(
        ...
        if alloc_mem is not None:
            ...
            infer_state.block_loc[:, seq_length_with_past - 1] = (
                infer_state.decode_mem_index
            )
        else:
            print(" *** Encountered allocation non-contiguous")
            print(
                f"    infer_state.cache_manager.max_len_in_batch: {infer_state.max_len_in_batch}"
            )
    ...
    if self.gradient_checkpointing and self.training:
        raise NotImplementedError(
            "not implement gradient_checkpointing and training options "
        )

    if past_key_values_length == 0:
        position_cos = torch.index_select(
            self._cos_cached, 0, position_ids.view(-1)
        ).view(position_ids.view(-1).shape[0], -1)
        position_sin = torch.index_select(
            self._sin_cached, 0, position_ids.view(-1)
        ).view(position_ids.view(-1).shape[0], -1)
    else:
        position_cos = torch.index_select(
            self._cos_cached, 0, position_ids.view(-1)
    ...
    infer_state.start_loc = infer_state.start_loc + torch.arange(
...
class SmoothLlamaForCausalLM(BaseSmoothForCausalLM):
    ...
    def get_act_dict(
        ...
        for name, m in llama_model.named_modules():
            if isinstance(m, LlamaAttention):
                m.q_apply_rotary = LlamaApplyRotary()
                m.k_apply_rotary = LlamaApplyRotary()
        ...
        self.collect_act_dict(
            llama_model, tokenizer, dataset, act_dict, device, num_samples, seq_len
        )
    ...
    def quantized(
        ...
        act_scales = self.get_act_scales(
            llama_model, tokenizer, dataset, num_samples, seq_len
        )
        ...
        for i, layer in enumerate(llama_model.model.layers):
            orig_layer = layer
            llama_model.model.layers[i] = LlamaSmoothquantDecoderLayer.pack(
                orig_layer, **decoder_layer_scales[i]
            )

        llama_model.model.forward = types.MethodType(
            llama_model_forward, llama_model.model
        )

...
def print_details_info(timestamps, model_config, args, whole_end2end):
    if dist.get_rank() == 0:
        ...
        with open(
            ...
            num_bytes = 2 if args.dtype in ["fp16", "bf16"] else 4
            ...
            f.write(
                f"Average prefill time: {sum(prefill) / len(prefill) * 1000:8.2f} ms\n"
            )
            f.write(
                f"Average encode time: {sum(encoder) / len(encoder) * 1000:8.2f} ms\n"
            )
            f.write(
                f"Average micro batch end2end time: {mb_avg_end2end * 1000:8.2f} ms\n"
            )
            f.write(
                f"Average micro batch Per Token Latency: {mb_avg_latency * 1000:8.2f} ms\n"
            )
            f.write(f"Whole batch end2end time: {whole_end2end * 1000:8.2f} ms\n")
            f.write(
                f"Whole batch Per Token Latency: {whole_avg_latency * 1000:8.2f} ms\n"
            )
            f.write(f"Throughput: {1000 / (whole_avg_latency * 1000)} tokens/s\n")
            f.write(
                f"flops: {1 / whole_avg_latency * num_parameters * num_bytes / 1e12:8.2f} TFlops/s\n"
            )
            f.write("----------------------------------------------------------\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_path", type=str, default="./log", help="where to store the benchmark log"
    )
    ...
    if args.model == "toy":
        model = transformers.LlamaForCausalLM(
            transformers.LlamaConfig(num_hidden_layers=8)
        )
    elif args.model == "7b":
        model = transformers.LlamaForCausalLM(
            transformers.AutoConfig.from_pretrained("decapoda-research/llama-7b-hf")
        )
    elif args.model == "13b":
        model = transformers.LlamaForCausalLM(
            transformers.AutoConfig.from_pretrained("decapoda-research/llama-13b-hf")
        )
    else:

...
class DynamicBatchManager:
    def __init__(
        ...
        waiting_req_list: list | None = None,
    ):
        """
        ...
        """
        if waiting_req_list is None:
            waiting_req_list = []
        self.engine = tp_engine
        self.max_total_token_num = max_total_token_num
        running_max_req_size = (
            self.engine.max_batch_size if self.engine is not None else 2
        )
        ...
        self.tokenizer = (
            get_tokenizer(tokenizer_name=self.model) if tokenizer is None else tokenizer
        )
    ...
    def add_req(
        ...
        sampling_params.max_new_tokens = min(
            sampling_params.max_new_tokens, self.engine.max_output_len
        )
    ...
    def _init_batch(self, batch: Batch, dtype="fp16"):
        ...
        if dtype == "fp16":
            ...
            raise AssertionError("error dtype")
    ...
    def _prefill_batch(self, batch):
        ...
        has_new_finished_req = batch.mark_finished_req(
            self.eos_id, self.engine.max_output_len
        )

        ...
        for metric_name, metric_calculator in trainer.states["metrics"][
            ...
            if isinstance(metric_calculator, ThroughputMetric):
                trainer.states["step_metrics"][metric_name.lower()] = (
                    metric_calculator.get_last_step_info()
                )
            else:
                trainer.states["step_metrics"][metric_name.lower()] = (
                    metric_calculator.get_last_step_value()
                )
...
class LogMetricByEpochHook(LogByEpochHook):
    ...
    def __init__(self, logger, interval: int = 1, priority: int = 10) -> None:
        ...
        self._is_rank_to_log = (
            is_dp_rank_0() and is_tp_rank_0() and is_no_pp_or_last_stage()
        )
            ...
            if metric_calculator.epoch_only:
                ...
                if self._is_valid_rank_to_log:
                    self.writer.add_scalar(
                        f"{metric_name}/{mode}", val, trainer.cur_step
                    )
        ...
        for timer_name, timer in self._timer:
            if timer_name.startswith(mode):
                ...
                if timer.has_history:
                    if (
                        timer_name == "Train-step"
                        and not self._is_train_step_history_trimmed
                    ):

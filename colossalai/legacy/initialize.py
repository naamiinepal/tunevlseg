...
from colossalai.legacy.utils import (
    is_using_ddp,
    is_using_pp,
    is_using_sequence,
    sync_model_param,
...
def get_default_parser():
    ...
    parser.add_argument(
        "--host", type=str, help="the master address for distributed training"
    )
    parser.add_argument(
        "--port", type=int, help="the master port for distributed training"
    )
    parser.add_argument(
...
def initialize(
    ...
    torch_ddp_cfg = gpc.config.get("torch_ddp", {})
    ...
    if gradient_handler_cfg is None:
        ...
        if isinstance(optimizer, ShardedOptimizerV2):
            gradient_handler_cfg = [{"type": "ZeROGradientHandler"}]
            if verbose:
                logger.info(
            ...
            gradient_handler_cfg = [{"type": "DataParallelGradientHandler"}]
            if verbose:
                logger.info(
            ...
            if getattr(param, "pipeline_shared_module_pg", None) is not None:
                if gradient_handler_cfg is None:
                    gradient_handler_cfg = [
                        {"type": "PipelineSharedModuleGradientHandler"}
                    ]
                else:
    ...
    if is_using_pp():
        ...
        use_interleaved = hasattr(gpc.config, "model") and hasattr(
    ...
    if grad_accum_size is not None:
        optimizer, train_dataloader, gradient_handlers, lr_scheduler = (
            accumulate_gradient(
                model=model,
                optimizer=optimizer,

...
class GradScaler:
    ...
    _grows_tracker: torch.Tensor | None
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def _unscale_grads_(self, optimizer, inv_scale, found_inf, allow_fp16):
        ...
        if (
            ...
            dist.all_reduce(
                coalesced, op=dist.ReduceOp.MAX, group=gpc.get_group(ParallelMode.MODEL)
            )
            for buf, synced in zip(
                vals, _unflatten_dense_tensors(coalesced, vals), strict=False
            ):
                buf.copy_(synced)

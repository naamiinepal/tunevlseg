...
def pyobj_map(obj: Any, fn: Callable, process_types: type | tuple[type] = ()) -> Any:
    ...
    if type(obj) is list:
        return [pyobj_map(o, fn, process_types) for o in obj]
    return obj
...
def split_batch(batch: Any, start, stop, device: str):
    if device == "cuda":
        ...
        def fn(x):
            return x[start:stop].cuda()
    else:
        fn = operator.itemgetter(slice(start, stop))
    return pytree_map(batch, fn=fn, process_types=torch.Tensor)


def type_detail(obj):
    return pytree_map(obj, type, map_all=True)

...
def find_input_in_partition(node, partitions, input_partitions=None):
    ...
    if direct_def and input_partitions is not None:
        ...
        for i, input_node in enumerate(input_partitions):
            if input_node == node:
                return PartitionInputVal(partition_id=partition_id, offset=i)
    # search submod in mid part
    if direct_def:
        for partition in partitions:
            if partition == node:
                partition_id = partition_name_to_id(partition.name)
                return PartitionInputVal(partition_id=partition_id, offset=0)
    # search temporary value in graph
        ...
        for partition in partitions:
            for offset, mid_val in enumerate(partition.users):
                if mid_val == node:
                    partition_id = partition_name_to_id(partition.name)
                    return PartitionInputVal(partition_id=partition_id, offset=offset)
    ...
    for user in node.users:
        ...
        if output_partitions is not None:
            ...
            if user.op == output_node.op:
                ...
                torch.fx.graph.map_arg(output_node.args[0], output_keys.setdefault)
...
def get_topology(gm: GraphModule):
    ...
    for i, partition in enumerate(partitions):
        ...
        for arg in partition.args:
            cur_node = arg
            p_input_val = find_input_in_partition(
                cur_node, partitions, input_partitions
            )
        ...
        if direct_use:
            cur_node = partition
            p_output_val = find_output_in_partition(
                cur_node, partitions, output_partitions
            )
    ...
    for partition in output_partitions:
        ...
        torch.fx.graph.map_arg(
            partition.args[0],
            lambda n: topo_output_partition.add_input_val(

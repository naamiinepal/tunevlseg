    ...
    def backward(ctx, grad_outputs):
        return (
            ctx.backward_trans_func(
                grad_outputs, ctx.dist_spec, ctx.old_dist_spec, ctx.pg
            ),
            None,
            None,
            None,
...
class DistSpecManager:
    ...
    def _shard_as(
        ...
        ), "The old_dist_spec of DistSpecManager._shard_as must be REPLICATE!"
    ...
    @staticmethod
    def _gather(
        tensor: torch.Tensor, old_dist_spec: _DistSpec, pg: ProcessGroup
    ) -> torch.Tensor:
        ...
        ), "The old_dist_spec of DistSpecManager._gather must be SHARD!"
        is_cpu_tensor = False
    ...
    def handle_trans_spec(
        ...
        assert isinstance(
            old_dist_spec, _DistSpec
        ), f"{type(old_dist_spec)} should be _DistSpec"
        assert isinstance(
            dist_spec, _DistSpec
        ), f"{type(dist_spec)} should be _DistSpec"

        trans_func_key = (old_dist_spec.placement, dist_spec.placement)
        trans_funcs = {
            (
                DistPlacementPattern.REPLICATE,
                DistPlacementPattern.REPLICATE,
            ): DistSpecManager._r2r,
            (
                DistPlacementPattern.REPLICATE,
                DistPlacementPattern.SHARD,
            ): DistSpecManager._r2s,
            (
                DistPlacementPattern.SHARD,
                DistPlacementPattern.REPLICATE,
            ): DistSpecManager._s2r,
            (
                DistPlacementPattern.SHARD,
                DistPlacementPattern.SHARD,
            ): DistSpecManager._s2s,
        }

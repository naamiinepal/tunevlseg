...
class PyTorchProcessGroupDict(metaclass=SingletonMeta):
    ...
    def log_pg_init(self, rank_list: list[int], backend: str):
        ...
        str_list.extend((f"backend: {backend}", f"ranks: {rank_list}"))
    ...
    def get(self, rank_list: list[int], backend: str = "nccl"):
        ...
        if processgroup_key not in self.dict:
            ...
            self.dict[processgroup_key] = torch.distributed.new_group(
                ranks=rank_list, backend=backend
            )
...
class ProcessGroup:
    ...
    def __init__(
        ...
        for i in range(self._dp_degree):
            i_tp_list = [
                self._rank_list[i * self._tp_degree + j] for j in range(self._tp_degree)
            ]
        ...
        for j in range(self._tp_degree):
            j_dp_list = [
                self._rank_list[i * self._tp_degree + j] for i in range(self._dp_degree)
            ]
    ...
    def __eq__(self, obj: "ProcessGroup") -> bool:
        ...
        if self._tp_degree != obj._tp_degree:
            return False
        return not self._dp_degree != obj._dp_degree

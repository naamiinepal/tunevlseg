    ...
    def forward(self, logits, targets):
        ...
        if self.reduction_mean:
            ...
            loss = reduce_by_batch_3d(
                loss, self.input_parallel_mode, self.weight_parallel_mode, True
            )
        return loss
...
class _VocabParallelCrossEntropy3D(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, output_parallel_mode):
        ...
        arange_1d = torch.arange(
            start=0, end=logits.size()[0], device=get_accelerator().get_current_device()
        )
...
class VocabParallelCrossEntropyLoss3D(_Loss):
    ...
    def forward(self, logits, targets):
        ...
        loss = _VocabParallelCrossEntropy3D.apply(
            logits, targets, self.output_parallel_mode
        )
        if self.reduction_mean:
            loss = loss.mean()
            loss = reduce_by_batch_3d(

...
from colossalai.legacy.nn.layer.parallel_2p5d import (
    reduce_by_batch_2p5d,
    split_batch_2p5d,
)
from colossalai.legacy.nn.layer.parallel_2p5d._utils import (
...
class _VocabParallelCrossEntropy2p5D(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets):
        ...
        dist.all_reduce(
            predicted_logits, group=gpc.get_group(ParallelMode.PARALLEL_2P5D_ROW)
        )

        exp_logits = torch.exp(logits)
        sum_exp_logits = exp_logits.sum(dim=1)
        dist.all_reduce(
            sum_exp_logits, group=gpc.get_group(ParallelMode.PARALLEL_2P5D_ROW)
        )

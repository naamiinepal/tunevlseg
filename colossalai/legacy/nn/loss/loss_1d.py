...
class _VocabParallelCrossEntropy1D(torch.autograd.Function):
    ...
    def forward(ctx, vocab_parallel_logits, targets, process_group):
        ...
        torch.distributed.all_reduce(
            logits_max, op=torch.distributed.ReduceOp.MAX, group=process_group
        )
        ...
        torch.distributed.all_reduce(
            predicted_logits, op=torch.distributed.ReduceOp.SUM, group=process_group
        )
        ...
        torch.distributed.all_reduce(
            sum_exp_logits, op=torch.distributed.ReduceOp.SUM, group=process_group
        )

...
class VanillaPatchEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.empty(
                (embed_size, in_chans, *self.patch_size),
        ...
        self.bias = nn.Parameter(
            torch.empty(
                embed_size, device=get_accelerator().get_current_device(), dtype=dtype
            )
        )
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(
                (1, self.num_patches + 1, embed_size),
                device=get_accelerator().get_current_device(),
                dtype=dtype,
            )
        )
...
class VanillaLayerNorm(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return F.layer_norm(
            x, self.normalized_shape, self.weight, self.bias, self.variance_epsilon
        )

...
class LayerNorm2p5D(ParallelLayer):
    ...
    def __init__(
        self, normalized_shape: int, eps: float = 1e-05, bias=True, dtype=None
    ):
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        output = layernorm_2p5d(
            x, E_x, Var_x, self.normalized_shape, ParallelMode.PARALLEL_2P5D_ROW
        )
        scale = add_bias_2p5d(
            None,
            self.weight,
...
class PatchEmbedding2p5D(ParallelLayer):
    ...
    def __init__(
        ...
        with seed(ParallelMode.TENSOR):
            ...
            self.cls_token = Parameter(
                torch.zeros(
                    (1, 1, self.embed_size_per_partition),
                    device=get_accelerator().get_current_device(),
                    dtype=dtype,
                )
            )
    ...
    def _set_tensor_parallel_attribute(self):
        ...
        set_tensor_parallel_attribute_by_partition(
            self.cls_token, self.tesseract_dim**2
        )
        set_tensor_parallel_attribute_by_partition(
            self.pos_embed, self.tesseract_dim**2
        )
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        cls_token = all_gather_tensor_2p5d(
            self.cls_token, -1, ParallelMode.PARALLEL_2P5D_COL
        )
        pos_embed = all_gather_tensor_2p5d(
            self.pos_embed, -1, ParallelMode.PARALLEL_2P5D_COL
        )
...
class Embedding2p5D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        return F.embedding(
            input_, weight, self.padding_idx, *self.embed_args, **self.embed_kwargs
        )
...
class VocabParallelEmbedding2p5D(ParallelLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_embeddings_per_partition = divide(
            self.num_embeddings, self.tesseract_dim
        )
    ...
    def _fill_padding_idx_with_zero(self) -> None:
        if (
            ...
            and self.vocab_start_index <= self.padding_idx < self.vocab_end_index
        ):
            with torch.no_grad():
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        return reduce_scatter_tensor_2p5d(
            output_parallel, 0, ParallelMode.PARALLEL_2P5D_COL
        )
...
class Classifier2p5D(ParallelLayer):
    ...
    def _set_tensor_parallel_attributes(self):
        if self.has_weight:
            set_tensor_parallel_attribute_by_partition(
                self.weight, self.tesseract_dim**2
            )

...
from colossalai.legacy.communication.collective import (
    all_gather,
    all_reduce,
    reduce_scatter,
)
...
class Matmul_AB_2p5D(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            A.shape[-1] == B.shape[-2]
        ), f"Invalid shapes: A={A.shape}, B={B.shape} for AB."
        ...
        C = torch.zeros(
            C_shape, dtype=A.dtype, device=get_accelerator().get_current_device()
        )
...
class Matmul_ABT_2p5D(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            A.shape[-1] == B.shape[-1]
        ), f"Invalid shapes: A={A.shape}, B={B.shape} for ABT."
...
class Matmul_ATB_2p5D(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            A.shape[-2] == B.shape[-2]
        ), f"Invalid shapes: A={A.shape}, B={B.shape} for ATB."
...
class _Add_Bias_2p5D(torch.autograd.Function):
    ...
    def forward(
        ...
        dist.broadcast(
            bias_temp, src=src_rank, group=get_parallel_group(col_parallel_mode)
        )
    ...
    def backward(ctx: Any, output_grad: Tensor) -> tuple[Tensor, ...]:
        ...
        if ctx.bias:
            ...
            dist.reduce(
                output_grad, dst=dst_rank, group=get_parallel_group(col_parallel_mode)
            )
            if row_rank == 0:
                return (
                    None,
...
class _AllGatherTensor2p5D(torch.autograd.Function):
    ...
    @custom_fwd(cast_inputs=torch.float16)
    def forward(
        ctx: Any, inputs: Tensor, dim: int, col_parallel_mode: ParallelMode
    ) -> Tensor:
        ctx.dim = dim
        ctx.col_parallel_mode = col_parallel_mode

        return all_gather(inputs, dim, col_parallel_mode)
...
def all_gather_tensor_2p5d(
    inputs: Tensor, dim: int, col_parallel_mode: ParallelMode
) -> Tensor:
...
class SplitFirst(torch.autograd.Function):
    ...
    @custom_fwd(cast_inputs=torch.float16)
    def forward(
        ctx: Any, inputs: Tensor, tesseract_dim: int, col_parallel_mode: ParallelMode
    ) -> Tensor:
        ctx.tesseract_dim = tesseract_dim
        ...
        return inputs.chunk(tesseract_dim, dim=0)[row_rank]
...
def split_batch_2p5d(input_: Tensor, dim: int = 0) -> Tensor:
    ...
    return torch.chunk(
        input_, gpc.get_world_size(ParallelMode.PARALLEL_2P5D_COL), dim=dim
    )[gpc.get_local_rank(ParallelMode.PARALLEL_2P5D_COL)].contiguous()
...
def reduce_scatter_tensor_2p5d(
    input_: Tensor, dim: int, parallel_mode: ParallelMode
) -> Tensor:

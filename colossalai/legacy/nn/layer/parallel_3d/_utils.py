...
def dbg_check_shape(tensor: Tensor, shape: tuple):
    ...
    if rank == 0:
        print(tensor.shape)
    assert tensor.shape == shape, f"{tensor.shape} does not match {shape}"


class AsyncGradientBucket:
    def __init__(self):
    ...
    def push(self, async_op, grad_tensor, param_id):
        self.bucket[param_id] = (async_op, grad_tensor)

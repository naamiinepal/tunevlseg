...
class _Linear3D(torch.autograd.Function):
    ...
    def forward(
        ...
        return reduce_scatter(output, 0, output_parallel_mode)
    ...
    def backward(ctx, output_grad: Tensor) -> tuple[Tensor, ...]:
        ...
        input_grad, input_op = reduce_scatter(
            input_grad, 0, ctx.input_parallel_mode, async_op=True
        )
        ...
        weight_grad, op = reduce_scatter(
            weight_grad, 0, ctx.weight_parallel_mode, async_op=True
        )
...
class _Classifier3D(torch.autograd.Function):
    ...
    def forward(
        ...
        src_rank = gpc.get_ranks_in_group(input_parallel_mode)[
            gpc.get_local_rank(output_parallel_mode)
        ]
    ...
    def backward(ctx, output_grad: Tensor) -> tuple[Tensor, ...]:
        ...
        if gpc.get_local_rank(ctx.input_parallel_mode) == gpc.get_local_rank(
            ...
            weight_grad, op = all_reduce(
                weight_grad, ctx.weight_parallel_mode, async_op=True
            )
            weight_grad = push_async_grad(op, weight_grad, ctx.weight_id)
        else:
            weight_grad = None
        ...
        if ctx.use_bias:
            ...
            bias_grad, op = all_reduce(
                bias_grad, ctx.weight_parallel_mode, async_op=True
            )
...
class _VocabParallelClassifier3D(torch.autograd.Function):
    ...
    def backward(ctx, output_grad: Tensor) -> tuple[Tensor, ...]:
        ...
        weight_grad, op = reduce_scatter(
            weight_grad.transpose(0, 1), 0, ctx.weight_parallel_mode, async_op=True
        )
        weight_grad = push_async_grad(op, weight_grad, ctx.weight_id)
...
@torch.jit.script
def norm_forward(
    x: Tensor, mean: Tensor, sqr_mean: Tensor, weight: Tensor, bias: Tensor, eps: float
):
    mu = x - mean
    var = sqr_mean - mean**2
...
class _Layernorm3D(torch.autograd.Function):
    ...
    def forward(
        ...
        mean, sqr_mean = (
            all_reduce(torch.stack((sum_, sqr_sum)), output_parallel_mode)
            / normalized_shape
        )
    ...
    def backward(ctx, output_grad: Tensor) -> tuple[Tensor, ...]:
        ...
        bias_grad, op = all_reduce(
            bias_grad, ctx.input_x_weight_parallel_mode, async_op=True
        )
        bias_grad = push_async_grad(op, bias_grad, ctx.bias_id)
        weight_grad = torch.sum(
            weight_grad, dim=tuple(range(len(weight_grad.shape))[:-1])
        )
        weight_grad, op = all_reduce(
            weight_grad, ctx.input_x_weight_parallel_mode, async_op=True
        )
        weight_grad = push_async_grad(op, weight_grad, ctx.weight_id)
...
def split_tensor_3d(tensor: Tensor, dim: int, parallel_mode: ParallelMode) -> Tensor:
    ...
    if tensor.size(dim) <= 1:
        return tensor
    return torch.chunk(tensor, gpc.get_world_size(parallel_mode), dim=dim)[
...
def split_batch_3d(
    ...
    output = torch.chunk(input_, weight_world_size, dim=dim)[
        gpc.get_local_rank(weight_parallel_mode)
    ].contiguous()
    return torch.chunk(output, input_world_size, dim=dim)[
        gpc.get_local_rank(input_parallel_mode)
    ].contiguous()
...
class _AllGatherTensor3D(torch.autograd.Function):
    ...
    def forward(ctx, input_, dim, parallel_mode):
        ...
        return all_gather(input_, dim, parallel_mode)
...
def all_gather_tensor_3d(
    tensor: Tensor, dim: int, parallel_mode: ParallelMode
) -> Tensor:
...
class _ReduceByBatch3D(torch.autograd.Function):
    ...
    def forward(
        ...
        if reduce_mean:
            reduce_size = gpc.get_world_size(input_parallel_mode) * gpc.get_world_size(
                weight_parallel_mode
            )
...
def reduce_by_batch_3d(
    ...
    return _ReduceByBatch3D.apply(
        tensor, input_parallel_mode, weight_parallel_mode, reduce_mean
    )

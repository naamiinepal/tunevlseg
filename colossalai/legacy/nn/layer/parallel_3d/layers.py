...
class Linear3D(ParallelLayer):
    ...
    def _sync_grad_hook(self, grad) -> Tensor:
        return all_reduce(grad.clone(), self.output_x_weight_parallel_mode)
...
class PatchEmbedding3D(ParallelLayer):
    ...
    def _sync_grad_hook(self, grad) -> Tensor:
        return all_reduce(grad.clone(), self.input_x_weight_parallel_mode)
...
class VocabParallelEmbedding3D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        return reduce_scatter_tensor_3d(output_parallel, 0, self.input_parallel_mode)

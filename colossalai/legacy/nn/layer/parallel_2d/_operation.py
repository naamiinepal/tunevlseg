...
def matmul_2d(
    ...
    data_parallel_rank = (
        ...
        else gpc.get_local_rank(ParallelMode.DATA)
    )
    ...
    pipeline_parallel_size = (
        1
        if not gpc.is_initialized(ParallelMode.PIPELINE)
        else gpc.get_world_size(ParallelMode.PIPELINE)
    )
...
def layernorm_2d(
    ...
    return _Layernorm_2D.apply(
        input_, E_x, Var_x, hidden_size, row_parallel_mode, col_parallel_mode
    )
...
class _AllGatherTensor2D(torch.autograd.Function):
    ...
    def forward(
        ...
        return all_gather(inputs, dim, parallel_mode)
...
def split_batch_2d(input_: Tensor, dim: int = 0) -> Tensor:
    ...
    return torch.chunk(
        input_, gpc.get_world_size(ParallelMode.PARALLEL_2D_COL), dim=dim
    )[gpc.get_local_rank(ParallelMode.PARALLEL_2D_COL)].contiguous()

...
class Classifier1D(ParallelLayer):
    ...
    def __init__(
        ...
        if weight is not None:
            ...
            self.weight = Parameter(
                torch.empty(
                    self.num_classes, self.input_size_per_partition, **factory_kwargs
                )
            )
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            ...
            ), f"Invalid shapes in Classifier1D forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1]}."
            input_ = input_
        else:
            assert (
                divide(input_.shape[-1], gpc.tensor_parallel_size)
                == self.weight.shape[-1]
            ), f"Invalid shapes in Classifier1D forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1] * gpc.tensor_parallel_size}."
...
class VocabParallelClassifier1D(ParallelLayer):
    ...
    def __init__(
        ...
        if weight is not None:
            ...
            self.weight = Parameter(
                torch.empty(
                    self.num_classes_per_partition, self.in_features, **factory_kwargs
                )
            )
            self.has_weight = True
        if bias:
            self.bias = Parameter(
                torch.empty(self.num_classes_per_partition, **factory_kwargs)
            )
        else:
            self.bias = None
        with seed(ParallelMode.TENSOR):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        assert (
            input_.shape[-1] == self.weight.shape[-1]
        ), f"Invalid shapes in VocabParallelClassifier1D forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1]}."
        # Set up backprop all-reduce.
...
class Linear1D_Col(ParallelLayer):
    ...
    def __init__(
        ...
        self.weight = Parameter(
            torch.empty(
                self.out_features_per_partition, self.in_features, **factory_kwargs
            )
        )
    ...
    def forward(self, input_: Tensor) -> tuple[Tensor, Tensor]:
        assert (
            input_.shape[-1] == self.weight.shape[-1]
        ), f"Invalid shapes in Linear1D_Col forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1]}."
        # Set up backprop all-reduce.
        # input_parallel = reduce_grad(input_, ParallelMode.PARALLEL_1D)
        ...
        output_parallel = linear_with_async_comm(
            input_parallel, self.weight, bias, ParallelMode.PARALLEL_1D, True
        )
        if self.gather_output:
            # All-gather across the partitions.
            output = gather_forward_split_backward(
...
class Linear1D_Row(ParallelLayer):
    ...
    def __init__(
        ...
        self.weight = Parameter(
            torch.empty(
                self.out_features, self.input_size_per_partition, **factory_kwargs
            )
        )
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            assert (
                input_.shape[-1] == self.weight.shape[-1]
            ), f"Invalid shapes in Linear1D_Row forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1]}."
            input_ = input_
        else:
            assert (
                divide(input_.shape[-1], gpc.tensor_parallel_size)
                == self.weight.shape[-1]
            ), f"Invalid shapes in Linear1D_Row forward: input={input_.shape}, weight={self.weight.shape}. Expected last dim of input {self.weight.shape[-1] * gpc.tensor_parallel_size}."
            input_ = split_forward_gather_backward(
                input_, ParallelMode.PARALLEL_1D, dim=-1
            )
...
class Embedding1D(ParallelLayer):
    ...
    def _set_tensor_parallel_attributes(self):
        set_tensor_parallel_attribute_by_partition(
            self.weight, gpc.tensor_parallel_size
        )
...
class VocabParallelEmbedding1D(ParallelLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vocab_start_index = (
            tensor_parallel_rank * self.num_embeddings_per_partition
        )
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        return reduce_input(output_parallel, ParallelMode.PARALLEL_1D)

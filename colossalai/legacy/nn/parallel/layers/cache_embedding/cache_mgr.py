...
def _wait_for_data(t, stream: torch.cuda.streams.Stream | None) -> None:
    if stream is None:
        return
...
class CachedParamMgr(torch.nn.Module):
    ...
    def timer(self, name):
        ...
        if name not in self._elapsed_dict:
    ...
    @torch.no_grad()
    def reorder(self, ids_freq_mapping: list[int] | None = None, warmup_ratio=0.7):
        """reorder
    ...
    def _id_to_cached_cuda_id(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        return self.inverted_cached_idx.index_select(0, ids)
    ...
    def prepare_ids(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        """
        torch.cuda.synchronize()
        with self.timer("cache_op"):
            # identify cpu rows to cache
            with self.timer("1_identify_cpu_row_idxs"):
            ...
            with record_function("(cache) prepare_rows_on_cuda"):
                with self.timer("prepare_rows_on_cuda"):
            ...
            with self.timer("6_update_cache"):
    ...
    def _prepare_rows_on_cuda(self, cpu_row_idxs: torch.Tensor) -> None:
        ...
        if evict_num > 0:
            with self.timer("2_identify_cuda_row_idxs"):
                ...
                if self._evict_strategy == EvictionStrategy.DATASET:
                    ...
                    with self.timer("2_1_find_evict_gpu_idxs"):
            ...
            with self.timer("3_evict_out"):
                if self.buffer_size > 0:
                    ...
                    if self._async_copy:
                        ...
                        with self.timer("3_1_evict_out_index_select"):
                            evict_out_rows_cpu = self.cuda_cached_weight.view(
                                self.cuda_row_num, -1
                            ).index_select(0, evict_gpu_row_idxs)
                        with self.timer("3_2_evict_out_gpu_to_cpu_copy"):
                            evict_out_rows_cpu = evict_out_rows_cpu.cpu()

                    with self.timer("3_2_evict_out_cpu_copy"):
        ...
        with self.timer("4_identify_cuda_slot"):
            slots = torch.nonzero(self.cached_idx_map == -1).squeeze(1)[
                : cpu_row_idxs.numel()
            ]

        # TODO wait for optimize
        with self.timer("5_evict_in"):
            ...
            if self.buffer_size > 0:
                ...
                if self._async_copy:
                    ...
                    with self.timer("5_1_evict_in_index_select"):
                    ...
                    with self.timer("5_2_evict_in_gpu_to_cpu_copy"):
                        evict_in_rows_gpu = evict_in_rows_gpu.cuda()

                    with self.timer("5_3_evict_in_index_copy"):
                        self.cuda_cached_weight.view(self.cuda_row_num, -1).index_copy_(
    ...
    def _evict(self) -> int:
        ...
        _max_row, max_cpu_row_idx = torch.max(self.cached_idx_map, dim=0)

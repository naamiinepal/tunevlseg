...
def is_colo_module(module: torch.nn.Module):
    global _COLOSSAL_MODULES
    return any(isinstance(module, module_type) for module_type in _COLOSSAL_MODULES)
...
def check_colo_module(module: torch.nn.Module, pg: ProcessGroup, recursive=True):
    if is_colo_module(module):
        ...
        if compute_pattern is not None:
            ...
            for param_specs in allowed_specs.values():
                cur_match = True
    ...
    if is_colo_module(module):
        ...
        for param_name, dist_spec in colo_module.get_dist_specs_with_mode(
            ...
            if isinstance(param, ColoParameter):
                ...
                modules_update_param.update(param.shared_param_modules)

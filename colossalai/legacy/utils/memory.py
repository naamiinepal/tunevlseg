...
def report_memory_usage(message, logger=None, report_cpu=False):
    ...
    if not dist.is_initialized():
        raise OSError("No distributed environment is initialized")
...
def colo_device_memory_used(device: torch.device) -> int:
    ...
    if device.type == "cpu":
        ...
        return mem_info.used / gpc.num_processes_on_current_node
    if device.type == "cuda":
...
def colo_set_process_memory_fraction(ratio: float) -> None:
    ...
    torch.cuda.set_per_process_memory_fraction(
        _GLOBAL_CUDA_MEM_FRACTION, get_accelerator().get_current_device()
    )
...
def colo_set_cpu_memory_capacity(size: int) -> None:
    ...
    _GLOBAL_CPU_MEM_CAPACITY = min(size, total_size)

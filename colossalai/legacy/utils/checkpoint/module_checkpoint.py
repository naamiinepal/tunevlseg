...
def save_checkpoint(
    ...
    **kwargs,
    ...
    if rank == 0:
        ...
        torch.save(save_state, path + f"/epoch_{epoch}_model.pth", *args, **kwargs)
    ...
    if optimizer is not None:
        ...
        if rank == 0:
            ...
            torch.save(save_state, path + f"/epoch_{epoch}_optim.pth", *args, **kwargs)
...
def load_checkpoint(
    ...
    load_state_dict_kwargs: dict | None = None,
):
    """load_checkpoint
    ...
    if rank == 0:
        load_state = torch.load(path + f"/epoch_{epoch}_model.pth", **torch_load_kwargs)
    ...
    if optimizer is not None:
        ...
        if rank == 0:
            colo_checkpoint = torch.load(
                path + f"/epoch_{epoch}_optim.pth", **torch_load_kwargs
            )
            optimizer.load_state_dict(
                colo_checkpoint["optim"], **load_state_dict_kwargs
            )
        dist.barrier()

...
def _get_code_location(depth: int):
    ...
    for i in range(3, length):
        ...
        ret.extend(
            (upper_frame.filename, "(", str(upper_frame.lineno), "): ", function_name)
        )
        if i != length - 1:
            ret.append("\n")
...
class CommProfiler(BaseProfiler):
    ...
    def to_tensorboard(self, writer):
        writer.add_text(
            tag="Collective Communication", text_string=self.result_str("\n\n")
        )
    ...
    def result_str(self, sep: str = "\n"):
        ...
        append(f"total cuda time: {_format_time(self.total_cuda_time)}")
        append(
            f"average bandwidth: {_format_bandwidth(self.total_comm_vol, self.total_cuda_time)}"
        )
        append(f"total number of calls: {self.total_count}")
        append("All events:")
        ...
        show_list = sorted(
            self.ops_record.items(), key=lambda kv: -kv[1].self_cuda_time
        )
        for location, event in show_list:
            ...
            append(
                row_format.format(
                    ...
                    f"{event.self_cuda_time / self.total_cuda_time * 100.0:.1f}%",
    ...
    def activate_profiler(self, kn: str, vol: float):
        ...
        self.profiler = profile(
            enabled=True, use_cuda=True, use_cpu=True, use_kineto=True
        )
    ...
    def close_profiler(self, group=None):
        ...
        if self.profiler.enabled and dist.get_world_size(group) > 1:
            ...
            buffer = torch.tensor(
                [current_comm_event.self_cuda_time],

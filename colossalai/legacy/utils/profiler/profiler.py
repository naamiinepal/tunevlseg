...
from colossalai.legacy.utils.profiler.stateful_tensor_mem_extention import (
    StatefulTensorMemoryProfilerExtention,
)
...
class profile(torch_profile):
        ...
        if profile_stateful_tensor_memory:
            if engine is None:
                self._logger.warning(
                    'Ignore "profile_model_data" since engine is None', ranks=[0]
                )
            else:
    ...
    def export_chrome_trace(self, path: str):
        ...
        fp = tempfile.NamedTemporaryFile(
            "w+t", suffix=".json", delete=False, encoding="locale"
        )
        fp.close()
        retvalue = self.profiler.export_chrome_trace(fp.name)
        with open(fp.name, encoding="locale") as fin:
            trace = json.load(fin)
            for ext in self.extentions:

...
def partition_tensor_parallel_state_dict(
    ...
    dims: dict | None = None,
    partition_states: dict | None = None,
):
    ...
    for key, (is_partitioned, dtype, shape, dim) in partition_info[0].items():
        if is_partitioned:
            ...
            output = state_dict[key] if is_rank0 else torch.empty(shape, dtype=dtype)
...
def _recv_state_dict(src, parallel_mode):
    ...
    return dist.distributed_c10d._tensor_to_object(state_tensor, state_size)
...
def load_checkpoint(
    ...
    # model states
    model_state = state_dict.pop("model") if state_dict is not None else {}
    # pipeline
    if is_using_pp():

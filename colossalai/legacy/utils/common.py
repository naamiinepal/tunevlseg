...
def _move_norm_to_cuda(norm: float | torch.Tensor) -> float | torch.Tensor:
    if torch.is_tensor(norm) and norm.device.type != "cuda":
        norm = norm.to(torch.cuda.current_device())
    return norm


def _get_tensor_norm(norm: float | torch.Tensor, move_to_cuda) -> torch.Tensor:
...
def _compute_buckets_lp(params: list[ColoParameter], norm_type: float) -> float:
    if len(params) == 0:
        return 0.0
    buckets: dict[ProcessGroup | None, list[ColoParameter]] = defaultdict(list)
    for p in params:
        if p.is_replicate():
...
def _compute_grad_lp(parameters, norm_type: float = 2.0) -> float:
    ...
    total_lp = max(cpu_lp, cuda_lp) if norm_type == inf else cpu_lp + cuda_lp

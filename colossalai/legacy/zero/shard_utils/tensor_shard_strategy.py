...
class TensorShardStrategy(BaseShardStrategy):
    ...
    def _shard_tensor(
        self, t: ShardedTensor, process_group: dist.ProcessGroup | None = None
    ):
        ...
        sharded_payload, _ = get_shard(
            t.payload, dist.get_rank(process_group), dist.get_world_size(process_group)
        )
    ...
    def _gather_tensor(
        ...
        buffer = torch.empty(
            payload_numel * world_size,
            dtype=t.payload.dtype,
        ...
        dist.all_gather(
            buffer_list, buffer_list[rank], group=process_group, async_op=False
        )
        gathered_payload = torch.narrow(buffer, 0, 0, t.origin_numel).reshape(
            t.origin_shape
        )

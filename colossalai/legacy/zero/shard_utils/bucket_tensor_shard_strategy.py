...
class BucketTensorShardStrategy(TensorShardStrategy):
    ...
    def gather(
        ...
        for i in range(world_size):
            if i == rank:
                buffer_list.append(
                    flatten([t.payload for t in tensor_list]).cuda(
        ...
        for i, t in enumerate(tensor_list):
            gathered_payload = [
                buffer[offset : offset + tensor_numels[i]] for buffer in buffer_list
            ]
            gathered_payload = torch.cat(gathered_payload)[: t.origin_numel].view(
                t.origin_shape
            )

...
class TensorPlacementPolicy(ABC):
    def __init__(
        ...
        mem_stats_collector: MemStatsCollector | None = None,
    ) -> None:
        self.device: torch.device | None = device
        self.mem_stats_collector: MemStatsCollector | None = mem_stats_collector
...
class CPUTensorPlacementPolicy(TensorPlacementPolicy):
    def __init__(self, mem_stats_collector: MemStatsCollector | None = None) -> None:
...
class CUDATensorPlacementPolicy(TensorPlacementPolicy):
    def __init__(self, mem_stats_collector: MemStatsCollector | None = None) -> None:
        ...
        super().__init__(
            ...
            mem_stats_collector=mem_stats_collector,
        )
...
class AutoTensorPlacementPolicy(TensorPlacementPolicy):
    ...
    def evict_tensors(
        ...
        cuda_demand: int = 0,
        warmup: bool = True,
        compute_list: list[StatefulTensor] | None = None,
        compute_idx: int = 0,
        **kwargs,
    ) -> int:
        """

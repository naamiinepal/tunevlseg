...
def colo_tensor_mem_usage(tensor: torch.Tensor | StatefulTensor) -> tuple[int, int]:
...
def colo_model_data_tensor_move(
    src_t: StatefulTensor | torch.Tensor, tgt_t: StatefulTensor | torch.Tensor
) -> None:
    """
    ...
    """
    src_t_payload = src_t.payload if isinstance(src_t, StatefulTensor) else src_t.data
    src_dev = src_t_payload.device

    tgt_t_payload = tgt_t.payload if isinstance(tgt_t, StatefulTensor) else tgt_t.data
...
def colo_model_data_tensor_move_inline(
    t: StatefulTensor | torch.Tensor, target_device: torch.device | int
) -> None:
    """
    ...
    if isinstance(t, torch.Tensor):
        ...
        raise TypeError(
            f"colo_model_data_tensor_move_inline dose not accept type {type(t)}"
        )


def colo_model_data_move_to_cpu(t: StatefulTensor | torch.Tensor) -> None:
...
def colo_model_tensor_clone(
    t: StatefulTensor | torch.Tensor, target_device: torch.device
) -> torch.Tensor:
    """

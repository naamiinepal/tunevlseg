...
def _named_params_with_replica(
    ...
    prefix: str = "",
    recurse: bool = True,
) -> Iterator[tuple[str, nn.Parameter | ColoTensor]]:
...
def _convert_to_coloparam(
    ...
    device: torch.device,
    dtype=torch.float,
    default_pg: ProcessGroup | None = None,
    default_dist_spec: Any | None = None,
) -> ColoParameter:
...
class ColoInitContext(InsertPostInitMethodToModuleSubClasses):
    ...
    def _post_init_method(self, module: torch.nn.Module, *args, **kwargs):
        ...
        replaced_tensors = {}  # record mapping between (torch.Tensor, ColoTensor) to distinguish the same reference

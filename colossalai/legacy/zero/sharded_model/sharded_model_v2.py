...
from colossalai.legacy.zero.gemini.tensor_placement_policy import (
...
class ShardedModelV2(nn.Module):
    ...
    def __init__(
        ...
        reduce_scatter_process_group: ProcessGroup | None = None,
        reduce_scatter_bucket_size_mb: int = 25,
        fp32_reduce_scatter: bool = False,
        tensor_placement_policy: str = "cuda",
        gradient_predivide_factor: float | None = 1.0,
        ...
        **kwargs,
        ...
        for submodule in module.modules():
            ...
            for param in submodule.parameters(recurse=False):
                ...
                if param.colo_attr.param_is_sharded:
                    ...
                    unshard_cnt += 1
            assert (not sharded_cnt) or (
                not unshard_cnt
            ), "nn.Module can not both have shard param and unshard param"
        ...
        self.reduce_scatter_process_group = (
            reduce_scatter_process_group or self.process_group
        )
        ...
        if self._use_memory_tracer:
            ...
            self._start_collect_memstats = disposable(
                self._memstats_collector.start_collection
            )
            self._finish_collect_memstats = disposable(
                self._memstats_collector.finish_collection
            )
        else:
            self._memstats_collector = None
        self._tensor_placement_policy: TensorPlacementPolicy = (
            TensorPlacementPolicyFactory.create(tensor_placement_policy)(
        ...
        if "warmup_non_model_data_ratio" in kwargs:
            if tensor_placement_policy != "auto":
                ...
                self.logger.info(
                    f"setting warmup_non_model_data_ratio as {ratio} for auto placement"
                )

        self._stateful_tensor_mgr = StatefulTensorMgr(self._tensor_placement_policy)
        param_tensor_list = [
            p.colo_attr.sharded_data_tensor
    ...
    def dump_memory_stats(self, filename: str | None = "dump_mem_stats.log") -> None:
        """
        ...
        if self._use_memory_tracer:
            self.logger.error(
                f"dump memory tracer collected information to a {filename}", ranks=[0]
            )
            if gpc.get_global_rank() == 0:
                with open(filename, "w+", encoding="locale") as f:
                    ...
                    f.write(
                        str(
                            self._memstats_collector._memstats.non_model_data_list(
                                "cuda"
                            )
                        )
                    )
    ...
    @torch.no_grad()
    def _grad_post_backward_hook(
        self, param: Parameter, grad: torch.Tensor
    ) -> torch.Tensor | None:
        """
        ...
        with torch.cuda.stream(self.comm_stream):
            ...
            if self.world_size > 1:
                grad_chunks = chunk_and_pad(
                    grad, self.reduce_scatter_process_group.size()
                )
    ...
    def _save_grad(self, param: Parameter, grad: torch.Tensor):
        ...
        if self.reuse_fp16_shard:
            ...
            param.colo_attr.data_payload_reset(
                torch.empty(0, device=grad.device, dtype=grad.dtype)
            )
            ...
            if param.colo_attr.saved_grad.is_null():
                ...
                param.colo_attr.grad_payload.add_(
                    fp32_grad.view_as(param.colo_attr.grad_payload)
                )
        ...
        for name, p in self.named_parameters():
            if name in state_dict:
                p.colo_attr.data_payload_reset(
                    state_dict[name].to(
                        dtype=p.colo_attr.data_payload.dtype,
    ...
    def _colo_state_dict(
        ...
        prefix="",
        keep_vars=False,
        shard_strategy: BaseShardStrategy | None = None,
        ...
        if shard_strategy is not None:
            shard_strategy.gather(
                [p.colo_attr.sharded_data_tensor for p in sharded_params], process_group
            )
        for p in sharded_params:
            p.data = p.colo_attr.data_payload
        module_to_load = module_to_load or self
        gathered_state_dict = state_dict_func(
            module_to_load, destination, prefix, keep_vars
        )
        gathered_state_dict = {
            k: v.cpu() if isinstance(v, torch.Tensor) else v
            for k, v in gathered_state_dict.items()
        }
        if shard_strategy is not None:
    ...
    def _colo_load_from_state_dict(
        ...
        for name, param in local_state.items():
            ...
            if key in state_dict:
                ...
                if hasattr(param, "colo_attr"):
                    param.colo_attr.data_payload_reset(
                        input_param.to(
                            dtype=param.colo_attr.data_payload.dtype,
        ...
        if (
            getattr(self.__class__, "set_extra_state", nn.Module.set_extra_state)

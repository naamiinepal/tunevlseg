...
from colossalai.legacy.zero.gemini.tensor_utils import (
...
class ShardedOptimizerV2(OptimizerWrapper):
    ...
    def __init__(
        ...
        mp_process_group: ProcessGroup | None = None,
        verbose: bool = False,
    ) -> None:
        assert isinstance(
            sharded_model, ShardedModelV2
        ...
        self.device = sharded_model._tensor_placement_policy.device or torch.device(
            "cpu"
        )
        ...
        self._found_overflow: Tensor = torch.IntTensor([0]).to(
        ...
        if self._verbose:
            self._logger.debug(
                f"After init ShardedOptimizerV2 consumes {self.get_memory_usage()[0] / 1e6} MB CUDA Memory!",
                ranks=[0],
            )
    ...
    def get_memory_usage(self) -> tuple[int, int]:
        ...
        for p_fp32 in self.master_params.values():
    ...
    def _register_master_weight(self):
        ...
        for group in self.optim.param_groups:
            for p in group["params"]:
                ...
                shard_flag = (
                    not p.colo_attr.sharded_data_tensor.is_sharded
                    and p.colo_attr.is_replicated
                )
                if shard_flag:
                    # we always shard replicated parameters
                    self.shard_strategy.shard(
                        [p.colo_attr.sharded_data_tensor], self.dp_process_group
                    )
                self.master_params[p] = StatefulTensor(
                    cast_tensor_to_fp32(p.colo_attr.data_payload.to(self.device))
                )
                if shard_flag:
                    # In this branch, there's no need to shard param
                    # So we gather here
                    self.shard_strategy.gather(
                        [p.colo_attr.sharded_data_tensor], self.dp_process_group
                    )
    ...
    def _maybe_move_fp32_shards(self):
        if self._should_move_fp32_shards_h2d:
            ...
            available_cuda_margin_mem = (
                self.model.cuda_margin_space * self.gpu_margin_mem_ratio
            )
            ...
            for group in self.optim.param_groups:
                for p in group["params"]:
                    if p.colo_attr.saved_grad.is_null():
                        continue
                    shard_mem = (
                        self.master_params[p].payload.numel()
                        * self.master_params[p].payload.element_size()
                    )
                    if (
                        ...
                        colo_model_data_tensor_move_inline(
                            self.master_params[p], torch.cuda.current_device()
                        )
                        colo_model_data_tensor_move_inline(
                            p.colo_attr.saved_grad, torch.cuda.current_device()
                        )
    ...
    def _copy_master_param_to_param_fp16(self, p):
        ...
        if p.colo_attr.keep_not_shard and p.colo_attr.is_replicated:
            ...
            p.colo_attr.sharded_data_tensor.is_sharded = (
                True  # since only gradient is sharded, we should set to True
            )
            self.shard_strategy.gather(
                [p.colo_attr.sharded_data_tensor], self.dp_process_group
            )
    ...
    def load_state_dict(self, *args, **kwargs):
        if "scaler" not in args[0]:
            self._logger.warning(
                "Missing scaler when loading optimizer state dict", ranks=[0]
            )
        else:
        ...
        for group in self.optim.param_groups:
            for p in group["params"]:
                ...
                for k, v in state.items():
                    if isinstance(v, Tensor):
                        state[k] = v.to(
                            dtype=self.master_params[p].dtype,
                            device=self.master_params[p].device,
                        )

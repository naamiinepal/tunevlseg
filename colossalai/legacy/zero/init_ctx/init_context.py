...
from colossalai.legacy.zero.sharded_model._utils import (
    cast_tensor_to_bf16,
    cast_tensor_to_fp16,
)
...
class ZeroInitContext(InsertPostInitMethodToModuleSubClasses):
    ...
    def __init__(
        ...
        self.config = ZeroContextConfig(
            target_device=target_device, is_replicated=True, shard_param=shard_param
        )
    ...
    def calc_fanin_fanout(tensor: torch.Tensor):
        ...
        assert isinstance(
            tensor, nn.Parameter
        ), "Sharded tensor initialization is only allowed for parameters"
    ...
    def _post_context_exec(self):
        ...
        for param in self.param_list:
            ...
            if not param.colo_attr.param_is_sharded and param.colo_attr.is_replicated:
                dist.broadcast(
                    tensor=param.data, src=src_rank, group=self.dp_process_group
                )
        ...
        for param in module.parameters(recurse=False):
            ...
            if self.shard_param:
                self.shard_strategy.shard(
                    [param.colo_attr.sharded_data_tensor], self.dp_process_group
                )
...
class ZeroContextMgr(metaclass=SingletonMeta):
    current_context: ZeroInitContext | None = None

...
class Engine:
    ...
    def __init__(
        ...
        gradient_handlers: list[BaseGradientHandler] | None = None,
        clip_grad_norm: float = 0.0,
        ophook_list: list[BaseOpHook] | None = None,
        verbose: bool = True,
    ...
    def uses_pipeline(self):
        ...
        return isinstance(
            self._schedule, PipelineSchedule | InterleavedPipelineSchedule
        )
    ...
    def add_hook(self, ophook: type[BaseOpHook]) -> None:
        ...
        for h in self._ophook_list:
            if type(h) == type(ophook):
                ...
                logger.warning(
                    f"duplicate hooks, at least two instance of {type(ophook)}"
                )

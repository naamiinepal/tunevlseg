...
class BaseSchedule(ABC):
    ...
    def __init__(self, data_process_func: Callable | None = None):
            ...
            for element in data:
                if isinstance(element, dict):
                    data_to_return.append(
                        {k: self._move_tensor(v) for k, v in element.items()}
                    )
                else:
    ...
    def _get_batch_size(self, data):
        ...
        if isinstance(data, list | tuple):
            if isinstance(data[0], dict):
                return data[0][next(iter(data[0].keys()))].size(0)
            return data[0].size(0)
        if isinstance(data, dict):
            return data[next(iter(data.keys()))].size(0)
        return None
    ...
    def _call_engine_criterion(engine, outputs, labels):
        ...
        if isinstance(outputs, tuple | list) and isinstance(labels, dict):
            return engine.criterion(*outputs, **labels)
        if isinstance(outputs, dict) and isinstance(labels, dict):
            return engine.criterion(**outputs, **labels)
        if isinstance(outputs, dict) and isinstance(labels, list | tuple):
            raise ValueError(
                f"Expected labels to be a dict when the model outputs are dict, but got {type(labels)}"
            )
        raise TypeError(

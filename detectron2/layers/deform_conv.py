...
class _DeformConv(Function):
    ...
    def forward(
        ...
        if input is not None and input.dim() != 4:
            msg = f"Expected 4D tensor as input, got {input.dim()}D tensor instead."
            raise ValueError(
                msg,
            )
        ...
        if not input.is_cuda:
            ...
            if deformable_groups != 1:
                msg = "Deformable Conv with deformable_groups != 1 is not supported on CPUs!"
    ...
    def backward(ctx, grad_output):
        ...
        if not grad_output.is_cuda:
            msg = "Deformable Conv is not supported on CPUs!"
    ...
    def _output_size(input, weight, padding, dilation, stride):
        ...
        if not all(s > 0 for s in output_size):
            msg = "convolution input is too small (output would be {})".format(

...
class Instances:
    ...
    def __getattr__(self, name: str) -> Any:
        if name == "_fields" or name not in self._fields:
            msg = f"Cannot find field '{name}' in the given Instances!"
    ...
    def __getitem__(self, item: int | slice | torch.BoolTensor) -> "Instances":
        ...
        if isinstance(item, int):
            if item >= len(self) or item < -len(self):
                msg = "Instances index out of range!"
    ...
    def __len__(self) -> int:
        for v in self._fields.values():
            ...
            return v.__len__()
        msg = "Empty Instances does not support __len__!"
        raise NotImplementedError(msg)

    def __iter__(self):
        msg = "`Instances` object is not iterable!"
        raise NotImplementedError(msg)
    ...
    def cat(instance_lists: list["Instances"]) -> "Instances":
        ...
        for k in instance_lists[0]._fields:
            ...
            if isinstance(v0, torch.Tensor):
                ...
                msg = f"Unsupported type {type(v0)} for concatenation"

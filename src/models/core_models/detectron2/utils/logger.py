...
def log_first_n(lvl, msg, n=1, *, name=None, key="caller") -> None:
...
def log_every_n(lvl, msg, n=1, *, name=None) -> None:
...
def log_every_n_seconds(lvl, msg, n=1, *, name=None) -> None:
    ...
    """
    caller_module, key = _find_caller()
    last_logged = _LOG_TIMER.get(key)
...
def create_small_table(small_dict):
    ...
    """
    keys, values = tuple(zip(*small_dict.items(), strict=True))
    return tabulate(
        [values],
        headers=keys,
        tablefmt="pipe",
...
def _log_api_usage(identifier: str) -> None:

...
class SideAdapterNetwork(nn.Module):
    ...
    def __init__(
        ...
        encode_layers = []
        for _i in range(cfg_encoder.num_encode_layer):
        ...
        conv_clips = []
        for _i in range(len(fusion_index)):
            conv_clips.append(
                nn.Sequential(
    ...
    def fuse_clip(
        ...
        return torch.cat([x[:, :-L, ...], x[:, -L:, ...] + fused_clip], dim=1)
...
class SideAdapterCLIPHead(BaseDecodeHead):
    ...
    def loss(
        ...
        # loss
        return self.loss_by_feat(all_mask_logits, all_mask_props, batch_gt_instances)
        ...
        for i in range(num_dec_layers):
            ...
            for loss_decode in losses_decode:
                if "loss_cls" in loss_decode.loss_name:
                    ...
                    raise AssertionError("Only support for 'loss_cls' and 'loss_mask'")

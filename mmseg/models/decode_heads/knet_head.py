...
class KernelUpdateHead(nn.Module):
    ...
    def __init__(
        ...
        self.attention_norm = build_norm_layer(
            {"type": "LN"}, in_channels * conv_kernel_size**2
        )[1]
        ...
        if self.with_ffn:
            ...
            self.ffn_norm = build_norm_layer({"type": "LN"}, in_channels)[1]

        self.mask_fcs = nn.ModuleList()
        for _ in range(num_mask_fcs):
            self.mask_fcs.append(nn.Linear(in_channels, in_channels, bias=False))
            self.mask_fcs.append(build_norm_layer({"type": "LN"}, in_channels)[1])
...
class IterativeDecodeHead(BaseDecodeHead):
    ...
    def loss_by_feat(
        self, seg_logits: list[Tensor], batch_data_samples: SampleList, **kwargs
    ) -> dict:
        losses = {}

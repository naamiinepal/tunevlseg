...
# Modified from https://github.com/MendelXu/SAN/blob/main/san/model/attn_helper.py
...
def cross_attn_with_self_bias(
    ...
    if not use_separate_proj_weight:
        ...
        if key is value or torch.equal(key, value):
    ...
    if attn_mask is not None:
        assert (
            ...
            f"attn_mask, not {attn_mask.dtype}"
        )
...
def cross_attn_layer(tf_layer: BaseTransformerLayer, x, mem, attn_bias):
    ...
    return tf_layer.ffns[0](tf_layer.norms[1](x), identity=x)
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            starmap(affine_func, zip([input_dim, *h], [*h, output_dim], strict=False))
        )

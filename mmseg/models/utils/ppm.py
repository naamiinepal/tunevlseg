...
class DAPPM(BaseModule):
    ...
    def __init__(
        ...
        if conv_cfg is None:
            conv_cfg = {"order": ("norm", "act", "conv"), "bias": False}
        if act_cfg is None:
            act_cfg = {"type": "ReLU", "inplace": True}
        if norm_cfg is None:
            norm_cfg = {"type": "BN", "momentum": 0.1}
        if paddings is None:
            paddings = [2, 4, 8]
        if strides is None:
            strides = [2, 4, 8]
    ...
    def forward(self, inputs: Tensor):
        ...
        for i in range(1, self.num_scales):
            feat_up = F.interpolate(
                self.scales[i](inputs), size=inputs.shape[2:], mode=self.unsample_mode
            )
...
class PAPPM(DAPPM):
    ...
    def forward(self, inputs: Tensor):
        ...
        return self.compression(torch.cat([x_, scale_out], dim=1)) + self.shortcut(
            inputs
        )

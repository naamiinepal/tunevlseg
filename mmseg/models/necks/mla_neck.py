...
class MLAModule(nn.Module):
    def __init__(self, in_channels=None, out_channels=256, norm_cfg=None, act_cfg=None):
    ...
    def forward(self, inputs):
        ...
        feat_list = []
        for x, conv in zip(inputs, self.channel_proj, strict=False):
        ...
        out_list = []
        for mid, conv in zip(mid_list, self.feat_extract, strict=False):
...
class MLANeck(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, norm_layer=None, norm_cfg=None, act_cfg=None
    ):
        if norm_layer is None:
            norm_layer = {"type": "LN", "eps": 1e-06, "requires_grad": True}
        super().__init__()

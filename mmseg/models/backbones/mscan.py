...
class StemConv(BaseModule):
    ...
    def __init__(self, in_channels, out_channels, act_cfg=None, norm_cfg=None):
        if norm_cfg is None:
...
class MSCAAttention(BaseModule):
    ...
    def __init__(self, channels, kernel_sizes=None, paddings=None):
        if paddings is None:
        ...
        for i, (kernel_size, padding) in enumerate(
            zip(kernel_sizes[1:], paddings[1:], strict=False)
        ):
            kernel_size_ = [kernel_size, kernel_size[::-1]]
            padding_ = [padding, padding[::-1]]
            conv_name = [f"conv{i}_1", f"conv{i}_2"]
            for i_kernel, i_pad, i_conv in zip(
                kernel_size_, padding_, conv_name, strict=False
            ):
                self.add_module(
                    i_conv,
                    nn.Conv2d(
...
class MSCASpatialAttention(BaseModule):
    ...
    def __init__(
        self,
        in_channels,
        attention_kernel_sizes=None,
        attention_kernel_paddings=None,
        act_cfg=None,
    ):
        if act_cfg is None:
            act_cfg = {"type": "GELU"}
        if attention_kernel_paddings is None:
            attention_kernel_paddings = [2, [0, 3], [0, 5], [0, 10]]
        if attention_kernel_sizes is None:
...
class OverlapPatchEmbed(BaseModule):
    ...
    def __init__(
        self, patch_size=7, stride=4, in_channels=3, embed_dim=768, norm_cfg=None
    ):
        if norm_cfg is None:
            norm_cfg = {"type": "SyncBN", "requires_grad": True}
        super().__init__()

...
class BEiTTransformerEncoderLayer(VisionTransformerEncoderLayer):
    ...
    def __init__(
        ...
        if act_cfg is None:
            act_cfg = {"type": "GELU"}
        attn_cfg.update({"window_size": window_size, "qk_scale": None})
    ...
    def forward(self, x):
        ...
        return x + self.drop_path(self.gamma_2 * self.ffn(self.norm2(x)))
...
class BEiT(BaseModule):
    ...
    def _geometric_sequence_interpolation(self, src_size, dst_size, sequence, num):
        ...
        return torch.cat(new_sequence, dim=-1)

...
class DecathlonDataset(BaseSegDataset):
    ...
    def load_data_list(self) -> list[dict]:
        ...
        raw_data_list = (
            annotations["training"] if not self.test_mode else annotations["test"]
        )
        data_list = []
        for raw_data_info in raw_data_list:
            ...
            if isinstance(raw_data_info, dict):
                data_info = {
                    "img_path": osp.join(self.data_root, raw_data_info["image"][2:])
                }
                data_info["seg_map_path"] = osp.join(
                    self.data_root, raw_data_info["label"][2:]
                )
            else:
                data_info = {"img_path": osp.join(self.data_root, raw_data_info)[2:]}

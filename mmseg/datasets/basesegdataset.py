...
class BaseSegDataset(BaseDataset):
    ...
    def __init__(
        ...
        self._metainfo.update({"palette": updated_palette})
    ...
    @classmethod
    def get_label_map(cls, new_classes: Sequence | None = None) -> dict | None:
    ...
    def _update_palette(self) -> list:
        ...
        if len(palette) == 0:
            ...
            for old_id, new_id in sorted(
                self.label_map.items(), key=operator.itemgetter(1)
            ):
                if new_id != 255:
    ...
    def load_data_list(self) -> list[dict]:
        ...
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            for line in lines:
                img_name = line.strip()
                data_info = {"img_path": osp.join(img_dir, img_name + self.img_suffix)}
            ...
            for img in fileio.list_dir_or_file(
                ...
                data_info = {"img_path": osp.join(img_dir, img)}
                if ann_dir is not None:
                ...
                data_info["seg_fields"] = []
                data_list.append(data_info)
            data_list = sorted(data_list, key=operator.itemgetter("img_path"))
        return data_list
...
class BaseCDDataset(BaseDataset):
    ...
    def __init__(
        ...
        if data_prefix is None:
            data_prefix = {"img_path": "", "img_path2": "", "seg_map_path": ""}
        if pipeline is None:
            pipeline = []
        self.img_suffix = img_suffix

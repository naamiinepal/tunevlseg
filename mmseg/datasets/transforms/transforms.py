...
class ResizeToMultiple(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = mmcv.imresize_to_multiple(
            ...
            self.size_divisor,
            scale_factor=1,
            interpolation=self.interpolation or "bilinear",
        )
...
class RandomCutOut(BaseTransform):
    ...
    def __init__(
        ...
        if seg_fill_in is not None:
            assert (
                isinstance(seg_fill_in, int) and seg_fill_in >= 0 and seg_fill_in <= 255
            )
        self.prob = prob
...
class Resize(MMCV_Resize):
    ...
    def _resize_seg(self, results: dict) -> None:
        ...
        for seg_key in results.get("seg_fields", []):
            if results.get(seg_key) is not None:
                if self.keep_ratio:
...
class ResizeShortestEdge(BaseTransform):
    ...
    def __init__(self, scale: int | tuple[int, int], max_size: int) -> None:
        super().__init__()
        self.scale = scale
...
class BioMedical3DRandomCrop(BaseTransform):
    ...
    def __init__(
        self, crop_shape: int | tuple[int, int, int], keep_foreground: bool = True
    ):
        super().__init__()
...
class BioMedical3DPad(BaseTransform):
    ...
    @staticmethod
    def _to_pad(
        img: np.ndarray, pad_shape: tuple[int, int, int], pad_val: int | float = 0
    ) -> np.ndarray:
        ...
        return np.pad(img, pad_list, mode="constant", constant_values=pad_val)
...
class BioMedical3DRandomFlip(BaseTransform):
    ...
    def __init__(
        ...
        prob: float,
        axes: tuple[int, ...],
        swap_label_pairs: list[tuple[int, int]] | None = None,
    ):
        self.prob = prob
        self.axes = axes

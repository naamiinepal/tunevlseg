...
class InferencerLoader(BaseTransform):
    ...
    def transform(self, single_input: str | np.ndarray | dict) -> dict:
        """Transform function to add image meta information.
        ...
        if isinstance(single_input, str):
            inputs = {"img_path": single_input}
        elif isinstance(single_input, np.ndarray):
            inputs = {"img": single_input}
...
class LoadSingleRSImageFromFile(BaseTransform):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(" f"to_float32={self.to_float32})"

...
class IoUMetric(BaseMetric):
    ...
    def __init__(
        ...
        iou_metrics: list[str] | None = None,
    ...
    def compute_metrics(self, results: list) -> dict[str, float]:
        ...
        # ([A_1, ..., A_n], ..., [D_1, ..., D_n])
        results = tuple(zip(*results, strict=False))
        ...
        for metric in metrics:
            if metric == "mIoU":
                ...
                f_value = torch.tensor(
                    [
                        f_score(x[0], x[1], beta)
                        for x in zip(precision, recall, strict=False)
                    ]
                )

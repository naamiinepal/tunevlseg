...
class SegLocalVisualizer(Visualizer):
    ...
    def _get_center_loc(self, mask: np.ndarray) -> np.ndarray:
        ...
        loc_sort = np.array(sorted(loc.tolist(), key=operator.itemgetter(0, 1)))
        y_list = loc_sort[:, 0]
        unique, _indices, counts = np.unique(
    ...
    def _draw_depth_map(self, image: np.ndarray, depth_map: PixelData) -> np.ndarray:
    ...
    def set_dataset_meta(
        ...
        classes = classes or get_classes(dataset_name)
        palette = palette or get_palette(dataset_name)

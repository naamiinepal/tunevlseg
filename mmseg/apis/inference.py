...
def inference_model(model: BaseSegmentor, img: ImageType) -> SegDataSample | SampleList:
...
def show_result_pyplot(
    ...
    if hasattr(model, "module"):
        model = model.module
    image = mmcv.imread(img, channel_order="rgb") if isinstance(img, str) else img
    if save_dir is not None:
        mkdir_or_exist(save_dir)
    # init visualizer
    visualizer = SegLocalVisualizer(
        vis_backends=[{"type": "LocalVisBackend"}], save_dir=save_dir, alpha=opacity
    )

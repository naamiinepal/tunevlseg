...
class _ParamScheduler:
    ...
    def __init__(
        ...
        if end <= begin:
            raise ValueError(
                "end should be larger than begin, but got" f" begin={begin}, end={end}"
            )
        self.begin = begin
        self.end = end
    ...
    def step(self):
        ...
        if self.begin <= self._global_step < self.end:
            ...
            for i, data in enumerate(
                zip(self.optimizer.param_groups, values, strict=False)
            ):
...
class CosineAnnealingParamScheduler(_ParamScheduler):
    ...
    def _get_value(self) -> list:
        ...
        if self.last_step == 0:
            return [group[self.param_name] for group in self.optimizer.param_groups]
        if (self.last_step - 1 - self.T_max) % (2 * self.T_max) == 0:
            return [
                ...
                * (1 - math.cos(math.pi / self.T_max))
                ...
                for base_value, group in zip(
                    self.base_values, self.optimizer.param_groups, strict=False
                )
            ]
        return [
...
class ReduceOnPlateauParamScheduler(_ParamScheduler):
    ...
    def __init__(
        ...
        if isinstance(min_value, list | tuple):
            if len(min_value) != len(raw_optimizer.param_groups):
                raise ValueError(
                    f"expected {len(raw_optimizer.param_groups)} min_lrs, got {len(min_value)}"
                )
            ...
            if (
                isinstance(optimizer, BaseOptimWrapper)
    ...
    def _get_value(self):
        ...
        return list(starmap(max, zip(values, self.min_values, strict=False)))
    ...
    def _is_better(self, a, best):
        ...
        if self.rule == "less" and self.threshold_rule == "abs":
            return a < best - self.threshold

        if self.rule == "greater" and self.threshold_rule == "rel":
            rel_epsilon = self.threshold + 1.0
            return a > best * rel_epsilon

        # rule == 'greater' and epsilon_mode == 'abs':
        return a > best + self.threshold

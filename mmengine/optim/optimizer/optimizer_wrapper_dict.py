...
class OptimWrapperDict(OptimWrapper):
    ...
    def update_params(  # type: ignore
        ...
        raise NotImplementedError(
            "`update_params` should be called by each " "optimizer separately`"
        )
    ...
    def backward(self, loss: torch.Tensor, **kwargs) -> None:
        ...
        """
        raise NotImplementedError(
            "`backward` should be called by each " "optimizer separately`"
        )
    ...
    def step(self, **kwargs) -> None:
        ...
        implemented either."""
        raise NotImplementedError(
            "`step` should be called by each " "optimizer separately`"
        )
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        ...
        for name, _state_dict in state_dict.items():
            assert name in self.optim_wrappers, (
                f"Mismatched `state_dict`! cannot found {name} in " "OptimWrapperDict"
            )

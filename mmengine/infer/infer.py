...
class InferencerMeta(ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert len(all_kwargs) == (
            len(self.preprocess_kwargs)
            + len(self.forward_kwargs)
            + len(self.visualize_kwargs)
...
class BaseInferencer(metaclass=InferencerMeta):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            raise TypeError(
                "model must be a filepath or any ConfigType"
    ...
    def _load_model_from_metafile(self, model: str) -> tuple[Config, str]:
        ...
        assert self.scope is not None, (
            "scope should be initialized if you want " "to load config from metafile."
        )
        assert self.scope in MODULE2PACKAGE, (
            f"{self.scope} not in {MODULE2PACKAGE}!," "please pass a valid scope."
        )
    ...
    def list_models(scope: str | None = None, patterns: str = r".*"):
        ...
        assert scope in MODULE2PACKAGE, (
            f"{scope} not in {MODULE2PACKAGE}!, please make pass a valid " "scope."
        )

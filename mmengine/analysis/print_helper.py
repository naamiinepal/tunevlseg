...
def _remove_zero_statistics(
    ...
    _force_keep: set[str] = force_keep or set() | {""}
...
def _get_single_child(name: str, statistics: dict[str, dict[str, str]]) -> str | None:
...
def complexity_stats_str(
    flops: FlopAnalyzer, activations: ActivationAnalyzer | None = None
) -> str:
...
def complexity_stats_table(
    flops: FlopAnalyzer,
    max_depth: int = 3,
    activations: ActivationAnalyzer | None = None,
...
def get_model_complexity_info(
    ...
    inputs: torch.Tensor | tuple[torch.Tensor, ...] | tuple[Any, ...] | None = None,
    ...
    if inputs is None:
        ...
        if is_tuple_of(input_shape, int):  # tuple of int, construct one tensor
            ...
            is_tuple_of(one_input_shape, int) for one_input_shape in input_shape

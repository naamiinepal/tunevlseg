...
Handle = Callable[[list[Any], list[Any]], typing.Counter[str] | int]


def get_shape(val: Any) -> list[int] | None:
...
def generic_activation_jit(op_name: str | None = None) -> Handle:
...
def addmm_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
...
def linear_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
    ...
    return prod(input_shapes[0]) * input_shapes[1][0]  # type: ignore


def bmm_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
...
def einsum_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
...
def matmul_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
...
def norm_flop_counter(affine_arg_index: int) -> Handle:
    ...
    def norm_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
        ...
        return prod(input_shape) * (5 if has_affine else 4)  # type: ignore

    return norm_flop_jit


def batchnorm_flop_jit(inputs: list[Any], outputs: list[Any]) -> int | Any:
...
def elementwise_flop_counter(input_scale: float = 1, output_scale: float = 0) -> Handle:
    ...
    def elementwise_flop(inputs: list[Any], outputs: list[Any]) -> int | Any:
        ret = 0
        if input_scale != 0:

...
def _get_scoped_trace_graph(
    ...
    aliases: dict[str | nn.Module, str],
) -> torch._C.Graph:
...
class JitModelAnalysis:
    ...
    def __init__(
        ...
        # of each submodule
        self._aliases: dict[nn.Module | str, str] = self._get_aliases(model)
    ...
    def set_op_handle(self, *args, **kwargs: Handle | None) -> "JitModelAnalysis":
        ...
        for name, handle in zip(args[::2], args[1::2], strict=False):
            kwargs[name] = handle
    ...
    def copy(
        self,
        new_model: nn.Module | None = None,
        new_inputs: None | Tensor | tuple[Tensor, ...] = None,
    ) -> "JitModelAnalysis":
    ...
    def _get_aliases(self, model: nn.Module) -> dict[str | nn.Module, str]:
        aliases = {}
    ...
    def _get_all_ancestors(self, module_name: str) -> set[str]:
        ...
        res = {""}
        res.update(".".join(parts[:k]) for k in range(len(parts) + 1))
        return res

...
class MMFullyShardedDataParallel(FullyShardedDataParallel):
    ...
    def __init__(
        ...
        param_init_fn: str | Callable[[nn.Module], None] | None = None,
        ...
        **kwargs,
        ...
        if not (
            isinstance(sharding_strategy, ShardingStrategy) or sharding_strategy is None
        ):
            raise TypeError(
        ...
        with FUNCTIONS.switch_scope_and_registry(None):
            ...
            if not (auto_wrap_policy is None or callable(auto_wrap_policy)):  # type: ignore
                raise TypeError(
                    "`auto_wrap_policy` should be a str, a "
            ...
            if not (
                isinstance(backward_prefetch, BackwardPrefetch)
            ...
            if not (callable(param_init_fn) or param_init_fn is None):
                raise TypeError(
                    "`param_init_fn` should be a str, a "
                    "callable, a dict or None, but has type "
    ...
    def _get_ignored_params(
        self, module: nn.Module, ignored_parameters: Iterable[str] | Iterable[nn.Module]
    ):
        ...
        if (
            not is_seq_of(ignored_parameters, nn.Parameter)
            and ignored_parameters is not None
        ):
            raise TypeError(
    ...
    def _get_ignored_modules(
        self, module: nn.Module, ignored_modules: Iterable[str] | Iterable[nn.Module]
    ):
    ...
    if digit_version(torch.__version__) < digit_version("2.0.1"):
        ...
        def set_state_dict_type(
            ...
            if optim_state_dict_config_type != type(optim_state_dict_config):
                raise RuntimeError(
                    "Expected optim_state_dict_config of type "

...
class LocalVisBackend(BaseVisBackend):
    ...
    def _dump(self, value_dict: dict, file_path: str, file_format: str) -> None:
        ...
        """
        with open(file_path, "a+", encoding="locale") as f:
    ...
    def _init_env(self):
        ...
        for handler in logger.handlers:
            if handler.stream is None or handler.stream.closed:
                handler.stream = open(handler.baseFilename, "a", encoding="locale")
...
class DVCLiveVisBackend(BaseVisBackend):
    ...
    def _init_env(self):
        ...
        if digit_version(platform.python_version()) < digit_version("3.8"):
            raise RuntimeError(
                "Please use Python 3.8 or higher version " "to use DVCLiveVisBackend."
            )

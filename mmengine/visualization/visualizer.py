...
class Visualizer(ManagerMixin):
    ...
    def __init__(
        ...
        if fig_save_cfg is None:
            fig_save_cfg = {"frameon": False}
        super().__init__(name)
        ...
        if not is_seq_of(vis_backends, (dict, BaseVisBackend)):
            raise TypeError(
                "vis_backends must be a list of dicts or a list "
        ...
        for vis_backend in vis_backends:  # type: ignore
            ...
            save_dir_arg = inspect.signature(
                vis_backend.__class__.__init__
            ).parameters.get("save_dir")
            if (
                ...
                warnings.warn(
                    f"Failed to add {vis_backend.__class__}, "
    ...
    def draw_binary_masks(
        ...
        for binary_mask, color, alpha in zip(
            binary_masks, colors, alphas, strict=False
        ):
    ...
    @master_only
    def add_scalar(
        self, name: str, value: int | float, step: int = 0, **kwargs
    ) -> None:
    ...
    @master_only
    def add_scalars(
        self, scalar_dict: dict, step: int = 0, file_path: str | None = None, **kwargs
    ) -> None:

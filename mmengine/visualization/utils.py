...
def value2list(
    value: Any, valid_type: type | tuple[type, ...], expand_dim: int
) -> list[Any]:
...
def check_type_and_length(
    name: str, value: Any, valid_type: type | tuple[type, ...], valid_length: int
) -> None:
...
def wait_continue(figure, timeout: float = 0, continue_key: str = " ") -> int:
    ...
    while True:
        ...
        if event is None or event.key == continue_key:

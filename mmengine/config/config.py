...
def _lazy2string(cfg_dict, dict_type=None):
    ...
    if isinstance(cfg_dict, tuple | list):
        return type(cfg_dict)(_lazy2string(v, dict_type) for v in cfg_dict)
    if isinstance(cfg_dict, LazyAttr | LazyObject):
...
class Config:
    ...
    def _parse_lazy_import(filename: str) -> tuple[ConfigDict, set]:
        ...
        with open(filename, encoding="utf-8") as f:
            ...
            for key, value in global_dict.items():
                if key.startswith("__") or key == "LazyObject":
                    continue
                ret[key] = value
    ...
    def _dict_to_config_dict(cfg: dict, scope: str | None = None, has_scope=True):
        ...
        if isinstance(cfg, dict):
            if has_scope and "type" in cfg:
                has_scope = False
                if scope is not None and cfg.get("_scope_") is None:
    ...
    def _get_base_files(filename: str) -> list:
        ...
        return base_files if isinstance(base_files, list) else [base_files]
    ...
    def pretty_text(self) -> str:
        ...
        def _format_list_tuple(k, v, use_mapping=False):
            ...
            if use_mapping:
                ...
                attr_str = f"{k!s}={v_str}"
            return _indent(attr_str, indent) + right

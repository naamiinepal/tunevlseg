...
class LogProcessor:
    ...
    def get_log_after_epoch(
        ...
        if "time" not in custom_keys:
            custom_cfg_copy.append(
                {"data_src": "time", "window_size": "epoch", "method_name": "mean"}
            )
        if "data_time" not in custom_keys:
            custom_cfg_copy.append(
                {"data_src": "data_time", "window_size": "epoch", "method_name": "mean"}
            )
    ...
    def _parse_windows_size(
        self, runner, batch_idx: int, custom_cfg: list | None = None
    ) -> list:
    ...
    def _get_iter(self, runner, batch_idx: int) -> int:
        ...
        """
        return batch_idx + 1 if self.by_epoch else runner.iter + 1

...
class Runner:
    ...
    def build_logger(
        self, log_level: int | str = "INFO", log_file: str | None = None, **kwargs
    ) -> MMLogger:
    ...
    def build_visualizer(
        self, visualizer: Visualizer | dict | None = None
    ) -> Visualizer:
    ...
    def wrap_model(
        self, model_wrapper_cfg: dict | None, model: nn.Module
    ...
    def _init_model_weights(self) -> None:
        ...
        if hasattr(model, "init_weights"):
            ...
            for params in model.state_dict().values():
                broadcast(params)

    def scale_lr(
        self, optim_wrapper: OptimWrapper, auto_scale_lr: dict | None = None
    ) -> None:
    ...
    def build_optim_wrapper(
        self, optim_wrapper: Optimizer | OptimWrapper | dict
    ...
    def build_param_scheduler(
        self, scheduler: _ParamScheduler | dict | list
    ) -> ParamSchedulerType:
    ...
    def build_dataloader(
        ...
        if isinstance(sampler_cfg, dict):
            ...
            sampler = DATA_SAMPLERS.build(
                sampler_cfg, default_args={"dataset": dataset, "seed": sampler_seed}
            )
        else:
            # fallback to raise error in dataloader
    ...
    def build_log_processor(self, log_processor: LogProcessor | dict) -> LogProcessor:
        ...
        if not isinstance(log_processor, dict):
            raise TypeError(
    ...
    def register_hook(
        self, hook: Hook | dict, priority: str | int | Priority | None = None
    ) -> None:
    ...
    def _check_scheduler_cfg(
        self, param_scheduler: dict | list | _ParamScheduler | None
    ) -> None:
        ...
        if is_seq_of(param_scheduler, dict):
            ...
            if "type" not in param_scheduler:
                for _param_scheduler in param_scheduler.values():
                    assert isinstance(

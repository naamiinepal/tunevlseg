...
class CheckpointLoader:
    ...
    def _register_scheme(cls, prefixes, loader, force=False):
        ...
        cls._schemes = OrderedDict(
            sorted(cls._schemes.items(), key=operator.itemgetter(0), reverse=True)
        )
...
def load_from_pavi(filename, map_location=None):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        return torch.load(downloaded_file, map_location=map_location)
...
def load_from_mmcls(filename, map_location=None):
    ...
    return _process_mmcls_checkpoint(checkpoint)
...
def _load_checkpoint_to_model(
    model, checkpoint, strict=False, logger=None, revise_keys=None
):
...
def load_checkpoint(
    model, filename, map_location=None, strict=False, logger=None, revise_keys=None
):
...
def find_latest_checkpoint(path: str) -> str | None:
    ...
    if os.path.exists(save_file):
        with open(save_file, encoding="locale") as f:

...
def assert_allclose(
    ...
    Args:
        ...
        msg (Optional[Union[str, Callable]]): Optional error message to use if
            ...
            < 1.6.
    """
    if "parrots" not in TORCH_VERSION and digit_version(TORCH_VERSION) >= digit_version(
        "1.6"
    ):
        _assert_allclose(
            actual, expected, rtol=rtol, atol=atol, equal_nan=equal_nan, msg=msg
        )
    else:
        # torch.testing.assert_allclose has no ``msg`` argument
        # when PyTorch < 1.6
        _assert_allclose(actual, expected, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def assert_dict_contains_subset(
    ...
    for key, value in expected_subset.items():
        if key not in dict_obj or _any(dict_obj[key] != value):
            return False
    return True

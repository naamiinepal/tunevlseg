...
# Modified from https://github.com/pytorch/pytorch/blob/master/torch/testing/_internal/common_distributed.py
import faulthandler
import logging
...
TEST_SKIPS = {
    "backend_unavailable": TestSkip(
        10, "Skipped because distributed backend is not available."
    ),
    "no_cuda": TestSkip(11, "CUDA is not available."),
    "multi-gpu-2": TestSkip(12, "Need at least 2 CUDA device"),
    "generic": TestSkip(
        13,
...
class MultiProcessTestCase(TestCase):
    ...
    def run_test(self, test_name: str, parent_pipe) -> None:
        ...
        try:
            ...
            logger.info(
                f"Process {self.rank} skipping test {test_name} for "
                f"following reason: {se!s}"
            )
        ...
        for rank, pipe in pipes:
            try:
                ...
                if pipe.poll(5):
                    ...
                    logger.error(
                        "Could not retrieve traceback for timed out " f"process: {rank}"
                    )
            except ConnectionError as e:
                logger.error(
        ...
        try:
            while True:
                ...
                if elapsed > self.timeout:
                    ...
                    print(
                        f"Timing out after {self.timeout} seconds and "
    ...
    def _check_return_codes(self, elapsed_time) -> None:
        ...
        # resources.
        self.skipTest(f"Skip test {self._testMethodName} due to " "the program abort")

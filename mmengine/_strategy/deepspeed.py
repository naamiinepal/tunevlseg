...
class DeepSpeedOptimWrapper(BaseOptimWrapper):
    ...
    def zero_grad(self, **kwargs) -> None:
        raise NotImplementedError(
            "DeepSpeedOptimWrapper does not support zero_grad method " "currently."
        )
...
class MMDeepSpeedEngineWrapper:
    ...
    def _cast_inputs_half(self, inputs: list | tuple | dict | None):
        ...
        if isinstance(inputs, list | tuple):
            ...
            for i, v in enumerate(inputs):
                if i in self._inputs_to_half:
                    new_inputs.append(
                        apply_to(v, lambda x: hasattr(x, "to"), lambda x: x.to(dtype))
                    )
                else:
                    new_inputs.append(v)
        ...
        if isinstance(inputs, dict):
            for k, v in inputs.items():
                if k in self._inputs_to_half:
                    inputs[k] = apply_to(
                        v, lambda x: hasattr(x, "to"), lambda x: x.to(dtype)
                    )
            return inputs
        raise TypeError(
            "inputs should be list, tuple or dict, " f"but got {type(inputs)}"
        )
...
class DeepSpeedStrategy(BaseStrategy):
    ...
    def _parse_config(self, config):
        if config is None:
            ...
            with open(config, encoding="locale") as f:
                config = json.load(f)
        return config
    ...
    def _wrap_model(self, model: nn.Module) -> nn.Module:
        ...
        return MMDeepSpeedEngineWrapper(
    ...
    def save_checkpoint(
        ...
        *,
        ...
        state_dict_kwargs = {}
        if digit_version(deepspeed.__version__) >= digit_version("0.13.2"):
            state_dict_kwargs["exclude_frozen_parameters"] = (

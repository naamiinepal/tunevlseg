...
def register_plugins():
    _plugins = inspect.getmembers(
        colo_plugin, lambda x: inspect.isclass(x) and issubclass(x, colo_plugin.Plugin)
    )
...
class ColossalAIOptimWrapper(OptimWrapper):
    ...
    def optim_context(self, model: nn.Module):
        assert isinstance(self.booster, Booster), (
            "Please set the booster attribute before using " "`ColossalAIOptimWrapper`."
        )
...
class ColossalAIModelWrapper:
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(
            f"{self.model_wrapper} and {self.model} has no " f"attribute {name}"
        )
...
class ColossalAIStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        mixed_precision: str | dict | None = None,
        plugin: str = "gemini",
    ...
    def prepare(
        ...
        *,
        ...
        if optim_wrapper is not None:
            self.model, self.optim_wrapper = self._wrap(model, optim_wrapper)  # type: ignore
        else:
        ...
        *,
        ...
        if resume_optimizer:
            self.booster.load_optimizer(
                self.optim_wrapper.optimizer, join_path(filename, self.OPTIMIZER_DIR)
            )
    ...
    def load_checkpoint(
        ...
        *,
        ...
        self.booster.load_model(
            self.model.model_wrapper, join_path(filename, self.MODEL_DIR)
        )
        return _load_checkpoint(osp.join(filename, "meta.pth"))
        ...
        *,
        ...
        if save_optimizer:
            self.booster.save_optimizer(
                self.optim_wrapper.optimizer, checkpoint=optimizer_dir, shard=True
            )
    ...
    def _build_plugin(self, plugin: str | dict):
        if isinstance(plugin, str):
            if plugin == "gemini":
                try:
                    ...
                    raise ValueError(
                        "placement policy must be one of "
                        + f"{list(colo_placement.policies.keys())}"
                    )
            elif plugin == "lowlevel-zero":
                plugin = colo_plugin.LowLevelZeroPlugin()
            else:
                raise ValueError('`plugin` must be "gemini" or ' '"lowlevel-zero"')
        elif isinstance(plugin, dict):
            plugin = PLUGINS.build(plugin)
        else:
            raise ValueError(
                "`plugin` must be dict or str, but got a " f"{type(plugin)} object)"
            )
        return plugin

    def _build_mixed_precision(self, mixed_precision: str | dict):
        if isinstance(mixed_precision, str):
            ...
            raise ValueError(
                "mixed precision should be dict or str, but got "
    ...
    def _wrap(
        self,
        model: nn.Module,
        optim_wrapper: OptimWrapper | None = None,
    ) -> tuple[ColossalAIModelWrapper, ColossalAIOptimWrapper] | ColossalAIModelWrapper:  # type: ignore
    ...
    def _setup_distributed(  # type: ignore
        ...
        **kwargs,
        ...
        init_dist(launcher, backend, init_backend="colossalai", config=self.config)

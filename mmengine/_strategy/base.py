...
class BaseStrategy(ABC):
    ...
    def _scale_lr(self) -> None:
        ...
        real_bs = (
            self.world_size * self.dispatch_kwargs["train_micro_batch_size_per_gpu"]
        )
        ...
        assert isinstance(
            self.optim_wrapper, BaseOptimWrapper
    ...
    def optim_state_dict(self) -> dict:
        ...
        raise TypeError(
            "self.optim_wrapper should be a `BaseOptimWrapper`"
        ...
        if isinstance(self.param_schedulers, dict):
            ...
            for name, schedulers in self.param_schedulers.items():
                for scheduler, ckpt_scheduler in zip(
                    schedulers, state_dict[name], strict=False
                ):

...
def pseudo_collate(data_batch: Sequence) -> Any:
    ...
    if isinstance(data_item, str | bytes):
        return data_batch
    if isinstance(data_item, tuple) and hasattr(data_item, "_fields"):
        # named tuple
        return data_item_type(
            *(pseudo_collate(samples) for samples in zip(*data_batch, strict=False))
        )
...
def default_collate(data_batch: Sequence) -> Any:
    ...
    if isinstance(data_item, tuple) and hasattr(data_item, "_fields"):
        # named_tuple
        return data_item_type(
            *(default_collate(samples) for samples in zip(*data_batch, strict=False))
        )
    if isinstance(data_item, Sequence):
        # check to make sure that the data_itements in batch have
        # consistent size
